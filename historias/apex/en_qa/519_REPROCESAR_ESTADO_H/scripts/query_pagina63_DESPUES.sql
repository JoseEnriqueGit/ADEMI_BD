-- =============================================================================
-- IRD-519: Query de la Pagina 63 - Reportes Generados Automaticamente
-- Estado: DESPUES del cambio (incluye reprocesar para estado H)
-- =============================================================================
WITH
...
JOIN (
    ...
    SELECT
        CASE
            -- IRD-519: Estado H con transicion segun tipo de documento
    ...
    SELECT
        ...
        CASE R.ID_TIPO_DOCUMENTO
            WHEN '204' THEN
                '<a href="' || APEX_UTIL.PREPARE_URL(p_url => (SELECT URL_BASE FROM CONFIG) || '/AcceptanceInsurance?creditNumber=' || IA.PKG_API_PKM.ObtieneParteReferencia(R.CODIGO_REFERENCIA, ':', 3), p_checksum_type => 'PUBLIC_BOOKMARK') || '" target="_blank">Descargar</a>'
            WHEN '218' THEN
                '<a href="' || APEX_UTIL.PREPARE_URL(p_url => (SELECT URL_BASE FROM CONFIG) || '/MiPymeInsurance?creditNumber=' || IA.PKG_API_PKM.ObtieneParteReferencia(R.CODIGO_REFERENCIA, ':', 3), p_checksum_type => 'PUBLIC_BOOKMARK') || '" target="_blank">Descargar</a>'
            WHEN '1850' THEN
                '<a href="' || APEX_UTIL.PREPARE_URL(p_url => (SELECT URL_BASE FROM CONFIG) || '/OptionalInsurance?creditNumber=' || IA.PKG_API_PKM.ObtieneParteReferencia(R.CODIGO_REFERENCIA, ':', 3), p_checksum_type => 'PUBLIC_BOOKMARK') || '" target="_blank">Descargar</a>'
            WHEN '451' THEN
                '<a href="' || APEX_UTIL.PREPARE_URL(p_url => (SELECT URL_BASE FROM CONFIG) || '/Deponente?creditNumber=' || IA.PKG_API_PKM.ObtieneParteReferencia(R.CODIGO_REFERENCIA, ':', 3), p_checksum_type => 'PUBLIC_BOOKMARK') || '" target="_blank">Descargar</a>'
            ELSE
                R.URL_REPORTE
        END AS URL_REPORTE,
        REPLACE(R.NOMBRE_ARCHIVO, ':', '_') AS NOMBRE_ARCHIVO,
