-------------------------------------------------------------------------------
-- EXPLAIN PLAN: Vista PR_V_ENVIO_REPRESTAMOS (v2 - con hints)
-- Ejecutar en DESARROLLO
-------------------------------------------------------------------------------
-- Cambios vs v1:
--   1. /*+ MATERIALIZE */ en CANALES_HABILITADOS para forzar TEMP TABLE
--   2. /*+ INDEX(r PK_REPRESTAMOS) */ en base para forzar PK
-------------------------------------------------------------------------------

EXPLAIN PLAN SET STATEMENT_ID = 'VISTA_NEW_V2' FOR
WITH VALOR_PARAM AS (
...
CANALES_HABILITADOS AS (
    SELECT /*+ MATERIALIZE */ UPPER(TRIM(column_value)) AS val
    FROM TABLE(pr.pr_pkg_represtamos.f_obt_valor_parametros('CANALES_HABILITADOS'))
),
base AS (
    SELECT /*+ INDEX(r PK_REPRESTAMOS) */
           r.id_represtamo,
           r.codigo_empresa,
           r.codigo_cliente,
           r.id_carga_dirigida,
...
-- Ver resultados
SELECT * FROM TABLE(DBMS_XPLAN.DISPLAY(NULL, 'VISTA_NEW_V2', 'ALL'));
