-------------------------------------------------------------------------------
-- EXPLAIN PLAN: Vista PR_V_ENVIO_REPRESTAMOS (ANTES - version original)
-- Ejecutar en DESARROLLO
-------------------------------------------------------------------------------

EXPLAIN PLAN SET STATEMENT_ID = 'VISTA_OLD' FOR
WITH params AS (
  SELECT
    UPPER(TRIM(column_value)) AS val
  FROM
    TABLE(
...
WHERE
  ...
  AND EXISTS (
    SELECT
      1
    FROM
      pr.pr_solicitud_represtamo o
    WHERE
      o.codigo_empresa = r.codigo_empresa
    ...
    OR (
      ...
      AND EXISTS (
        ...
        FROM
          params p
        WHERE
          p.val = 'CANAL_CAMPANA_ESPECIAL'
      )
    )
  );

-- Ver resultados
SELECT * FROM TABLE(DBMS_XPLAN.DISPLAY(NULL, 'VISTA_OLD', 'ALL'));
