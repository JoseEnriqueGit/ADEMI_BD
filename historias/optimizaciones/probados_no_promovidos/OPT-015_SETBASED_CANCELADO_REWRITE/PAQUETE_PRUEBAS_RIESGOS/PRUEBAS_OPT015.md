...
- Archivos necesarios:
  - `body_ANTES_OPT015.sql` (body original)
  - `body_DESPUES_OPT015.sql` (body optimizado)
  - `05_MEDIR_JOB_CANCELADO_DETALLADO.sql` (script de medicion)
- Indice `PR.IDX_GARANTIAS_TIPO_SB` debe existir (verificar paso 1)
...
```
**Screenshot 1a**: Resultado del query de indice
...
1. Abrir `body_ANTES_OPT015.sql` en Toad
...
1. Abrir `05_MEDIR_JOB_CANCELADO_DETALLADO.sql` en Toad
2. Ejecutar con F5
...
1. Abrir `body_DESPUES_OPT015.sql` en Toad
2. Seleccionar todo (Ctrl+A)
3. Compilar (F9)
4. Verificar que no hay errores
...
DROP TABLE JOOGANDO.OPT015_RE_ANTES;
CREATE TABLE JOOGANDO.OPT015_RE_ANTES AS
...
SELECT COUNT(*) FROM JOOGANDO.OPT015_RE_ANTES;
...
Ejecutar `05_MEDIR_JOB_CANCELADO_DETALLADO.sql` y luego:

```sql
-- Guardar resultado DESPUES
DROP TABLE JOOGANDO.OPT015_RESULTADO_DESPUES;
CREATE TABLE JOOGANDO.OPT015_RESULTADO_DESPUES AS
SELECT ID_REPRESTAMO, CODIGO_CLIENTE, NO_CREDITO, ESTADO,
       DIAS_ATRASO, MTO_CREDITO_ACTUAL, OBSERVACIONES, CODIGO_PRECALIFICACION
...
Compilar `body_ANTES_OPT015.sql` (F9), ejecutar `05_MEDIR_JOB_CANCELADO_DETALLADO.sql` y luego:

```sql
-- Guardar resultado ANTES
DROP TABLE JOOGANDO.OPT015_RESULTADO_ANTES;
CREATE TABLE JOOGANDO.OPT015_RESULTADO_ANTES AS
SELECT ID_REPRESTAMO, CODIGO_CLIENTE, NO_CREDITO, ESTADO,
       DIAS_ATRASO, MTO_CREDITO_ACTUAL, OBSERVACIONES, CODIGO_PRECALIFICACION
FROM PR.PR_REPRESTAMOS
WHERE ADICIONADO_POR = USER AND FECHA_ADICION >= TRUNC(SYSDATE);
...
FROM JOOGANDO.OPT015_RESULTADO_DESPUES D
WHERE NOT EXISTS (SELECT 1 FROM JOOGANDO.OPT015_RESULTADO_ANTES A
...
FROM JOOGANDO.OPT015_RESULTADO_ANTES A
WHERE NOT EXISTS (SELECT 1 FROM JOOGANDO.OPT015_RESULTADO_DESPUES D
...
FROM JOOGANDO.OPT015_RESULTADO_ANTES A
JOIN JOOGANDO.OPT015_RESULTADO_DESPUES D
...
```sql
-- Compilar body_DESPUES_OPT015.sql (F9)
...
| # | Archivo | Descripcion |
|---|---------|-------------|
| 1 | `body_ANTES_OPT015.sql` | Body original (sin optimizacion) para compilar como ANTES |
| 2 | `body_DESPUES_OPT015.sql` | Body optimizado (con OPT-015) para compilar como DESPUES |
| 3 | `05_MEDIR_JOB_CANCELADO_DETALLADO.sql` | Script de medicion V$MYSTAT (ejecutar con F5) |
| 4 | `PRUEBAS_OPT015.md` | Esta guia paso a paso |
| 5 | `01_CREAR_INDICE_IDX_GARANTIAS_TIPO_SB.sql` | Script para crear indice requerido (si no existe) |
...
- DBMS Output activado (View > DBMS Output)
- Indice `PR.IDX_GARANTIAS_TIPO_SB` debe existir (ejecutar script #5 si no existe)

### Pasos resumidos
1. Verificar prerequisitos (indice + RE disponibles)
2. Restaurar RE con el UPDATE del Paso 2
3. Compilar `body_ANTES_OPT015.sql` (F9), verificar COUNT OPT-015 = 0
4. Ejecutar `05_MEDIR_JOB_CANCELADO_DETALLADO.sql` (F5), anotar tiempos
5. Restaurar RE de nuevo
6. Compilar `body_DESPUES_OPT015.sql` (F9), verificar COUNT OPT-015 = 10
7. Ejecutar `05_MEDIR_JOB_CANCELADO_DETALLADO.sql` (F5), anotar tiempos
8. (Opcional) Validacion de equivalencia con snapshots (Paso 8 de esta guia)
9. Dejar `body_DESPUES_OPT015.sql` compilado
10. Restaurar RE
