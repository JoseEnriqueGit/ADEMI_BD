-- ============================================================
-- OPT-003 BEFORE: COMMITs dentro de loops en Actualiza_Precalificacion
-- Paquete: PR_PKG_REPRESTAMOS (body.sql, QA)
-- ============================================================

-- LOOP 1: Actualizar_Mto_Credito_Actual (linea ~2704)
-- 2 COMMITs por iteracion
FOR y in Actualizar_Mto_Credito_Actual LOOP
    ...
    COMMIT;     -- <<< COMMIT #1 dentro del loop
    ...
    COMMIT;     -- <<< COMMIT #2 dentro del loop
END LOOP;


-- LOOP 2: PRECALIFICADOS (linea ~2738)
-- 1 COMMIT por iteracion
FOR a IN PRECALIFICADOS LOOP
    UPDATE PR_REPRESTAMOS
     ...
     WHERE rowid = a.id;
    COMMIT;     -- <<< COMMIT dentro del loop
END LOOP;


-- LOOP 3: CUR_FIADOR (linea ~2749)
-- 1 COMMIT por iteracion
