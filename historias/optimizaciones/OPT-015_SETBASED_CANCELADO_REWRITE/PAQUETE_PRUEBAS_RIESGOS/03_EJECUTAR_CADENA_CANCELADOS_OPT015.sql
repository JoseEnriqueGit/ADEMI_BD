-- =============================================================
-- OPT-015 - Ejecuta cadena deterministica para equivalencia
-- Cadena incluida:
--   1. Precalifica_Repre_Cancelado
--   2. Precalifica_Repre_Cancelado_hi
--   3. Actualiza_Precalificacion
--   4. Registro de solicitud y bitacora inicial por ID del run
--   5. Loop final de bitacora/validaciones solo para IDs del run
--
-- NO incluye WORLD_COMPLIANCE ni XCORE para evitar ruido externo.
-- =============================================================
...
BEGIN
    DELETE FROM OPT015_RUN_IDS WHERE RUN_LABEL = v_run_label;
    DELETE FROM OPT015_SNAP_REPRESTAMOS WHERE RUN_LABEL = v_run_label;
    DELETE FROM OPT015_SNAP_SOLICITUD WHERE RUN_LABEL = v_run_label;
    DELETE FROM OPT015_SNAP_CANALES WHERE RUN_LABEL = v_run_label;
    DELETE FROM OPT015_SNAP_OPCIONES WHERE RUN_LABEL = v_run_label;
    DELETE FROM OPT015_SNAP_BITACORA WHERE RUN_LABEL = v_run_label;
    DELETE FROM OPT015_RUN_CONTROL WHERE RUN_LABEL = v_run_label;
    COMMIT;
    ...
    INSERT INTO OPT015_RUN_CONTROL (
        RUN_LABEL,
        TEST_USER,
        STARTED_AT,
        LOTE_ORIGINAL,
    ...
    SELECT v_run_label,
           ...
           B.VALOR_ORIGINAL,
           v_lote_actual,
           'Cadena deterministica sin WORLD_COMPLIANCE ni XCORE'
      FROM OPT015_PARAM_BACKUP B
    ...
    DBMS_OUTPUT.PUT_LINE('RE insertados por cancelados: ' || v_re_insert);

    IF v_re_insert = 0 THEN
        UPDATE OPT015_RUN_CONTROL
    ...
    INSERT INTO OPT015_SNAP_REPRESTAMOS
    ...
    INSERT INTO OPT015_SNAP_SOLICITUD
    ...
    INSERT INTO OPT015_SNAP_CANALES
    ...
    INSERT INTO OPT015_SNAP_OPCIONES
    ...
    INSERT INTO OPT015_SNAP_BITACORA
    ...
    SELECT COUNT(*) INTO v_solicitudes FROM OPT015_SNAP_SOLICITUD WHERE RUN_LABEL = v_run_label;
    SELECT COUNT(*) INTO v_canales FROM OPT015_SNAP_CANALES WHERE RUN_LABEL = v_run_label;
    SELECT COUNT(*) INTO v_opciones FROM OPT015_SNAP_OPCIONES WHERE RUN_LABEL = v_run_label;
    SELECT COUNT(*) INTO v_bitacora FROM OPT015_SNAP_BITACORA WHERE RUN_LABEL = v_run_label;
