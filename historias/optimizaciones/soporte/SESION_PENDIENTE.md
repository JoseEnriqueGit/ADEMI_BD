# Sesion Pendiente â€” Continuacion de optimizaciones OPT
...
**Entorno**: DESARROLLO (JOOGANDO@ADMQA1_19C) â€” QA estaba bloqueado por un job activo (ORA-04021).
...
**Historia Jira**: IRD-519 â€” "Optimizacion de rendimiento del job de precalificacion de cancelados"
...
**Resultado**: Job total 23 min â†’ 11.3 min (**-51%**). Pasos 5+6: 856 seg â†’ 378 seg (**-56%**)
...
3. **A.0.1 - Verificacion funciones PEP/NEGRA (COMPLETADO)**:
   - `esta_en_lista_pep` (L.314 body PA): SELECT INTO a `lista_pep` â€” read-only
   - `esta_en_lista_negra` (L.129 body PA): SELECT INTO a `lista_negra` â€” read-only
...
6. **A.1 - Explain Plans Q1-Q12 (COMPLETADO)**:
   ...
   - IDX_GARANTIAS_TIPO_SB aparece en Q1/Q2 â€” NOT EXISTS inline usa el indice
   - Sin FULL TABLE SCAN en tablas grandes
   - Fase A aprobada

7. **Fase B - Modificacion del body.sql (COMPLETADO)**:
   - **Cambio 1 (Cursor)**: `F_TIENE_GARANTIA(a.no_credito) = 0` reemplazado por NOT EXISTS inline con JOIN a PR_CREDITOS â†’ PR_GARANTIAS_X_CREDITO â†’ PR_GARANTIAS. PEP/NEGRA eliminados del cursor.
...
11. **Commit**: `02c8df3` â€” incluye body.sql modificado, body rollback, explain plans, HANDOFF, PA.P_DATOS_PERSONA, README actualizado.
...
- **Resultado**: 24.2 min â†’ 14.2 min (**-41%**) solo con 4 indices
- **Entorno**: DESARROLLO (ADMQA1 / bmadev0004) â€” QA estaba ocupado por otro desarrollador
- **Detalle**: Ver `historias/optimizaciones/diagnosticos/OPT-014_INDICES_MEDICION_REAL/README.md`
- **Indices creados en DESARROLLO**: Los 4 indices quedaron creados y VALID

### Investigacion pasos 5-6 (CERRADA para indices)
- Se probo 5to indice en PA_DETALLADO_DE08 â†’ descartado (empeoro paso 5)
- Causa raiz: CPU/PL/SQL (funciones por fila, context switching), no I/O
- La solucion requiere cambios de codigo, no indices adicionales â†’ resuelto por OPT-015

---

## Estado actual de DESARROLLO (ADMQA1)

- **Body compilado**: OPT-015 (optimizado) â€” verificado con `SELECT COUNT(*) FROM ALL_SOURCE WHERE TEXT LIKE '%OPT-015%'` = 10
...
- **Resultados**: 15.3 min â†’ 10.4 min (-32%)
...
### 2. Propuestas de hardcodeo â€” 3 cursores del job mensual (cost total 11,698 â†’ 62)
...
| `historias/optimizaciones/soporte/PENDIENTES.md` | SQLs pendientes del Quest |
| `historias/optimizaciones/soporte/MAPA_JOBS.md` | Mapa completo de los 5 jobs |
| `historias/optimizaciones/probados_no_promovidos/OPT-015_SETBASED_CANCELADO_REWRITE/HANDOFF.md` | Contexto completo OPT-015 |
| `historias/optimizaciones/probados_no_promovidos/OPT-015_SETBASED_CANCELADO_REWRITE/body_ANTES_OPT015.sql` | Body rollback |
| `historias/optimizaciones/probados_no_promovidos/OPT-015_SETBASED_CANCELADO_REWRITE/body_DESPUES_OPT015.sql` | Body optimizado |
