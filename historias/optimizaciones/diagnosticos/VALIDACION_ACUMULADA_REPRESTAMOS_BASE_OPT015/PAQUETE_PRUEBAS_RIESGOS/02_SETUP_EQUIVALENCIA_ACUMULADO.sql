-- =============================================================
-- ACUMULADO - Setup de prueba rigurosa de equivalencia
...
BEGIN
    ensure_table(
        'OPTACUM_PARAM_BACKUP',
        q'[
            CREATE TABLE OPTACUM_PARAM_BACKUP (
    ...
    ensure_table(
        'OPTACUM_RUN_CONTROL',
        q'[
            CREATE TABLE OPTACUM_RUN_CONTROL (
    ...
    ensure_table(
        'OPTACUM_SNAP_REPRESTAMOS',
        q'[
            CREATE TABLE OPTACUM_SNAP_REPRESTAMOS AS
            SELECT
    ...
    ensure_table(
        'OPTACUM_SNAP_SOLICITUD',
        q'[
            CREATE TABLE OPTACUM_SNAP_SOLICITUD AS
            SELECT
                CAST(NULL AS VARCHAR2(30)) RUN_LABEL,
                CAST(NULL AS TIMESTAMP)    CAPTURED_AT,
                S.*
    ...
    ensure_table(
        'OPTACUM_SNAP_CANALES',
        q'[
            CREATE TABLE OPTACUM_SNAP_CANALES AS
            SELECT
                CAST(NULL AS VARCHAR2(30)) RUN_LABEL,
                CAST(NULL AS TIMESTAMP)    CAPTURED_AT,
                C.*
    ...
    ensure_table(
        'OPTACUM_SNAP_OPCIONES',
        q'[
            CREATE TABLE OPTACUM_SNAP_OPCIONES AS
            SELECT
                CAST(NULL AS VARCHAR2(30)) RUN_LABEL,
                CAST(NULL AS TIMESTAMP)    CAPTURED_AT,
                O.*
    ...
    ensure_table(
        'OPTACUM_SNAP_BITACORA',
        q'[
            CREATE TABLE OPTACUM_SNAP_BITACORA AS
            SELECT
                CAST(NULL AS VARCHAR2(30)) RUN_LABEL,
                CAST(NULL AS TIMESTAMP)    CAPTURED_AT,
                B.*
...
MERGE INTO OPTACUM_PARAM_BACKUP B
USING (
...
PROMPT
PROMPT Setup completado. Si falta alguno de los 6 indices o RE_HOY_USUARIO > 0, limpiar antes de correr ANTES.
