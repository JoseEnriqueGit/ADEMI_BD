-- =====================================================================
-- Script de medicion DETALLADA: cadena acumulada de represtamos
-- Mide cada sub-procedimiento individualmente con V$MYSTAT
-- Ejecutar con F5 en Toad con DBMS Output activado
-- Cadena basada en el paquete acumulado preparado desde DESARROLLO
-- =====================================================================
...
BEGIN
    DBMS_OUTPUT.PUT_LINE('=== MEDICION DETALLADA CADENA ACUMULADA REPRESTAMOS ===');
    ...
    -- 12. Loop final cacheado: F_Existe_Solicitudes + F_Existe_Canales + F_EXISTE_CREDITO + Bitacora
    marca_inicio;
    FOR A IN (SELECT ID_REPRESTAMO, ESTADO, XCORE_GLOBAL FROM PR.PR_REPRESTAMOS WHERE ESTADO = 'RE') LOOP
        ...
        IF v_tiene_solicitud
           AND v_tiene_canales
           AND v_tiene_credito THEN
    ...
    END LOOP;
    COMMIT;
    marca_fin('12. Loop Bitacora+Validaciones (cache)');
