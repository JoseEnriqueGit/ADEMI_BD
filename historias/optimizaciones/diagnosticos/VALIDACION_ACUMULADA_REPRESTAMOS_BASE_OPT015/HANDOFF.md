# ACUMULADO sobre base OPT-015 - Handoff de validacion

## Actualizacion 2026-04-17 | Estado real de equivalencia acumulada

- **Ejecuciones completadas**: ya se corrieron `04_LIMPIAR_RUN_EQUIVALENCIA_ACUMULADO.sql`, `03_EJECUTAR_CADENA_EQUIVALENCIA_ACUMULADO.sql`, `06_COMPARAR_EQUIVALENCIA_ACUMULADO.sql` y `06A_DIAGNOSTICAR_EXCLUSIVOS_ACUMULADO.sql` sobre `ANTES` y `DESPUES`.
- **Ajuste aplicado a soporte**: se corrigio `06_COMPARAR_EQUIVALENCIA_ACUMULADO.sql` para evitar ambiguedad en la seccion `4B` de `PR_SOLICITUD_REPRESTAMO`, expandiendo columnas explicitas en vez de `S.*`.
- **Resultado confirmado**:
  - `ANTES`: `RE=149`, `SOLICITUDES=149`, `CANALES=100`, `OPCIONES=0`, `BITACORA=365`
  - `DESPUES`: `RE=149`, `SOLICITUDES=149`, `CANALES=101`, `OPCIONES=0`, `BITACORA=363`
  - En llaves coincidentes no hay diferencias en `PR_REPRESTAMOS`, `PR_SOLICITUD_REPRESTAMO`, `PR_CANALES_REPRESTAMO`, `PR_OPCIONES_REPRESTAMO` ni en `PR_BITACORA_REPRESTAMO` normalizada.
- **Desfase pendiente**: siguen existiendo solo `4` llaves exclusivas de negocio en `PR_REPRESTAMOS`.
  - Solo en `ANTES`: `(1, 2996640, 1510274, 28-NOV-25, RCS)` y `(1, 3429942, 1715018, 28-NOV-25, CP)`
  - Solo en `DESPUES`: `(1, 1135399, 1768146, 28-NOV-25, NP)` y `(1, 1688259, 1767236, 28-NOV-25, CP)`
- **Lectura actual**: el problema ya no parece estar en hijos ni en bitacora final; el desvio se esta originando en la seleccion del universo de `PR_REPRESTAMOS` antes de capturar `OPTACUM_RUN_IDS`.
- **Siguiente paso recomendado**: instrumentar una corrida por etapa para snapshot despues de cada una de las 5 precalificaciones, aislar exactamente en cual procedimiento aparecen o desaparecen esas 4 llaves y seguir desde ahi.

## Actualizacion 2026-04-16 | Handoff para otra PC / sesion Codex

- **Estado actual**: se dejo preparado un paquete de pruebas mas riguroso en `PAQUETE_PRUEBAS_RIESGOS` para validar equivalencia funcional del acumulado en `DESARROLLO` desde una PC con acceso a Oracle.
- **Importante**: en esta sesion **no** se ejecutaron scripts contra la BD. Solo se preparo documentacion, scripts SQL y metodologia.
- **Objetivo de la siguiente sesion**: correr la validacion rigurosa de equivalencia del acumulado y luego, si pasa, cerrar evidencia con tiempos ANTES/DESPUES.

### Archivos nuevos preparados

- `PAQUETE_PRUEBAS_RIESGOS/02_SETUP_EQUIVALENCIA_ACUMULADO.sql`
- `PAQUETE_PRUEBAS_RIESGOS/03_EJECUTAR_CADENA_EQUIVALENCIA_ACUMULADO.sql`
- `PAQUETE_PRUEBAS_RIESGOS/04_LIMPIAR_RUN_EQUIVALENCIA_ACUMULADO.sql`
- `PAQUETE_PRUEBAS_RIESGOS/06_COMPARAR_EQUIVALENCIA_ACUMULADO.sql`
- `PAQUETE_PRUEBAS_RIESGOS/07_RESTAURAR_PARAMETRO_LOTE_ACUMULADO.sql`
- `PAQUETE_PRUEBAS_RIESGOS/PRUEBAS_ACUMULADO_RIGUROSA.md`
...
- Se excluyen `PVALIDA_WORLD_COMPLIANCE` y `PVALIDA_XCORE` como criterio de equivalencia porque meten dependencias externas y ruido ajeno al objetivo del acumulado.

### Flujo recomendado para la siguiente sesion

1. Confirmar acceso a `DESARROLLO`.
2. Ejecutar `01_CREAR_INDICES_SOPORTE_ACUMULADO.sql` solo si falta alguno de los indices.
3. Ejecutar `02_SETUP_EQUIVALENCIA_ACUMULADO.sql`.
4. Compilar `body_ANTES_BASE_OPT015.sql`.
5. Ejecutar `03_EJECUTAR_CADENA_EQUIVALENCIA_ACUMULADO.sql` con `RUN_LABEL = ANTES`.
6. Ejecutar `05_MEDIR_JOB_REPRESTAMOS_ACUMULADO.sql` para tiempos ANTES.
7. Ejecutar `04_LIMPIAR_RUN_EQUIVALENCIA_ACUMULADO.sql` con `RUN_LABEL = ANTES`.
8. Compilar `body_DESPUES_ACUMULADO.sql`.
9. Ejecutar `03_EJECUTAR_CADENA_EQUIVALENCIA_ACUMULADO.sql` con `RUN_LABEL = DESPUES`.
10. Ejecutar `05_MEDIR_JOB_REPRESTAMOS_ACUMULADO.sql` para tiempos DESPUES.
11. Ejecutar `06_COMPARAR_EQUIVALENCIA_ACUMULADO.sql`.
12. Ejecutar `07_RESTAURAR_PARAMETRO_LOTE_ACUMULADO.sql`.
13. Si se desea limpiar los datos funcionales del segundo run, ejecutar `04_LIMPIAR_RUN_EQUIVALENCIA_ACUMULADO.sql` con `RUN_LABEL = DESPUES`.
...
- Salida final de `03_EJECUTAR_CADENA_EQUIVALENCIA_ACUMULADO.sql` y de `06_COMPARAR_EQUIVALENCIA_ACUMULADO.sql`.
...
- `body_ANTES_BASE_OPT015.sql` â€” body original para revertir
- `body_DESPUES_ACUMULADO.sql` â€” body con cambios OPT-015
...
| `historias/optimizaciones/soporte/scripts_medicion/05_MEDIR_JOB_REPRESTAMOS_ACUMULADO.sql` | Script medicion |
