CREATE OR REPLACE PACKAGE BODY PR.PR_PKG_REPRESTAMOS IS
   ...
   PROCEDURE Precalifica_Represtamo  IS 
      ...
      -- Se valida que sea a una sola firma        
        ...
        -- OPT-010: inline NOT EXISTS preservando la semantica de F_TIENE_GARANTIA
       AND NOT EXISTS (
            SELECT 1
            FROM PR_CREDITOS cr
            JOIN PR_GARANTIAS_X_CREDITO gxc ON gxc.codigo_empresa = cr.codigo_empresa
 ...
 PROCEDURE Precalifica_Carga_Dirigida IS
  ...
  PROCEDURE Precalifica_Campana_Especiales IS
   ...
   --ACTUALIZA EL MONTO CREDITO ACTUAL
          ...
          -- OPT-004: loop row-by-row reemplazado por UPDATEs set-based
          UPDATE PR.PR_REPRESTAMOS R
          SET R.MTO_CREDITO_ACTUAL = (SELECT D.monto_desembolsado
                                        FROM PA.PA_DETALLADO_DE08 D
                                       WHERE D.FUENTE         = 'PR'
