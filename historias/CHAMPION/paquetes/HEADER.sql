CREATE OR REPLACE PACKAGE PR.PR_PKG_REPRESTAMOS IS
    ...
    PROCEDURE Precalifica_Represtamo(pIDAPLICACION IN OUT NUMBER);  
    PROCEDURE Precalifica_Repre_Cancelado(pIDAPLICACION IN OUT NUMBER);
    PROCEDURE Precalifica_Repre_Cancelado_hi(pIDAPLICACION IN OUT NUMBER);
    PROCEDURE Precalifica_Represtamo_fiadores(pIDAPLICACION IN OUT NUMBER);
    PROCEDURE Precalifica_Represtamo_fiadores_hi(pIDAPLICACION IN OUT NUMBER);
    PROCEDURE Precalifica_Carga_Dirigida;
    PROCEDURE Precalifica_Campana_Especiales;         
    PROCEDURE Actualiza_Precalificacion(pIDAPLICACION IN OUT NUMBER);   
    PROCEDURE Actualiza_Preca_Dirigida;
    PROCEDURE Actualiza_Preca_Campana_Especiale; 
    PROCEDURE Actualiza_XCORE_CUSTOM(pIDAPLICACION IN OUT NUMBER);
    PROCEDURE ACTUALIZA_XCORE_DIRIGIDA;
    PROCEDURE ACTUALIZA_XCORE_CAMPANA_ESPECIAL;
    PROCEDURE PVALIDA_XCORE(pIDAPLICACION IN OUT NUMBER);   
    PROCEDURE PVALIDA_WORLD_COMPLIANCE(pIDAPLICACION IN OUT NUMBER);
...
PROCEDURE P_Desactivar_Activar_FrontEnd(pIdReprestamo     IN     VARCHAR2,
  ...
  FUNCTION F_Obtener_plazo(pIdReprestamo IN NUMBER, pMtoSeleccionado IN NUMBER)
   ...
   PROCEDURE P_Generar_reporte_deponente(p_No_Credito       IN     NUMBER,
                                                         ...
                                                         pError             IN OUT VARCHAR2);  
   PROCEDURE P_REGISTRO_SOLICITUD(pIDAPLICACION IN OUT NUMBER);
