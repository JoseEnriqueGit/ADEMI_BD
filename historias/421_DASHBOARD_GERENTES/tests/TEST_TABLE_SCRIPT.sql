WITH params AS (
    SELECT  TO_DATE('01/08/2024', 'DD/MM/YYYY')                 AS fecha_ini,
            ADD_MONTHS(TO_DATE('01/09/2024', 'DD/MM/YYYY'), 1)  AS fecha_fin
    FROM    dual
),
LatestBitacora AS (
...
SELECT
        ...
        LB.FECHA_ADICION                                                      AS "Fecha_Ultimo_Estado",
        (TRUNC(R.FECHA_PROCESO) + CAST(PR.PR_PKG_REPRESTAMOS.F_OBT_PARAMETRO_REPRESTAMO('DIA_CADUCA_LINK') AS NUMBER))
                                                                               AS "Fecha_Vencimiento_Link",
        ...
        CASE WHEN S.EMAIL IS NULL THEN 'No' ELSE 'Si' END                      AS "Correo_Electronico",
        CASE
        ...
        -- Columnas de Sucursal y Oficial
        ...
        LB.ADICIONADO_POR                                                      AS "Atendido_Por"
FROM
        params                pr
JOIN    PA.EMPLEADOS          P      ON 1 = 1
JOIN    PA.AGENCIA            A_EMP  ON TO_NUMBER(P.COD_EMPRESA)        = A_EMP.COD_EMPRESA
...
JOIN    PR.PR_REPRESTAMOS      R     ON S.ID_REPRESTAMO = R.ID_REPRESTAMO
                                     AND S.CODIGO_EMPRESA = R.CODIGO_EMPRESA
JOIN    LatestBitacora         LB    ON S.ID_REPRESTAMO = LB.ID_REPRESTAMO
                                     AND LB.rn = 1
LEFT JOIN HasCryStatus         HCS   ON S.ID_REPRESTAMO = HCS.ID_REPRESTAMO
LEFT JOIN PR.PR_CREDITOS       C     ON R.NO_CREDITO    = C.NO_CREDITO
                                     AND R.CODIGO_EMPRESA = C.CODIGO_EMPRESA
LEFT JOIN PR.PR_CREDITOS_HI    H     ON R.NO_CREDITO    = H.NO_CREDITO
                                     AND R.CODIGO_EMPRESA = H.CODIGO_EMPRESA
LEFT JOIN PA.AGENCIA           AG    ON AG.COD_EMPRESA  = R.CODIGO_EMPRESA
                                     AND AG.COD_AGENCIA = NVL(C.CODIGO_AGENCIA, H.CODIGO_AGENCIA)
LEFT JOIN PR.PR_TIPO_CREDITO   TC    ON TC.TIPO_CREDITO = NVL(S.TIPO_CREDITO,
                                                               NVL(C.TIPO_CREDITO, H.TIPO_CREDITO))
                                     AND TC.CODIGO_EMPRESA = R.CODIGO_EMPRESA
LEFT JOIN PR.PR_CANALES_REPRESTAMO A_CANAL
                                     ON R.ID_REPRESTAMO = A_CANAL.ID_REPRESTAMO
                                     AND A_CANAL.CANAL   = 1
WHERE
    SUBSTR(P.EMAIL1, 1, INSTR(P.EMAIL1, '@') - 1) = 'MAMATOS'
    AND P.ID_EMPLEADO = A_EMP.GERENTE
    AND P.ESTA_ACTIVO = 'S'
    AND TRUNC(R.FECHA_PROCESO) >= pr.fecha_ini
    AND TRUNC(R.FECHA_PROCESO) <  pr.fecha_fin
    AND NOT EXISTS (
            SELECT 1
