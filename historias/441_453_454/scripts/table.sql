...
base_table AS (
    ...
    FROM
        ...
        /* 
        ---------------------------------------------------------------------
        -- CORRECCIÓN: Se cambia a LEFT JOIN para evitar que se eliminen filas
        -- si la información de la agencia o el gerente no se encuentra.
        ---------------------------------------------------------------------
        */
    ...
    WHERE
        /* excluir cargas dirigidas finalizadas el mismo día */
        NOT EXISTS (
            SELECT 1
...
WHERE
    ...
    AND ( :P133_TIPO_DESEMBOLSO IS NULL
          OR "Tipo de Desembolso" = :P133_TIPO_DESEMBOLSO
    )
ORDER BY "FECHA_PROCESO" DESC, "ID Représtamo" DESC;