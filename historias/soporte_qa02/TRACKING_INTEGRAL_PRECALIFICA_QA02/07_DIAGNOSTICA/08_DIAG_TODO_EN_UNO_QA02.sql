...
-- Gating: cada INSERT solo entra si TRACK_PRECALIFICA_DETALLE_CURSOR='S'.
-- Mira la pestania Script Output: deben salir 5 lineas 'N rows created'
-- + 'Commit complete', y al final el conteo por flujo (6 filas:
-- los 5 flujos + TOTAL).
-- =====================================================================

PROMPT ===== Limpieza de filas de wrappers de la ultima ejecucion =====

DELETE FROM PR.PR_JOB_PRECALIFICA_FILTRO_TRACK
 WHERE tipo_medicion = 'DIAGNOSTICA'
   AND flujo <> 'TOTAL'
   AND id_ejecucion = (SELECT id_ejecucion
...
PROMPT ===== Wrapper 01: 01_DIAG_PRECALIFICA_REPRESTAMO_QA02.sql =====
...
PROMPT ===== Wrapper 02: 02_DIAG_PRECALIFICA_REPRE_CANCELADO_QA02.sql =====
...
WITH params AS (
    SELECT (SELECT MAX(p.fecha_corte)
             ...
             WHERE p.fuente = 'PR'
               AND p.fecha_corte < (
                    SELECT MAX(p2.fecha_corte)
                      FROM PA.PA_DETALLADO_DE08 p2
                     WHERE p2.fuente = 'PR'
               )) fecha_corte,
           TO_NUMBER(PR.PR_PKG_REPRESTAMOS.F_OBT_PARAMETRO_REPRESTAMO('LOTE_DE_CARAGA_REPRESTAMO')) lote,
...
pasos_cursor AS (
    ...
    SELECT 11, 'No incumple regla de sola firma' FROM dual UNION ALL
    SELECT 12, 'F_TIENE_GARANTIA = 0' FROM dual UNION ALL
...
PROMPT ===== Wrapper 03: 03_DIAG_PRECALIFICA_REPRE_CANCELADO_HI_QA02.sql =====
...
PROMPT ===== Wrapper 04: 04_DIAG_PRECALIFICA_REPRESTAMO_FIADORES_QA02.sql =====
...
conteos_cursor AS (
    ...
    SELECT 12, COUNT(*) FROM nacionalidad_documento_ok UNION ALL
    SELECT 13, COUNT(*) FROM sin_represtamo_en_proceso UNION ALL
    SELECT 14, COUNT(*) FROM con_fiador UNION ALL
    SELECT 15, COUNT(*) FROM sin_garantia UNION ALL
    SELECT 16, COUNT(*) FROM no_pep UNION ALL
    SELECT 17, COUNT(*) FROM no_lista_negra UNION ALL
    SELECT 18, COUNT(*) FROM dos_creditos_cancelados
),
pasos_cursor AS (
    SELECT 0 orden, 'BASE: PR_CREDITOS' filtro FROM dual UNION ALL
    SELECT 1, 'TIPO_CREDITO existe en PR_TIPO_CREDITO_REPRESTAMO' FROM dual UNION ALL
    ...
    SELECT 12, 'Nacionalidad y tipo documento validos' FROM dual UNION ALL
    SELECT 13, 'Sin represtamo en estados no reproceso' FROM dual UNION ALL
    SELECT 14, 'Tiene fiador/aval diferente al cliente' FROM dual UNION ALL
    SELECT 15, 'F_TIENE_GARANTIA = 0' FROM dual UNION ALL
    SELECT 16, 'No esta en listas PEP' FROM dual UNION ALL
    SELECT 17, 'No esta en lista negra' FROM dual UNION ALL
    SELECT 18, 'Cliente con exactamente dos creditos cancelados' FROM dual
),
resumen_cursor AS (
    SELECT 1 tipo_orden,
           'SECUENCIAL_CURSOR' tipo_medicion,
...
PROMPT ===== Wrapper 05: 05_DIAG_PRECALIFICA_REPRESTAMO_FIADORES_HI_QA02.sql =====
...
PROMPT ===== Verificacion: filas DIAGNOSTICA por flujo (esperado 5 flujos + TOTAL) =====

SELECT f.flujo, COUNT(*) filas
