...
Scripts preparados, **pendientes de ejecutar en Toad/QA02**. Orden de ejecucion:

| # | Script | Crea |
|---|---|---|
| 0 | `00_VALIDAR_EXISTENCIA_OBJETOS_QA02.sql` | Solo lectura. Confirma version, nombres libres, tipo de `ID_REPRESTAMO`, tablespace `PR_IDX` y columnas de `PA_PARAMETROS_MVP`. |
| 1 | `01_CREATE_PR_JOB_PRECALIFICA_FILTRO_TRACK_QA02.sql` | Tabla `PR.PR_JOB_PRECALIFICA_FILTRO_TRACK` (Capa B) + secuencia `SEQ_PR_JOB_PRECAL_FILTRO` + indices `IX_PRECAL_FILTRO_CONSULTA` e `IX_PRECAL_FILTRO_FECHA`. |
| 2 | `02_CREATE_PR_JOB_PRECALIFICA_CANDIDATO_TRACK_QA02.sql` | Tabla `PR.PR_JOB_PRECALIFICA_CANDIDATO_TRACK` (Capa C) + indice `IX_PRECAL_CAND_FECHA`. |
| 3 | `03_PARAMETROS_TRACK_PRECALIFICA_QA02.sql` | Parametros `TRACK_PRECALIFICA_*` en `PA.PA_PARAMETROS_MVP` (`CODIGO_MVP='REPRESTAMOS'`). |
