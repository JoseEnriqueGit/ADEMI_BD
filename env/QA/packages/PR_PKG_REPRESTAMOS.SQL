create
or replace PACKAGE BODY PR_PKG_REPRESTAMOS IS PROCEDURE Precalifica_Represtamo IS --- => Condicones a tomar en cuenta para la Precalificaci¿n de Cr¿ditos
...
CURSOR CREDITOS_PROCESAR (P_FECHA_CORTE DATE) IS -- AUDIT TAG: PRE_RE
-- AUDIT SELECT START  
select
    ...
    0 PIN,
    --lpad(PA.PKG_NOTIFICACIONES.GENERAR_PIN_RANDOM(100,999999),6, '0') PIN,
    ...
    --   TO_NUMBER(OBT_PARAMETROS('1', 'PR', 'INTENTOS_IDENT')) INTENTOS_IDENTIFICACION,    ---DEBE INSERTAR ESTE PARAMETRO 
    ...
    NULL XCORE_GLOBAL,
    --NVL(PA_PKG_CONSULTA_DATACREDITO.OBTIENE_XCORE(PA.OBT_IDENTIFICACION_PERSONA(  a.codigo_cliente,'1')),0) XCORE_GLOBAL,
    NULL XCORE_CUSTOM,
    --NVL(PA_PKG_CONSULTA_DATACREDITO.OBTIENE_XCORE_CUSTOM(PA.OBT_IDENTIFICACION_PERSONA( A.codigo_cliente,'1')),0) XCORE_CUSTOM
    NULL ID_CARGA_DIRIGIDA,
    NULL ID_CAMPANA_ESPECIALES,
    'N' ES_FIADOR
FROM
    PR.PR_CREDITOS a
    JOIN PA.pa_detallado_De08 b ON b.no_credito = a.no_credito
    JOIN PR.PR_tipo_credito_REPRESTAMO c ON a.tipo_credito = c.tipo_credito
    AND c.tipo_credito = b.tipo_credito
WHERE
    1 = 1
    /*F01: Lote*/
    AND ROWNUM <= TO_NUMBER(
        PR.PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('LOTE_DE_CARAGA_REPRESTAMO')
    ...
    AND (
        EXISTS (
            ...
            FROM
                TABLE(
                    PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros('PERIODOS_CUOTA')
                ) subq
            WHERE
                a.CODIGO_PERIODO_CUOTA = subq.COLUMN_VALUE
        )
        OR NOT EXISTS (
            SELECT
                1
    ...
    /*F03: Fecha corte*/
    AND b.fecha_corte = P_FECHA_CORTE --AND  b.no_credito = a.no_credito
    /*F04: Fuente PR*/
    AND b.fuente = 'PR'
    /*F05: Carga S*/
    AND c.CARGA = 'S'
    /*F06: Mora <= parámetro*/
    AND b.dias_atraso <= PR.PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('PRECAL_MORA_MAYOR_PR')
    /*F07: Clasificacion SIB*/
    AND b.CALIFICA_CLIENTE IN (
        select
            COLUMN_VALUE
        FROM
            TABLE(
                PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('CLASIFICACION_SIB')
            )
    )
    /*F08: Monto Capital Pagado*/
    AND (
        (
            b.MTO_BALANCE_CAPITAL / CASE
                WHEN b.MONTO_DESEMBOLSADO = 0 then b.MONTO_CREDITO
                else b.MONTO_DESEMBOLSADO
            END
        ) * 100
    ) <= 100 - TO_NUMBER(
        PR.PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('CAPITAL_PAGADO')
    )
    /*F09: Obtener empresa*/
    and a.codigo_empresa = PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
    /*F10: Desembolso max 6 meses*/
    and not exists (
        SELECT
            1
        FROM
            PR.PR_CREDITOS C
        WHERE
            C.CODIGO_EMPRESA = a.CODIGO_EMPRESA
            AND C.NO_CREDITO != a.NO_CREDITO
            AND C.CODIGO_CLIENTE = a.CODIGO_CLIENTE
            AND C.F_PRIMER_DESEMBOLSO > ADD_MONTHS(
                SYSDATE,
                - PR.PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('MESES_MAX_X_DESEMBOLSO')
            ...
            AND C.ESTADO IN (
                ...
                FROM
                    TABLE(
                        PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_CREDITOS')
                    )
            )
    )
    /*F11: Creditos en estado E*/
    AND not exists (
        SELECT
            1
        FROM
    ...
    AND exists (
        ...
        FROM
            PA.PERSONAS a
        WHERE
            COD_PERSONA = cast(a.codigo_cliente as varchar2(15))
    ...
    AND EXISTS (
        ...
        WHERE
            ...
            AND COD_PAIS IN (
                ...
                FROM
                    TABLE(
                        PR.PR_PKG_REPRESTAMOS.F_OBT_VALOR_PARAMETROS ('NACIONALIDAD')
                    )
            )
            AND COD_TIPO_ID IN (
                SELECT
                ...
                FROM
                    TABLE(
                        PR.PR_PKG_REPRESTAMOS.F_OBT_VALOR_PARAMETROS ('TIPO_DOCUMENTO')
                    )
            )
    ) --  se valida que no exista el mismo crédito con un proceso iniciado de représtamo 
    /*F14: Estados No Reproceso*/
    AND not exists (
        select
            1
        from
        ...
        where
            ...
            and estado in (
                ...
                FROM
                    TABLE(
                        PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_NO_REPROCESO')
                    )
    ...
    /*F15: Clientes Sola Firma*/
    AND not EXISTS (
        select
            1 --a.no_credito---, a.codigo_cliente, b.codigo_aval_repre
        from
            PR.PR_CREDITOS a1,
            PR.PR_AVAL_REPRE_X_CREDITO b
        where
            a1.codigo_empresa = 1
    ...
    /*F16: Tiene Garantia*/
    AND PR.PR_PKG_REPRESTAMOS.F_TIENE_GARANTIA(a.no_credito) = 0 -- Se valida que los clientes no esten en lista PEP
    /*F17: Validar lista pep*/
    AND PR.PR_PKG_REPRESTAMOS.F_Validar_Listas_PEP (1, a.codigo_cliente) = 0 -- Se valida que los clientes no esten en lista NEGRA
    /*F18: Validar lista negras*/
    AND PR.PR_PKG_REPRESTAMOS.F_Validar_Lista_NEGRA(1, a.codigo_cliente) = 0 -- AUDIT SELECT END
;
...
v_del_mancomunado NUMBER := 0;

v_del_edad NUMBER := 0;

v_survivors_final NUMBER := 0;

v_campana NUMBER := TO_NUMBER(TO_CHAR(SYSDATE, 'YYYYMM'));
...
v_atraso_30 := TO_NUMBER(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('PRECAL_DIA_ATRASO_TC')
);
...
BEGIN
SELECT
    P.FECHA_PROCESO INTO v_fecha_proceso
FROM
    PR.PR_REPRESTAMOS P
WHERE
    P.FECHA_PROCESO IS NOT NULL
    AND ROWNUM = 1;

EXCEPTION
WHEN NO_DATA_FOUND THEN v_fecha_proceso := TRUNC(SYSDATE) -30;
...
BEGIN v_fecha_proceso := v_fecha_proceso + TO_NUMBER(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('PRECAL_DIAS_PROCESAR')
);

IF v_fecha_proceso > TRUNC(SYSDATE) THEN --PR_REPRESTAMOS.ACTUALIZA_PRECALIFICACION;
...
SELECT
    MAX (P.FECHA_CORTE) INTO v_fecha_corte
FROM
    PA_DETALLADO_DE08 P
WHERE
    P.FUENTE = 'PR'
...
DBMS_OUTPUT.PUT_LINE ('Antes de entrar al SP de Auditoria.');

BEGIN PR_AUDIT_TRACK(
    p_empresa = > 1,
    p_id_campana = > TO_NUMBER(TO_CHAR(SYSDATE, 'YYYYMM')),
    p_SP_name = > 'Precalifica_Represtamo',
    p_tag = > 'PRE_RE',
    p_bind_date = > v_fecha_corte,
    p_bind_num = > v_atraso_30,
    p_object_name = > 'PR_PKG_REPRESTAMOS',
    p_object_type = > 'PACKAGE BODY'
);

END;

DBMS_OUTPUT.PUT_LINE ('Despues de entrar al SP de Auditoria.');
...
LOOP VCREDITOS_PROCESAR.DELETE;

FETCH CREDITOS_PROCESAR BULK COLLECT INTO VCREDITOS_PROCESAR
LIMIT
    100;
...
INSERT INTO
    PR.PR_REPRESTAMOS
VALUES
    VCREDITOS_PROCESAR (i);

-- TRACK: insertados en lote
v_insertados_batch := VCREDITOS_PROCESAR.COUNT;

PR_AUDIT_PUSH_STEP(
    v_emp,
    v_campana,
    'Precalifica_Represtamo',
    'I00 - INSERTADOS en lote',
    v_insertados_batch
);

-- 7 - Excluir cr¿ditos con atraso mayor a 45 d¿as
-- 7 - en los ¿ltimos 6 meses = v_dias_180 
FORALL x IN 1..VCREDITOS_PROCESAR.COUNT -- Se actualiza el campo DIAS_ATRASO en la Tabla PR_REPRESTAMOS
...
SET
    Y.DIAS_ATRASO = (
        ...
        WHERE
            ...
            AND D.CODIGO_CLIENTE = VCREDITOS_PROCESAR(x).CODIGO_CLIENTE --  AND D.DIAS_ATRASO      >= v_mayor_45 
    )
WHERE
    y.CODIGO_EMPRESA = VCREDITOS_PROCESAR(x).CODIGO_EMPRESA
...
-- TRACK: cuántos pasaron a X3 en este batch
v_desc_x3 := 0;

FOR i IN 1..VCREDITOS_PROCESAR.COUNT LOOP v_desc_x3 := v_desc_x3 + NVL(SQL % BULK_ROWCOUNT(i), 0);

END LOOP;

IF v_desc_x3 > 0 THEN PR_AUDIT_PUSH_STEP(
    v_emp,
    v_campana,
    'Precalifica_Represtamo',
    'D19 - DESCARTADOS por TC (X3)',
    v_desc_x3
);
...
-- TRACK: cuántos pasaron a X1
v_desc_x1 := 0;

FOR i IN 1..VCREDITOS_PROCESAR.COUNT LOOP v_desc_x1 := v_desc_x1 + NVL(SQL % BULK_ROWCOUNT(i), 0);

END LOOP;

IF v_desc_x1 > 0 THEN PR_AUDIT_PUSH_STEP(
    v_emp,
    v_campana,
    'Precalifica_Represtamo',
    'D20 - DESCARTADOS por desembolsos recientes (X1)',
    v_desc_x1
);
...
EXIT
WHEN CREDITOS_PROCESAR % NOTFOUND;
...
WHERE
    P.DIAS_ATRASO > OBT_PARAMETROS('1', 'PR', 'PRECAL_MORA_MAYOR_PR')
    AND P.ESTADO = 'RE';

v_desc_x2 := SQL % ROWCOUNT;

IF v_desc_x2 > 0 THEN PR_AUDIT_PUSH_STEP(
    v_emp,
    v_campana,
    'Precalifica_Represtamo',
    'D21 - DESCARTADOS por mora 6m (X2)',
    v_desc_x2
);
...
v_del_mancomunado := SQL % ROWCOUNT;

IF v_del_mancomunado > 0 THEN PR_AUDIT_PUSH_STEP(
    v_emp,
    v_campana,
    'Precalifica_Represtamo',
    'D22 - DESCARTADOS por mancomunado (delete)',
    v_del_mancomunado
);
...
v_del_edad := SQL % ROWCOUNT;

IF v_del_edad > 0 THEN PR_AUDIT_PUSH_STEP(
    v_emp,
    v_campana,
    'Precalifica_Represtamo',
    'D23 - DESCARTADOS por edad (delete)',
    v_del_edad
);
...
BEGIN pMensaje := 'ERROR CON EL STORE PROCEDURE PRECALIFICA_REPRESTAMO';

setError(
    pProgramUnit = > 'Precalifica_Represtamo',
    pPieceCodeName = > NULL,
...
PROCEDURE Precalifica_Repre_Cancelado IS CURSOR CREDITOS_PROCESAR (P_FECHA_CORTE DATE) IS
select
    a.codigo_empresa CODIGO_EMPRESA,
    pr_pkg_represtamos.f_genera_secuencia ID_REPRESTAMO,
    a.codigo_cliente,
...
WHERE
    ROWNUM <= TO_NUMBER(
        PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('LOTE_DE_CARAGA_REPRESTAMO')
    )
    and a.tipo_credito = c.tipo_credito
    AND (
        EXISTS (
    ...
    and A.F_CANCELACION = (
        ...
        FROM
            PR_CREDITOS d
        WHERE
            d.F_CANCELACION >= SYSDATE - TO_NUMBER(
                PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('DIAS_CANCELACION')
            )
    ...
    and not exists (
        ...
        WHERE
            ...
            AND C.F_PRIMER_DESEMBOLSO > ADD_MONTHS(
                SYSDATE,
                - PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('MESES_MAX_X_DESEMBOLSO')
            ) -- 9 - Excluir cliente que tengas prestamos desembolsados de los últimos 6 meses
            AND C.ESTADO IN (
                select
                    COLUMN_VALUE
...
WHERE
    R.CODIGO_EMPRESA = VCREDITOS_PROCESAR(y).CODIGO_EMPRESA
...
BEGIN pMensaje := 'ERROR CON EL STORE PROCEDURE PRECALIFICA_REPRE_CANCELADO';
...
PROCEDURE Precalifica_Repre_Cancelado_hi IS CURSOR CREDITOS_PROCESAR (P_FECHA_CORTE DATE) IS
select
    a.codigo_empresa CODIGO_EMPRESA,
    pr_pkg_represtamos.f_genera_secuencia ID_REPRESTAMO,
    a.codigo_cliente,
...
WHERE
    ...
    AND PR.PR_PKG_REPRESTAMOS.F_TIENE_GARANTIA_HISTORICO(a.no_credito) = 0 -- Se valida que los clientes no esten en lista PEP
    AND PR.PR_PKG_REPRESTAMOS.F_Validar_Listas_PEP (1, a.codigo_cliente) = 0 -- Se valida que los clientes no esten en lista NEGRA
    AND PR.PR_PKG_REPRESTAMOS.F_Validar_Lista_NEGRA(1, a.codigo_cliente) = 0;
...
BEGIN pMensaje := 'ERROR CON EL STORE PROCEDURE PRECALIFICA_REPRE_CANCELADO_HI';
...
PROCEDURE Precalifica_Represtamo_fiadores IS CURSOR CREDITOS_PROCESAR (P_FECHA_CORTE DATE) IS
select
    a.codigo_empresa CODIGO_EMPRESA,
    pr_pkg_represtamos.f_genera_secuencia ID_REPRESTAMO,
    a.codigo_cliente,
...
WHERE
    ...
    AND (
        ...
        PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('CAPITAL_PAGADO')
    )
    and a.codigo_empresa = PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
    and not exists (
        SELECT
    ...
    AND EXISTS (
        ...
        FROM
            PR_CREDITOS C2
        WHERE
            C2.CODIGO_CLIENTE = a.CODIGO_CLIENTE
            AND C2.ESTADO = 'C'
        GROUP BY
            C2.CODIGO_CLIENTE
        HAVING
...
BEGIN pMensaje := 'ERROR CON EL STORE PROCEDURE Precalifica_Represtamo_fiadores';

setError(
    pProgramUnit = > 'Precalifica_Represtamo_fiadores',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE Precalifica_Represtamo_fiadores_hi IS CURSOR CREDITOS_PROCESAR (P_FECHA_CORTE DATE) IS
select
    a.codigo_empresa CODIGO_EMPRESA,
    pr_pkg_represtamos.f_genera_secuencia ID_REPRESTAMO,
    a.codigo_cliente,
...
BEGIN pMensaje := 'ERROR CON EL STORE PROCEDURE Precalifica_Represtamo_fiadores_hi';
...
PROCEDURE Precalifica_Carga_Dirigida IS CURSOR CREDITOS_PROCESAR(P_FECHA_CORTE DATE, P_ESTADO VARCHAR) IS
SELECT
    A.CODIGO_EMPRESA CODIGO_EMPRESA,
    PR_PKG_REPRESTAMOS.F_GENERA_SECUENCIA ID_REPRESTAMO,
    A.CODIGO_CLIENTE,
    ...
    NULL XCORE_GLOBAL,
    NULL XCORE_CUSTOM,
    A.ID_CARGA_DIRECCIONADA,
    --PR_PKG_REPRESTAMOS.F_GENERA_SECUENCIA_CARGA_DIRIGIDA  ID_CARGA_DIRIGIDA 
    NULL ID_CAMPANA_ESPECIALES,
    'N' ES_FIADOR
FROM
    PR.PR_CARGA_DIRECCIONADA A
WHERE
    A.ESTADO = P_ESTADO;
...
BEGIN -- Para obtener la fecha m¿xima anterior
SELECT
    MAX (P.FECHA_CORTE) INTO v_fecha_corte
FROM
    PA_DETALLADO_DE08 P
...
-- Se valida que sea a una sola firma   
SELECT
    COUNT(*) INTO v_Sola_Firma
FROM
    PR_CREDITOS A1,
    PR_AVAL_REPRE_X_CREDITO B
WHERE
    A1.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
    AND A1.NO_CREDITO = A.NO_CREDITO
    AND B.CODIGO_EMPRESA = A1.CODIGO_EMPRESA
    AND B.NO_CREDITO = A1.NO_CREDITO
    AND B.CODIGO_AVAL_REPRE != A1.CODIGO_CLIENTE
...
SELECT
    COUNT(*) INTO V_CREDITOS_ESTADO_E
FROM
    PR_CREDITOS C
WHERE
    C.CODIGO_EMPRESA = A.CODIGO_EMPRESA
...
SELECT
    COUNT(*) INTO v_Creditos_Activo_represtamos
FROM
    PR_REPRESTAMOS
WHERE
    CODIGO_EMPRESA = A.CODIGO_EMPRESA
...
SELECT
    COUNT(*) INTO v_Creditos_Valido
FROM
    PR_TIPO_CREDITO_REPRESTAMO R
    JOIN PR_CREDITOS C ON C.TIPO_CREDITO = R.TIPO_CREDITO
WHERE
    C.NO_CREDITO = A.NO_CREDITO;
...
SELECT
    MAX(D.DIAS_ATRASO) into v_Dias_atraso
FROM
    PA.PA_DETALLADO_DE08 D
WHERE
    D.FUENTE = 'PR'
    AND D.FECHA_CORTE >= ADD_MONTHS(
        (
            SELECT
                MAX (P.FECHA_CORTE)
            FROM
...
WHERE
    ...
    AND (
        (
            (
                D.MTO_BALANCE_CAPITAL /CASE
                    WHEN D.MONTO_DESEMBOLSADO = 0 then D.MONTO_CREDITO
                    else D.MONTO_DESEMBOLSADO
                END
            ) * 100
        ) <= 100 - TO_NUMBER(
            PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('CAPITAL_PAGADO_CARGA_DIRIGIDA')
        )
    );
...
WHERE
    ...
    AND D.DIAS_ATRASO >= TO_NUMBER(
        PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('PRECAL_DIA_ATRASO_TC')
    );

IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_PERSONA_FISICA_CARGADIRIGIDA') = 'S' THEN IF v_Persona_Fisica = 0 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este cliente no es Físico',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este cliente no tiene Nacionalidad Dominicana',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_FIADORES_CARGADIRIGIDA') = 'S' THEN IF v_Sola_Firma = 0 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este cliente no es a una sola firma o no tiene 2 creditos cancelados',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
-- Se valida que los clientes no tengan no garantes    
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_TIENE_GARANTE_CARGADIRIGIDA') = 'S' THEN IF PR.PR_PKG_REPRESTAMOS.F_TIENE_GARANTIA(A.no_credito) = 1 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este cliente tiene garante',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este cliente es mayor a la edad valida',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_CREDITO_ESTADO_E_CARGADIRIGIDA') = 'S' THEN IF v_Creditos_Estado_E = 1 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este cliente tiene un Crédito con estado E',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
DBMS_OUTPUT.PUT_LINE (
    'v_Creditos_Estado_E = ' || v_Creditos_Estado_E
);
...
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_REPRESTAMO_ACTIVO_CARGADIRIGIDA') = 'S' THEN IF v_Creditos_Activo_represtamos > 0 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este credito tiene un représtamo activo',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
DBMS_OUTPUT.PUT_LINE (
    'v_Creditos_Activo_represtamos = ' || v_Creditos_Activo_represtamos
);
...
--Validar que el tipo de credito sea valido
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_CREDITO_VALIDO_CARGADIRIGIDA') = 'S' THEN IF v_Creditos_Valido = 0 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este Cliente no tiene un tipo de Crédito valido en Represtamo Digital',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
--Se valida que no tenga un credito activo
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_CREDITO_ACTIVO_CARGADIRIGIDA') = 'S' THEN IF v_Creditos_Activo > 0 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este Cliente tiene un Crédito activo',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_DIAS_ATRASO_CARGADIRIGIDA') = 'S' THEN IF v_Dias_atraso >= PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('PRECAL_MORA_MAYOR_PR') THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este Cliente tiene días de atraso mayor al parametrizado',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_CAPITAL_PAGADO_CARGADIRIGIDA') = 'S' THEN IF v_Capital_Pagado = 0 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este Crédito no tiene suficiente capital pagado',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
DBMS_OUTPUT.PUT_LINE (
    'v_Creditos_capital_pagado = ' || v_Capital_Pagado
);
...
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_DIAS_ATRASO_TC_CARGADIRIGIDA') = 'S' THEN IF v_atraso_tc > 0 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este Cliente tiene Tarjetas de Crédito con atraso mayor al permitido',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
DBMS_OUTPUT.PUT_LINE (
    'v_Creditos_atraso_tc = ' || v_Creditos_monto_valido
);
...
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_LISTA_PEPS_CARGADIRIGIDA') = 'S' THEN IF PR.PR_PKG_REPRESTAMOS.F_Validar_Listas_PEP (1, a.codigo_cliente) = 1 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este Cliente esta en lista PEP',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_LISTA_NEGRA_CARGADIRIGIDA') = 'S' THEN IF PR.PR_PKG_REPRESTAMOS.F_Validar_Lista_NEGRA(1, a.codigo_cliente) = 1 THEN
UPDATE
    PR.PR_CARGA_DIRECCIONADA
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este Cliente en lista NEGRA',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
BEGIN setError(
    pProgramUnit = > 'Precalifica_Carga_Dirigida',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE Precalifica_Campana_Especiales IS CURSOR CREDITOS_PROCESAR(P_FECHA_CORTE DATE, P_ESTADO VARCHAR) IS
SELECT
    A.CODIGO_EMPRESA CODIGO_EMPRESA,
    PR_PKG_REPRESTAMOS.F_GENERA_SECUENCIA ID_REPRESTAMO,
    A.CODIGO_CLIENTE,
    ...
    NULL XCORE_CUSTOM,
    NULL ID_CARGA_DIRIGIDA,
    A.ID_CAMPANA_ESPECIALES,
    --PR_PKG_REPRESTAMOS.F_GENERA_SECUENCIA_CARGA_DIRIGIDA  ID_CARGA_DIRIGIDA 
    'N' ES_FIADOR
FROM
    PR.PR_CAMPANA_ESPECIALES A
WHERE
    A.ESTADO = P_ESTADO;
...
Select
    COUNT(*) INTO v_TIPO_CREDITO
FROM
    PR.PR_REPRESTAMO_CAMPANA_DET C
WHERE
    C.TIPO_CREDITO_ORIGEN = (
        SELECT
            TIPO_CREDITO
        FROM
            (
...
DBMS_OUTPUT.PUT_LINE ('DIAS DE ATRASO campana' || v_Dias_atraso);
...
WHERE
    d.F_CANCELACION >= SYSDATE - TO_NUMBER(
        PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('DIAS_CANCELACION_CAMPANA_ESPECIAL')
    )
    AND d.F_CANCELACION <= SYSDATE
    AND d.NO_CREDITO = a.NO_CREDITO
    AND d.ESTADO = 'C';

IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_TIPO_CREDITO_VALIDO_CAMPANA') = 'S' THEN IF v_TIPO_CREDITO = 0 THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Este Cliente no tiene un tipo de Crédito valido en esta campaña',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = A.NO_CREDITO
...
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_PERSONA_FISICA_CAMPANA') = 'S' THEN IF v_Persona_Fisica = 0 THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
...
-- Se valida que sea a una sola firma  
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_FIADORES_CAMPANA') = 'S' THEN IF v_Sola_Firma = 0 THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
...
-- Valida que no tenga un credito con estado E
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_CREDITO_ESTADO_E_CAMPANA') = 'S' THEN IF v_Creditos_Estado_E = 1 THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
...
--Se valida represtamo activo     
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_REPRESTAMO_ACTIVO_CAMPANA') = 'S' THEN IF v_Creditos_Activo_represtamos > 0 THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
...
--Validar que el tipo de credito sea valido     
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_CREDITO_VALIDO_CAMPANA') = 'S' THEN IF v_Creditos_Valido = 0 THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
...
--Se valida que no tenga un credito activo
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_CREDITO_ACTIVO_CAMPANA') = 'S' THEN IF v_Creditos_Activo > 0 THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
...
/*IF  v_Creditos_monto_valido = 0 THEN
 ...
 END IF; */
--Se valida dias atraso
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_DIAS_ATRASO_CAMPANA') = 'S' THEN IF v_Dias_atraso >= PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('PRECAL_MORA_MAYOR_PR') THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
...
/* IF  v_Capital_Pagado =0  THEN
 ...
 END IF;*/
--Se valida atraso en tc
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_DIAS_ATRASO_TC_CAMPANA') = 'S' THEN IF v_atraso_tc > 0 THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
...
-- Se valida que los clientes no esten en lista PEP
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_LISTA_PEPS_CAMPANA') = 'S' THEN IF PR.PR_PKG_REPRESTAMOS.F_Validar_Listas_PEP (1, a.codigo_cliente) = 1 THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
...
-- Se valida que los clientes no esten en lista NEGRA
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_LISTA_NEGRA_CAMPANA') = 'S' THEN IF PR.PR_PKG_REPRESTAMOS.F_Validar_Lista_NEGRA(1, a.codigo_cliente) = 1 THEN
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'E',
...
BEGIN setError(
    pProgramUnit = > 'Precalifica_Campana_Especiales',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE Actualiza_Precalificacion IS -- QUERY DE LOS CREDITOS PRECALIFICADOS
...
WHERE
    a.codigo_empresa = vCodigoEmpresa
    AND A.ID_REPRESTAMO = A.ID_REPRESTAMO + 0
    AND a.ESTADO = 'RE'
    AND a.DIAS_ATRASO BETWEEN P.DESDE
    AND HASTA
...
SELECT
    B.ROWID ID,
    b.id_represtamo,
    NVL(A.CLASIFICACION, 'NULA') CLASIFICACION
FROM
    PA_DE08_SIB A,
    PR_REPRESTAMOS B
WHERE
...
FOR a IN PRECALIFICADOS LOOP --IF a.CODIGO_REPRESTAMO != 'NI' THEN
UPDATE
    PR_REPRESTAMOS
SET
    codigo_precalificacion = a.CODIGO_REPRESTAMO,
...
FOR a in CUR_FIADOR LOOP -- Validar si el cliente tiene un fiador
SELECT
    COUNT(1) INTO v_fiador_exist
FROM
    PR_CREDITOS a1
    JOIN PR_AVAL_REPRE_X_CREDITO b ON a1.codigo_empresa = b.codigo_empresa
...
SELECT
    COUNT(1) INTO v_dos_prestamos_cancelados
FROM
    (
        SELECT
            1
...
IF v_fiador_exist > 0
AND v_dos_prestamos_cancelados > 0
AND a.CODIGO_PRECALIFICACION != 01 THEN vEstado := 'RSB';

vComentario := ' RECHAZO: Cliente no muy bueno con FIADOR ';

DBMS_OUTPUT.PUT_LINE(
    'Antes de generar bitaciora de ' || a.id_represtamo || ' a ' || vEstado
);

PR_PKG_REPRESTAMOS.p_generar_bitacora(
    a.id_represtamo,
    NULL,
    vEstado,
    NULL,
...
DECLARE CURSOR CUR_SIN_PRECALIFICACION IS
SELECT
    B.ROWID ID,
    b.ID_REPRESTAMO,
    B.CODIGO_CLIENTE
...
DBMS_OUTPUT.PUT_LINE(
    'La persona está en la lista OFAC.' || A.CODIGO_CLIENTE
);

ELSE DBMS_OUTPUT.PUT_LINE(
    'La persona no está en la lista OFAC.' || A.CODIGO_CLIENTE
);
    ...
    (
        WITH CLASIFICACIONES AS (
            ...
            where
                ...
                AND CODIGO_PARAMETRO = 'CLASIFICACION_SIB'
        )
        SELECT
            regexp_substr (VALOR, '[^,]+', 1, level) CLASIFICACION
        FROM
            CLASIFICACIONES connect by level <= length(VALOR) - length(replace(VALOR, ',')) + 1
    ) T1
WHERE
    T1.CLASIFICACION = a.clasificacion;
...
--Cambio el estado del detalle de la bitacora
--PR.PR_PKG_TRAZABILIDAD.PR_ACTUALIZAR_BITACORA_DET (pIDAPLICACION, 'ENPROCESO', 90, 'EN PROCESO', pMensaje );     
FOR a in CUR_DE05_SIB LOOP PR_PKG_REPRESTAMOS.p_generar_bitacora(
    a.id_represtamo,
    NULL,
    'RCS',
    NULL,
    'RECHAZO: Cedula: ' || a.cedula || ' con castigo en ' || a.entidad,
    USER
);
...
BEGIN pMensaje := 'ERROR CON EL STORE PROCEDURE ACTUALIZA_PRECALIFICACION';

setError(
    pProgramUnit = > 'Actualiza_Precalificacion',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE Actualiza_Preca_Dirigida IS -- QUERY DE LOS CREDITOS PRECALIFICADOS
CURSOR Actualizar_Mto_Credito_Actual IS
SELECT
    R.ID_REPRESTAMO,
    NO_CREDITO,
...
FROM
    PR.PR_REPRESTAMOS a
    LEFT JOIN PR.PR_CODIGOS_REPRESTAMO P ON P.CODIGO_EMPRESA = a.CODIGO_EMPRESA
    AND a.DIAS_ATRASO BETWEEN P.DESDE
    AND HASTA
WHERE
    a.ESTADO = 'RE'
    AND a.codigo_empresa = vCodigoEmpresa;
...
SELECT
    COUNT(*) into vcontador
FROM
    PR_REPRESTAMOS
WHERE
    ESTADO = 'RSB'
    AND NO_CREDITO = y.NO_CREDITO;
...
IF vClasificaion = 1 THEN vEstado := 'CLS';

vComentario := 'Cliente en clasificacion: ' || a.clasificacion;

ELSIF a.clasificacion = 'NULA'
OR vClasificaion != 1 THEN -- Cliente sin clasificación
IF PR.PR_PKG_REPRESTAMOS.f_obt_parametro_represtamo('VALIDAR_CLASIFICACION_SIB_CARGADIRIGIDA') = 'S' THEN vEstado := 'RSB';

vComentario := 'Cliente sin clasificación y parámetro activo';

-- Se marca como finalizado en la campaña
UPDATE
    PR.PR_CAMPANA_ESPECIALES
SET
    ESTADO = 'F',
...
vComentario := 'Cliente sin clasificación, pero parámetro deshabilitado';
...
FOR a in CUR_DE05_SIB LOOP --CASTIGADOS SIB
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_CASTIGADOS_SIB_CARGADIRIGIDA') = 'S' THEN PR_PKG_REPRESTAMOS.p_generar_bitacora(
    a.id_represtamo,
    NULL,
    'RCS',
    NULL,
    'Cedula: ' || a.cedula || ' con castigo en ' || a.entidad,
    USER
);
...
WHERE
    ...
    AND CODIGO_PARAMETRO IN (
        ...
        'VALIDAR_CASTIGADOS_SIB_CARGADIRIGIDA'
    );
...
BEGIN setError(
    pProgramUnit = > 'Actualiza_Preca_Dirigida',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE Actualiza_Preca_Campana_Especiale IS CURSOR Actualizar_Mto_Credito_Actual IS
SELECT
    R.ID_REPRESTAMO,
    NO_CREDITO,
    CODIGO_CLIENTE
...
ELSIF a.clasificacion = 'NULA'
OR vClasificaion != 1 THEN -- Cliente sin clasificación
IF PR.PR_PKG_REPRESTAMOS.f_obt_parametro_represtamo('VALIDAR_CLASIFICACION_SIB_CAMPANA') = 'S' THEN vEstado := 'RSB';
...
/*ELSE
 -- Cliente con clasificación no válida (pero no nula)
 vEstado := 'RSB';
 vComentario := 'Cliente en clasificacion: ' || a.clasificacion;
...
BEGIN setError(
    pProgramUnit = > 'Actualiza_Preca_Campana_Especiale',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE Actualiza_XCORE_CUSTOM IS --Represtamos En estado RE sin xcore ademi
...
WHERE
    ...
    AND NOT EXISTS(
        ...
        WHERE
            CODIGO_ESTADO = 'CLS'
            AND ID_REPRESTAMO = R.ID_REPRESTAMO
    );
...
WHERE
    ...
    AND ROWNUM <= TO_NUMBER(
        PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('LOTE_PROCESO_XCORE')
    );
...
/*BEGIN
 ...
 WHEN NO_DATA_FOUND THEN
 --SI NO SE ENCUENTRA NINGUN REGISTRO CREARA UNO NUEVO
 PR.PR_PKG_TRAZABILIDAD.PR_CREAR_BITACORA_DET ( 'RD_CARGA_PRECALIFICACION', 'RD_CARGA.ACTUALIZA_XCORE_CUSTOM', 'INICIADO', pMensaje ); 
 ...
 PR.PR_PKG_TRAZABILIDAD.PR_ACTUALIZAR_BITACORA_DET (pIDAPLICACION, 'ENPROCESO', 20, 'EN PROCESO', pMensaje );
 END IF;*/
Vlote_Proceso_Xcore := TO_NUMBER(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('LOTE_PROCESO_XCORE')
);

BEGIN
SELECT
    COUNT(*) INTO vTotal_Carga
FROM
    PR.PR_REPRESTAMOS A
WHERE
    A.ESTADO = 'RE';
...
FOR i IN 1..vCantidad_Procesar LOOP FOR A IN CUR_UPDATE_XCORE LOOP --Cambio el estado del detalle de la bitacora
--PR.PR_PKG_TRAZABILIDAD.PR_ACTUALIZAR_BITACORA_DET (pIDAPLICACION, 'ENPROCESO', 60, 'EN PROCESO', pMensaje ); 
BEGIN xcore := 750;

--NVL(TRIM(JSON_VALUE( PA.PA_PKG_CONSULTA_DATACREDITO.CONSULTAR_JSON(PA.OBT_IDENTIFICACION_PERSONA(a.codigo_cliente,'1') ,'C','COMPATIBILIDAD',NULL,NULL,NULL,NULL,0), '$.respuesta.valor.applicants.primaryConsumer.interconnectResponse.Xcore_PD12M_ALL_PC_NC_Global.Xcore')),0);
--NVL(PA_PKG_CONSULTA_DATACREDITO.OBTIENE_XCORE(PA.OBT_IDENTIFICACION_PERSONA(  a.codigo_cliente,'1')),0);
IF xcore IS NULL THEN xcore := 0;
...
BEGIN pMensaje := 'ERROR CON EL STORE PROCEDURE ACTUALIZA_XCORE_CUSTOM';

setError(
    pProgramUnit = > 'Actualiza_XCORE_CUSTOM',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
DBMS_OUTPUT.PUT_LINE (
    'DBMS_UTILITY.FORMAT_ERROR_BACKTRACE 2 = ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE
);
...
BEGIN Vlote_Proceso_Xcore := TO_NUMBER(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('LOTE_PROCESO_XCORE')
);
...
FOR A IN VALIDACION_CLASIFICACION LOOP PR_PKG_REPRESTAMOS.p_generar_bitacora(
    a.ID_REPRESTAMO,
    NULL,
    'RSB',
    NULL,
    'Cliente sin clasificación',
    USER
);
...
FOR i IN 1..vCantidad_Procesar LOOP FOR A IN CUR_UPDATE_XCORE LOOP --DBMS_OUTPUT.PUT_LINE ( 'ACTUALIZA XCORE: ' );
BEGIN xcore := NVL(
    TRIM(
        JSON_VALUE(
            PA.PA_PKG_CONSULTA_DATACREDITO.CONSULTAR_JSON(
                PA.OBT_IDENTIFICACION_PERSONA(a.codigo_cliente, '1'),
                'C',
                'COMPATIBILIDAD',
                NULL,
                NULL,
            ...
            '$.respuesta.valor.applicants.primaryConsumer.interconnectResponse.Xcore_PD12M_ALL_PC_NC_Global.Xcore'
        )
    ),
    0
);
...
BEGIN setError(
    pProgramUnit = > 'ACTUALIZA_XCORE_DIRIGIDA',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FOR A IN VALIDACION_CLASIFICACION LOOP PR_PKG_REPRESTAMOS.p_generar_bitacora(
    ...
    'RSB',
    NULL,
    'Cliente sin clasificación XCORE',
    USER
);
...
/*BEGIN
 ...
 WHEN NO_DATA_FOUND THEN
 --SI NO SE ENCUENTRA NINGUN REGISTRO CREARA UNO NUEVO
 PR.PR_PKG_TRAZABILIDAD.PR_CREAR_BITACORA_DET ( 'RD_CARGA_PRECALIFICACION', 'RD_CARGA.VALIDA_XCORE', 'INICIADO', pMensaje ); 
 ...
 --ACTUALIZAR BITACORA SI YA EXISTE
 IF idCabeceraDet IS NOT NULL THEN
 PR.PR_PKG_TRAZABILIDAD.PR_ACTUALIZAR_BITACORA_DET (pIDAPLICACION, 'ENPROCESO', 35, 'EN PROCESO', pMensaje );
 END IF;*/
FOR cliente IN c_clientes LOOP v_tipo_credito := cliente.TIPO_CREDITO;
...
IF v_grupo_tipo_credito = 'C'
AND NVL(CLIENTE.XCORE_GLOBAL, 0) <= TO_NUMBER(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('XCORE_CREDITO_COMERCIAL')
)
OR CLIENTE.XCORE_GLOBAL IS NULL THEN --DBMS_OUTPUT.PUT_LINE ( ' ENTRO' );
PR.PR_PKG_REPRESTAMOS.P_Generar_Bitacora(
    CLIENTE.ID_REPRESTAMO,
    NULL,
    'RXC',
    NULL,
    'Rechazado por Xcore inferior para Credito Comercial',
    USER
);
...
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Descartado por Xcore',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    NO_CREDITO = CLIENTE.NO_CREDITO
    AND ESTADO = 'T';
...
--Cambio el estado del detalle de la bitacora
--PR.PR_PKG_TRAZABILIDAD.PR_ACTUALIZAR_BITACORA_DET (pIDAPLICACION, 'ENPROCESO', 90, 'EN PROCESO', pMensaje ); 
ELSIF v_grupo_tipo_credito = 'P'
AND NVL(CLIENTE.XCORE_GLOBAL, 0) <= TO_NUMBER(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('XCORE_CREDITO_CONSUMO')
)
OR CLIENTE.XCORE_GLOBAL IS NULL THEN PR.PR_PKG_REPRESTAMOS.P_Generar_Bitacora(
    CLIENTE.ID_REPRESTAMO,
    NULL,
    'RXC',
    NULL,
    'Rechazado por Xcore inferior para Credito de Consumo',
    USER
);
...
BEGIN pMensaje := 'ERROR CON EL STORE PROCEDURE PVALIDA_XCORE';

setError(
    pProgramUnit = > 'PVALIDA_XCORE',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
UPDATE
    PR.PR_SOLICITUD_REPRESTAMO
SET
    WORLD_COMPLIANCE = VALOR
WHERE
    ID_REPRESTAMO = A.ID_REPRESTAMO;
...
BEGIN --ACTUALIZO EL DETALLE DE LA BITACORA
...
vRutaWallet := PA.DECIFRAR (
    PR_PKG_REPRESTAMOS.F_Obt_Parametro_Represtamo_Raw ('RUTA_WALLET'),
    vKey
);

vPassWallet := PA.DECIFRAR (
    PR_PKG_REPRESTAMOS.F_Obt_Parametro_Represtamo_Raw ('CLAVE_WALLET'),
    vKey
);
...
BEGIN pMensaje := 'ERROR CON EL STORE PROCEDURE OBT_WORLD_COMPLIANCE';
...
FUNCTION F_Genera_Secuencia RETURN NUMBER IS CURSOR cur_Secuencia is
SELECT
    REPRESTAMOS_SEQ.NEXTVAL
FROM
    DUAL;

vSecuencia NUMBER(10);

BEGIN OPEN cur_Secuencia;
...
BEGIN setError(
    pProgramUnit = > 'F_Genera_Secuencia',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
--     
--
FUNCTION F_Genera_Secuencia_Carga_Dirigida RETURN NUMBER IS CURSOR cur_Secuencia is
SELECT
    PA.SQ_CARGA_DIRIGIDA.NEXTVAL
FROM
    DUAL;
...
BEGIN setError(
    pProgramUnit = > 'F_Genera_Secuencia_Carga_Dirigida',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION F_Genera_Secuencia_Campana_Especiales RETURN NUMBER IS CURSOR cur_Secuencia is
SELECT
    PA.SQ_CAMPANA_ESPECIALES.NEXTVAL
FROM
    DUAL;
...
BEGIN setError(
    pProgramUnit = > 'F_Genera_Secuencia_Campana_Especiales',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION F_Obt_Parametro_Represtamo(pCodigo IN VARCHAR) RETURN VARCHAR2 IS CURSOR CUR_parametro IS
SELECT
    VALOR
FROM
    PA_PARAMETROS_MVP
WHERE
    CODIGO_EMPRESA = VCODIGOEMPRESA
    AND codigo_mvp = vTipo_parametro
    and codigo_parametro = pCodigo;

vValor PA_PARAMETROS_MVP.valor % TYPE;

BEGIN OPEN CUR_parametro;
...
BEGIN IA.LOGGER.ADDPARAMVALUEV('pCodigo', pCodigo);

setError(
    pProgramUnit = > 'F_Obt_Parametro_Represtamo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
--
FUNCTION F_Obt_Valor_Parametros(pParametro IN VARCHAR2) RETURN string_table pipelined as delimited_string_cleaned varchar2(32767);
...
begin delimited_string := PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo(pParametro);

delimited_string_cleaned := trim (
    both separator
    from
        delimited_string
) || separator;
...
EXCEPTION
WHEN NO_DATA_NEEDED THEN -- Consumidor dejó de leer; es normal en funciones pipelined
NULL;

WHEN OTHERS THEN DECLARE vIdError PLS_INTEGER := 0;

BEGIN IA.LOGGER.ADDPARAMVALUEV('pParametro', pParametro);

setError(
    pProgramUnit = > 'F_Obt_Valor_Parametros',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION F_Obt_Parametro_Represtamo_Raw(pCodigo IN VARCHAR) RETURN RAW IS CURSOR CUR_parametro IS
SELECT
    VALOR_RAW
FROM
    PA_PARAMETROS_MVP
...
setError(
    pProgramUnit = > 'F_Obt_Parametro_Represtamo_raw',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION F_Obt_Descripcion_Estado(pCodigo IN VARCHAR2) RETURN VARCHAR2 IS vDescripcion PR.PR_ESTADOS_REPRESTAMO.DES_ESTADO % TYPE;

BEGIN BEGIN
SELECT
    E.DES_ESTADO INTO vDescripcion
FROM
    PR.PR_ESTADOS_REPRESTAMO E
WHERE
    E.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
    AND E.CODIGO_ESTADO = pCodigo;

EXCEPTION
WHEN NO_DATA_FOUND THEN vDescripcion := NULL;
...
setError(
    pProgramUnit = > 'F_Obt_Descripcion_Estado',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
--
FUNCTION F_Obt_Telefono(pCodigo IN VARCHAR2, pTipo IN VARCHAR2) RETURN VARCHAR2 IS v_telefono VARCHAR2(100);

v_valor VARCHAR2(100);

BEGIN DECLARE vid_represtamo VARCHAR2(100);

--v_telefono      VARCHAR2(100);
BEGIN
select
    RE.ID_REPRESTAMO,
    TP.COD_AREA || TP.NUM_TELEFONO INTO vid_represtamo,
    v_telefono
FROM
    PR.PR_REPRESTAMOS RE
    JOIN PA.TEL_PERSONAS TP ON TP.COD_PERSONA = RE.CODIGO_CLIENTE
WHERE
    RE.CODIGO_CLIENTE = pCodigo --'1002466' --:COD_PERSONA
    AND TP.TIP_TELEFONO = 'C'
    AND TP.FEC_INCLUSION = (
        SELECT
            MAX(EP.FEC_INCLUSION)
        FROM
            TEL_PERSONAS EP
        WHERE
            EP.COD_PERSONA = pCodigo
    ) --:COD_PERSONA);
...
IF pTipo = 'I' THEN v_valor := vid_represtamo;

ELSE IF pTipo = 'T' THEN v_valor := v_telefono;
...
WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE(
    SQLERRM || ' El represtamo esta en estado AN o CRN' || vid_represtamo
);
...
FUNCTION F_Validar_Telefono (pTelefono IN VARCHAR2) RETURN VARCHAR2 IS vSubtring VARCHAR2(100);

BEGIN DECLARE vSubtringSinParentesis VARCHAR2(100);

BEGIN
SELECT
    REPLACE(REPLACE(pTelefono, '(', ''), ')', '') INTO vSubtringSinParentesis
FROM
    DUAL;

SELECT
    REGEXP_SUBSTR (
        vSubtringSinParentesis,
        PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('EXPREG_TELEFONO')
    ) INTO vSubtring
FROM
    dual;
...
FUNCTION F_Obt_Des_Precalificacion(pCodigo_cliente IN NUMBER) RETURN VARCHAR2 IS CURSOR CUR_Valor IS
SELECT
    b.descripcion,
    CASE
        A.ESTADO
        WHEN 'CRD' THEN 'REPR¿STAMO DIGITAL'
        ELSE NULL
    END Desembolsado
FROM
    PR_REPRESTAMOS a,
...
BEGIN OPEN CUR_Valor;

FETCH CUR_Valor INTO vDescripcion,
vDesembolso;

CLOSE CUR_Valor;

IF vDesembolso IS NOT NULL THEN vDescripcion := vDesembolso;

ELSE IF vDescripcion IS NOT NULL THEN vDescripcion := 'PRECALIFICADO-' || vDescripcion;
...
BEGIN IA.LOGGER.ADDPARAMVALUEN('pCodigo_Cliente', pCodigo_Cliente);

setError(
    pProgramUnit = > 'F_Obt_Des_Precalificacion',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION F_Obt_Empresa_Represtamo RETURN number IS BEGIN RETURN vCodigoEmpresa;

END F_Obt_Empresa_Represtamo;

FUNCTION F_Validar_Edad(pCodigo_cliente NUMBER, pTipo VARCHAR2) RETURN NUMBER IS BEGIN DECLARE FECHA DATE;

EDAD NUMBER;

BEGIN
SELECT
    FEC_NACIMIENTO INTO FECHA
FROM
    PA.PERSONAS_FISICAS
WHERE
    COD_PER_FISICA = cast(pCodigo_cliente as varchar2(15));

IF pTipo = 'CARGA' THEN IF FECHA IS NULL THEN RETURN 0;

ELSE --DBMS_OUTPUT.PUT_LINE ( 'FECHA: ' || FECHA );
...
IF EDAD BETWEEN TO_NUMBER(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('EDAD_MINIMA')
)
AND TO_NUMBER(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('EDAD_MAXIMA')
) THEN RETURN 1;
...
ELSIF pTipo = 'SEGURO' THEN IF FECHA IS NULL THEN RETURN 0;

ELSE EDAD := PA.OBTENEREDAD(FECHA);

IF EDAD BETWEEN TO_NUMBER(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('EDAD_MINIMA_SEGURO')
)
AND TO_NUMBER(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('EDAD_MAXIMA_SEGURO')
) THEN RETURN 1;
...
BEGIN IA.LOGGER.ADDPARAMVALUEV('pCodigo_cliente', pCodigo_cliente);

setError(
    pProgramUnit = > 'F_Validar_Edad',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION f_Validar_Canal(pCanal IN VARCHAR2) RETURN BOOLEAN IS vRetorno BOOLEAN := FALSE;

vSMS NUMBER := NVL(f_obt_parametro_Represtamo('CANAL_SMS'), 1);

vEMAIL NUMBER := NVL(f_obt_parametro_Represtamo('CANAL_EMAIL'), 2);

BEGIN IF TO_NUMBER(pCanal) IN (vSMS, vEMAIL) THEN vRetorno := TRUE;
...
BEGIN -- Determina periodicidad
BEGIN
SELECT
    T1.DIAS_PERIODO,
    T1.Codigo_Periodo_Cuota,
    T1.TIPO_CUOTA INTO pPeriodicidad,
    pCodigoPeriodoCuota,
    pTipoCuota
FROM
    (
        SELECT
            P.DIAS_PERIODO,
            C.Codigo_Periodo_Cuota,
            C.TIPO_CUOTA --INTO pPeriodicidad, pCodigoPeriodoCuota, pTipoCuota
        FROM
            PR.PR_PERIODICIDAD P,
            PR_CREDITOS c
        WHERE
            c.codigo_empresa = pEmpresa
...
EXCEPTION
WHEN NO_DATA_FOUND THEN pMensajeError := 'Datos de la periodicidad no encontrados para el Crédito ' || pNoCredito;
...
-- Calcular Cuotas
nMontoCuota := PR_PKG_CUOTA.FN_CALCULA_CUOTA(
    pF_Primer_Desem = > NVL(pFechaPrestamo, SYSDATE),
    pF_Vencimiento = > pFechaVenc,
    pGracia_Principal = > 0,
    pSaldo_Real = > pMontoPrestamo,
    pTipo_Cuota = > pTipoCuota,
    pPeriodicidad = > pPeriodicidad,
    --pr_periodicidad.dias_periodo,
    pTasa = > pTasa,
    pTipo_Interes = > 'V',
    pTipo_Calendario = > 4,
    pPlazo_Total = > pPlazoTotal,
    pF_Calculo = > NVL(pFechaPrestamo, SYSDATE),
    pDias_Extra = > pDiaExtra,
    -- D+C
...
IF PCalculaSeguroDesempleo = 'S' THEN nMontoSeguroDesempleo := pr_pkg_util.calcular_seguro_desempleo(
    pEmpresa,
    pTipo_Credito,
    pMontoPrestamo,
    pCodigo_Cliente,
...
SELECT
    MONTO_ASEGURADORA INTO nMontoMypime
FROM
    PR_COBERTURAS_X_TIPO_POLIZA
WHERE
    TIPO_POLIZA = 17
    AND pMontoPrestamo BETWEEN MONTO_MINIMO_PAGAR
    AND MONTO_MAXIMO_PAGAR;
...
nMontoCancelacion := NVL(
    PR.pr_pagos_prestamos.F_SALDO_TOTAL_OPERACIONES (
        pEmpresa,
        pNoCredito,
        NVL(pFechaPrestamo, SYSDATE)
    ),
...
setError(
    pProgramUnit = > 'P_Montos_Represtamos',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
setError(
    pProgramUnit = > 'P_Montos_Represtamos_Cancelado',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Cargar_Opcion_Represtamo(
...
SELECT
    ...
    (
        SELECT
            COUNT(1) EXISTE
        FROM
            PR.PR_OPCIONES_REPRESTAMO o
        WHERE
            O.CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
            AND o.id_represtamo = pId
            AND o.plazo = PL.PLAZO
    ) EXISTE,
    ROWNUM ORDEN
FROM
    PR.PR_REPRESTAMOS r,
    pr.PR_PLAZO_CREDITO_REPRESTAMO pl,
    PR_CREDITOS c
WHERE
    R.CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
...
BEGIN vIndSeguroVida := NVL(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('SEGURO_VIDA'),
    'S'
);

vIndSeguroDesempleo := NVL(
    PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('SEGURO_DESEMPLEO'),
    'S'
);

OPEN cOpciones(pIdReprestamo);

LOOP FETCH cOpciones BULK COLLECT INTO vOpciones
LIMIT
    5000;

FOR i IN 1..vOpciones.COUNT LOOP IF vTipoCredito IS NULL
OR vTipoCredito <= 0 THEN vTipoCredito := vOpciones(i).TIPO_CREDITO;
...
DBMS_OUTPUT.PUT_LINE (
    'pMensajeError = ' || pMensajeError || 'vOpciones(i).Existe:' || vOpciones(i).Existe
);

IF pMensajeError is null then IF vOpciones(i).Existe = 0 THEN DBMS_OUTPUT.PUT_LINE ('nMontoCuota = ' || nMontoCuota);

IF nMontoCuota > 0 THEN BEGIN
INSERT INTO
    PR.PR_OPCIONES_REPRESTAMO (
        CODIGO_EMPRESA,
        ID_REPRESTAMO,
        PLAZO,
        MTO_PRESTAMO,
...
UPDATE
    PR.PR_SOLICITUD_REPRESTAMO S
SET
    S.TIPO_CREDITO = vTipoCredito
WHERE
    S.ID_REPRESTAMO = pIdReprestamo;
...
BEGIN IA.LOGGER.ADDPARAMVALUEV('pIdReprestamo', pIdReprestamo);

setError(
    pProgramUnit = > 'P_Cargar_Opcion_Represtamo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Actualizar_Opcion_Front (
...
FROM
    ...
    pr.PR_PLAZO_CREDITO_REPRESTAMO pl,
    PR_CREDITOS c,
    PR_SOLICITUD_REPRESTAMO S
WHERE
    R.CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
    AND S.NO_CREDITO = pNoCredito
    AND S.ID_REPRESTAMO = R.ID_REPRESTAMO --AND R.ID_REPRESTAMO = pId
    AND C.CODIGO_EMPRESA = R.CODIGO_EMPRESA
    AND C.NO_CREDITO = R.NO_CREDITO
    AND PL.CODIGO_EMPRESA = R.CODIGO_EMPRESA
    ...
    AND pPlazo <= (
        SELECT
            MAX (PLAZO)
        FROM
            pr.PR_PLAZO_CREDITO_REPRESTAMO
    )
    AND pMonto >= TO_NUMBER(
        PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('MONTO_MINIMO')
...
BEGIN BEGIN
SELECT
    COUNT(*) INTO vTipo_Mipyme
FROM
    PR_POLIZAS_X_CREDITO
WHERE
    NO_CREDITO = pNoCredito
    AND TIPO_POLIZA = 17;

EXCEPTION
WHEN NO_DATA_FOUND THEN vTipo_Mipyme := 0;

END;

BEGIN
SELECT
    COUNT(*) INTO vTipo_Desempleo
FROM
    PR_POLIZAS_X_CREDITO
WHERE
    TO_NUMBER(NO_CREDITO) = TO_NUMBER(pNoCredito)
    AND TO_NUMBER(TIPO_POLIZA) = 24;

EXCEPTION
WHEN NO_DATA_FOUND THEN vTipo_Desempleo := 0;
...
IF vTipo_Mipyme > 0 THEN vSeguroMipyme := 'S';

ELSE vSeguroMipyme := 'N';
...
IF vTipo_Desempleo > 0 THEN vSeguroDesempleo := 'S';

ELSE vSeguroDesempleo := 'N';

END IF;

--DBMS_OUTPUT.PUT_LINE ( 'vTipo_Desempleo ' || vTipo_Desempleo);
--DBMS_OUTPUT.PUT_LINE ( 'vSeguroDesempleo ' || vSeguroDesempleo);
BEGIN p_montos_represtamos_cancelado(
    vOpciones(i).NO_CREDITO,
    vOpciones(i).TIPO_CREDITO,
    vOpciones(i).CODIGO_CLIENTE,
    Calendar.Fecha_Actual_Calendario(
        'PR',
        PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo,
        '50'
    ),
...
EXCEPTION
WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE(SQLERRM || ' ' || pMensajeError);
...
SET
    S.PLAZO = pPlazo,
    S.TIPO_CREDITO = pTIPO_CREDITO
WHERE
    S.ID_REPRESTAMO = vOpciones(i).ID_REPRESTAMO;

COMMIT;

DBMS_OUTPUT.PUT_LINE ('nMontoCuota = ' || nMontoCuota);

IF nMontoCuota > 0 THEN DBMS_OUTPUT.PUT_LINE (vOpciones(i).ID_REPRESTAMO);
...
SET
    ...
    FECHA_MODIFICACION = SYSDATE
WHERE
    ID_REPRESTAMO = vOpciones(i).ID_REPRESTAMO
    AND PLAZO = vOpciones(i).PLAZO_SOLICITUD;

UPDATE
    PR.PR_REPRESTAMOS
SET
    MTO_PREAPROBADO = pMonto
WHERE
    ID_REPRESTAMO = vOpciones(i).ID_REPRESTAMO;
...
IF nMontoCuotaTotal <= 0 THEN pMensajeError := 'Opción no se pudo calcular por parámetros inválidos';
...
/* IF  Montodepositar < 0  THEN
 ...
 END IF;*/
EXCEPTION
WHEN NO_DATA_FOUND THEN pMensajeError := 'Opcion no se pudo calcular por parametros invalidos';
...
setError(
    pProgramUnit = > 'P_Calcular_Opcion_Front',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION F_ES_REPRESTAMO_DIGITAL(PNO_CREDITO IN NUMBER) RETURN VARCHAR2 IS vExiste VARCHAR2(10);

BEGIN
SELECT
    COUNT(1) INTO vExiste
FROM
    PR_REPRESTAMOS R
WHERE
    R.NO_CREDITO = PNO_CREDITO
    AND R.ESTADO IN (
        select
            COLUMN_VALUE
        FROM
            TABLE(
                PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_VALIDOS_CARTERA_DIGITAL')
            )
    );

IF vExiste > 0 THEN RETURN 'S';
...
PROCEDURE P_Calcular_Opcion_Front (
    pIdReprestamo IN VARCHAR2,
    pMonto IN VARCHAR2,
    pPlazo IN VARCHAR2,
    PCalculaSeguroMypime IN VARCHAR2,
...
DBMS_OUTPUT.PUT_LINE (
    'nMontoCuotaTotal fuera del loop = ' || nMontoCuotaTotal
);
...
IF nMontoCuota > 0 THEN Montoaprobado := vOpciones(i).MTO_PREAPROBADO;
...
IF nMontoCuotaTotal <= 0 THEN DBMS_OUTPUT.PUT_LINE (
    'nMontoCuotaTotal en error = ' || nMontoCuotaTotal
);
...
IF Montodepositar < 0 THEN pMensajeError := 'El monto seleccionado no puede ser menor al monto que debe';
...
PROCEDURE P_Carga_Opcion_Front (
    pIdReprestamo IN VARCHAR2,
    Plazo IN Varchar2,
...
SET
    s.PLAZO = v_PLAZO,
    S.TIPO_CREDITO = v_Tipo_Credito
WHERE
    ID_REPRESTAMO = pIdReprestamo;

COMMIT;

IF F_Existe_Opciones(pIdReprestamo) THEN --COLOCAR EL REPRESTAMO EN ESTADO SC LUEGO DE QUE SE CREE LA OPCION     
/* UPDATE PR_REPRESTAMOS R SET ESTADO = 'SC' WHERE ID_REPRESTAMO =  pIdReprestamo;
 COMMIT;*/
DECLARE pError VARCHAR2(20);

vERRORDESCRIPTION VARCHAR2(20);

vERRORCODE VARCHAR2(20);

BEGIN PR.PR_PKG_REPRESTAMOS.P_Generar_Bitacora(
    pIdReprestamo,
    NULL,
    'SC',
    NULL,
    'Cambiando a estado SC luego de cargar la opcion.',
    USER
);
...
EXCEPTION
WHEN OTHERS THEN pError := 'Linea ' || $ $ plsql_line || 'sqlerrm ' || SQLERRM || ' vERRORDESCRIPTION ' || vERRORDESCRIPTION || ' vERRORCODE ' || vERRORCODE;
...
setError(
    pProgramUnit = > 'P_Carga_Opcion_Front',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Validar_Idreprestamos(
    pIdReprestamo IN VARCHAR2,
    pCanal IN VARCHAR2,
...
) IS vCodCliente CLIENTES_B2000.COD_CLIENTE % TYPE;
...
BEGIN pPinTiempo := NVL(
    f_obt_parametro_Represtamo('TIEMPO_EXPIRA_PIN'),
    12000
);
...
SELECT
    ...
    R.INTENTOS_PIN,
    R.ESTADO,
    f_obt_descripcion_estado(R.ESTADO) INTO vCodCliente,
    pMonto,
    pIntentosIdent,
    pIntentosPin,
    pEstado,
...
FROM
    PR.PR_REPRESTAMOS r,
    PR.PR_CANALES_REPRESTAMO cr -- TODO: Incluir la tabla de los canales por donde se notific¿ al cliente
WHERE
    r.CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
...
EXCEPTION
WHEN NO_DATA_FOUND THEN pPinTiempo := 12000;

vCodCliente := NULL;

END;

IF vCodCliente IS NOT NULL THEN p_datos_primarios(
    vCodCliente,
    pNombres,
    pApellidos,
    pIdentificacion,
...
SELECT
    CASE
        ...
        ELSE 'FALSE'
    END INTO pOrigenCampana
FROM
    PR.PR_REPRESTAMOS R
WHERE
    R.ID_REPRESTAMO = pIdReprestamo;

EXCEPTION
WHEN NO_DATA_FOUND THEN pOrigenCampana := 'FALSE';
...
setError(
    pProgramUnit = > 'P_Validar_Idreprestamos',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION F_Validar_Pin(
    pIdReprestamo IN VARCHAR2,
    pCanal IN VARCHAR2,
    pIdentificacion IN VARCHAR2,
    pPin IN VARCHAR2
) RETURN BOOLEAN IS vRetorno PLS_INTEGER := 0;
...
EXCEPTION
WHEN NO_DATA_FOUND THEN vRetorno := 0;

END;

DBMS_OUTPUT.PUT_LINE(
    'IdReprestamo=' || pIdReprestamo || ' pCanal=' || pCanal || ' pPin=' || pPin || ' pIdentificacion=' || pIdentificacion || ' Validar PIN ' || vRetorno
);
...
PROCEDURE P_Datos_Primarios(
    ...
    pMensaje IN OUT VARCHAR2
) IS vPrimer_Apellido PA.PERSONAS_FISICAS.PRIMER_APELLIDO % TYPE;

vSegundo_Apellido PA.PERSONAS_FISICAS.SEGUNDO_APELLIDO % TYPE;

vCasada_Apellido PA.PERSONAS_FISICAS.APELLIDO_CASADA % TYPE;

BEGIN IF pCodCliente IS NOT NULL THEN BEGIN
SELECT
    REPLACE(C.NUMERO_IDENTIFICACION, '-'),
    C.NOMBRES,
    C.PRIMER_APELLIDO || ' ' || C.SEGUNDO_APELLIDO || CASE
        NVL(C.APELLIDO_DE_CASADA, '0')
        WHEN '0' THEN ''
        ELSE 'DE ' || C.APELLIDO_DE_CASADA
    END,
    C.SEXO,
    C.FECHA_DE_NACIMIENTO,
    C.ESTADO_CIVIL,
    pf.NACIONALIDAD INTO pIdentificacion,
    pNombres,
    pApellidos,
    pSexo,
    pFec_Nacimiento,
...
FROM
    ...
    PERSONAS_FISICAS Pf
WHERE
    C.CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
    AND C.CODIGO_CLIENTE = pCodCliente
    AND PF.COD_PER_FISICA = C.CODIGO_CLIENTE;

EXCEPTION
WHEN NO_DATA_FOUND THEN pIdentificacion := NULL;

END;

ELSE pMensaje := 'Datos del cliente no encontrados.';
...
BEGIN IA.LOGGER.ADDPARAMVALUEV('pCodCliente', pCodCliente);
...
setError(
    pProgramUnit = > 'P_Datos_Primarios',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Datos_Secundarios(
    ...
    pMensaje IN OUT VARCHAR2
) IS CURSOR CUR_email IS
SELECT
    C.EMAIL_USUARIO,
    NVL(
        PA.obt_telefono_persona(C.COD_PER_FISICA, 'C'),
        'N/A'
    ) telefono_celular,
    NVL(
        NVL(
            PA.obt_telefono_persona(C.COD_PER_FISICA, 'D'),
            NVL(
                PA.obt_telefono_persona(C.COD_PER_FISICA, 'R'),
                PA.obt_telefono_persona(C.COD_PER_FISICA, 'T')
            )
        ),
        'N/A'
    ) telefono_residencia,
    NVL(
        NVL(
            PA.obt_telefono_persona(C.COD_PER_FISICA, 'O'),
            PA.obt_telefono_persona(C.COD_PER_FISICA, 'X')
        ),
        'N/A'
    ) telefono_oficina,
...
FROM
    PERSONAS_FISICAS c
WHERE
    C.COD_PER_FISICA = pCodCliente;

BEGIN --  Consulta del datos secundarios del Core 
OPEN CUR_email;

FETCH CUR_email INTO pCorreo,
pTelefonoCelular,
pTelefonoResidencia,
pTelefonoTrabajo,
pDireccion;
...
setError(
    pProgramUnit = > 'P_Datos_Secundarios',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
SELECT
    TRIM(
        REPLACE(
            PA.OBT_NOMBRE_PERSONA(R.CODIGO_CLIENTE),
            SR.APELLIDOS
        )
    ) AS NOMBRES,
    SR.APELLIDOS INTO PNombres,
    PApellidos
FROM
    PR.PR_SOLICITUD_REPRESTAMO SR
    JOIN PR_REPRESTAMOS R ON R.ID_REPRESTAMO = SR.ID_REPRESTAMO
WHERE
    SR.ID_REPRESTAMO = pNum_Represtamo;
...
pSegundoNombre := TRIM(
    TRIM(REGEXP_SUBSTR(PNombres, '[^ ]+', 1, 2)) || ' ' || TRIM(REGEXP_SUBSTR(PNombres, '[^ ]+', 1, 3)) || ' ' || TRIM(REGEXP_SUBSTR(PNombres, '[^ ]+', 1, 4))
);

IF LENGTH(TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 1))) > 3 THEN pPrimerApellido := TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 1));

pSegundoApellido := TRIM(
    TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 2)) || ' ' || TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 3)) || ' ' || TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 4))
);

ELSIF LENGTH(TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 1))) <= 3
AND LENGTH(TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 2))) <= 3 THEN pPrimerApellido := TRIM(
    TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 1)) || ' ' || TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 2)) || ' ' || TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 3))
);

pSegundoApellido := TRIM(
    TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 4)) || ' ' || TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 5)) || ' ' || TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 6))
);

ELSE pPrimerApellido := TRIM(
    TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 1)) || ' ' || TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 2))
);

pSegundoApellido := TRIM(
    TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 3)) || ' ' || TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 4)) || ' ' || TRIM(REGEXP_SUBSTR(PApellidos, '[^ ]+', 1, 5))
);
...
BEGIN IF f_Validar_Canal(pCanal) = FALSE THEN pRespuesta := 'Debe especificar el canal v¿lido';

RETURN vPin;

END IF;

BEGIN -- Determinar nuevo PIN a traves de la notificacion Inicial
SELECT
    lpad(
        PKG_NOTIFICACIONES.GENERAR_PIN_RANDOM(100, 999999),
        6,
        '0'
    ),
    c.VALOR,
    s.NOMBRES,
    S.APELLIDOS INTO vPIN,
    vContacto,
    vNombres,
    vApellidos
FROM
    PR_CANALES_REPRESTAMO c,
    PR_SOLICITUD_REPRESTAMO s
WHERE
    c.CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
...
EXCEPTION
WHEN NO_DATA_FOUND THEN vPIN := NULL;
...
IF vPIN IS NOT NULL THEN vLinkActivado := PR.PR_PKG_REPRESTAMOS.P_Total_Estado_Bitacora_ID('LA', pIdReprestamo);

IF vLinkActivado = 0 THEN PR_PKG_REPRESTAMOS.p_generar_bitacora(
    pIdReprestamo,
    pCanal,
    'LA',
    '0',
...
EXCEPTION
WHEN OTHERS THEN pRespuesta := 'Error actualizando el PIN en la maestra de represtamos';
...
IF pCanal = vSMS THEN -- Reemplazar el mensaje del parametro 
vMensaje := REPLACE(
    f_obt_parametro_Represtamo('TEXTO_SMS_PIN'),
    '[PIN]',
    vPIN
);
...
DBMS_OUTPUT.PUT_LINE (
    'IDREPRESTAMO DENTRO DEL P_SOLICITAR PIN: ' || pIdReprestamo
);

EXCEPTION
WHEN OTHERS THEN pRespuesta := SQLERRM;
...
IF pObservacion IS NULL THEN p_generar_bitacora(
    pIdReprestamo,
    pCanal,
    'PS',
    null,
    'Solicitado nuevo PIN',
    NVL(
        pUsuario,
...
PROCEDURE P_Validar_Cambio_Estado(
    pIdReprestamo IN VARCHAR2,
    pEstado IN VARCHAR2
) IS PRAGMA AUTONOMOUS_TRANSACTION;
...
SELECT
    NVL(B.IND_CAMBIA_ESTADO_ORIGINAL, 'N') ESTADO_ORIGINAL,
    B.IND_NOTIFICA_CLIENTE,
    B.IND_CAMBIA_ESTADO_REPRE
FROM
    PR_ESTADOS_REPRESTAMO B
WHERE
    B.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
...
BEGIN OPEN CUR_CAMBIO_ESTADO;

FETCH CUR_CAMBIO_ESTADO INTO vOriginal,
vNotifica,
vCambiaEstado;
...
--
IF vCambiaEstado = 'S' THEN BEGIN
UPDATE
    PR_REPRESTAMOS
SET
    estado = pEstado,
    estado_original = CASE
        vOriginal
        WHEN 'S' THEN estado
        ELSE NULL
    END
WHERE
    codigo_empresa = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
...
IF vNotifica = 'S' THEN IF pEstado = 'CRA' THEN PR_PKG_REPRESTAMOS.P_Notificar_Desembolso(pIdReprestamo, pEstado, vError);

ELSIF pEstado = 'CRD' THEN -- Notificacion Encuesta
PR_PKG_REPRESTAMOS.P_Notificar_Encuesta(pIdReprestamo, vError);

ELSE PR_PKG_REPRESTAMOS.p_Notificar_Estado(pIdReprestamo, NULL, pEstado, vError);
...
SELECT
    COUNT (COLUMN_VALUE) INTO vEstadoBloqueo
FROM
    TABLE(
        PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_DESACTIVAR_ACCESO_FRONTEND')
    )
WHERE
    COLUMN_VALUE = pEstado;

SELECT
    COUNT (COLUMN_VALUE) INTO vEstadoAcceso
FROM
    TABLE(
        PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_ACTIVAR_ACCESO_FRONTEND')
    )
WHERE
    COLUMN_VALUE = pEstado;

SELECT
    COUNT (1) CODIGO_ESTADO INTO vEstado
FROM
    PR_BITACORA_REPRESTAMO
WHERE
    CODIGO_ESTADO = 'NR'
...
IF vEstadoBloqueo >= 1
AND vEstado >= 1 THEN PR_PKG_REPRESTAMOS.P_Desactivar_Activar_FrontEnd (pIdReprestamo, 1, vError);

COMMIT;

ELSIF vEstadoAcceso >= 1 THEN PR_PKG_REPRESTAMOS.P_Desactivar_Activar_FrontEnd (pIdReprestamo, 2, vError);
...
BEGIN -- Cambiar el estado del represtamo
...
EXCEPTION
WHEN OTHERS THEN vMensaje := 'Fallo, Ingresando Bitacora del Represtamo.';
...
setError(
    pProgramUnit = > 'P_Generar_Bitacora',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Actualizar_Intentos(
    pIdReprestamo IN VARCHAR2,
    pTipoIntento IN VARCHAR2,
    -- TIPO I = Identificacion, P = PIN
...
BEGIN IF pNumeroIntento >= 0 THEN IF pTipoIntento = 'P' THEN BEGIN
UPDATE
    PR.PR_REPRESTAMOS r
SET
    R.INTENTOS_PIN = pNumeroIntento
WHERE
    CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
    AND R.ID_REPRESTAMO = pIdReprestamo;
...
EXCEPTION
WHEN OTHERS THEN pRespuesta := 'Fall¿, Al Actualizar el intento de validaci¿n de PIN en el Represtamo. Intente m¿s tarde';
...
IF pNumeroIntento = 0 THEN p_Bloquear_represtamo(pIdReprestamo, pTipoIntento, pRespuesta);

END IF;

ELSIF pTipoIntento = 'I' THEN BEGIN
UPDATE
    PR.PR_REPRESTAMOS r
SET
    R.INTENTOS_IDENTIFICACION = pNumeroIntento
WHERE
    R.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
    AND R.ID_REPRESTAMO = pIdReprestamo;
...
EXCEPTION
WHEN OTHERS THEN pRespuesta := 'Fall¿, Al Actualizar el intento de validaci¿n de Identificaci¿n en el Represtamo. Intente m¿s tarde';
...
setError(
    pProgramUnit = > 'P_Actualizar_Intentos',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION P_Total_Estado_Bitacora(pEstado IN VARCHAR2) RETURN NUMBER IS vTotal PLS_INTEGER := 0;
...
BEGIN IA.LOGGER.ADDPARAMVALUEV('pEstado', pEstado);

setError(
    pProgramUnit = > 'P_Total_Estado_Bitacora',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION P_Total_Estado_Bitacora_ID(
    pEstado IN VARCHAR2,
    pIdReprestamo IN VARCHAR2
) RETURN NUMBER IS vTotal PLS_INTEGER := 0;
...
setError(
    pProgramUnit = > 'P_Total_Estado_Bitacora_ID',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION P_Total_Estado(pEstado IN VARCHAR2) RETURN NUMBER IS vTotal PLS_INTEGER := 0;
...
FROM
    PR.PR_REPRESTAMOS r
WHERE
    R.CODIGO_EMPRESA = f_obt_Empresa_Represtamo
...
FUNCTION f_Step_Actual(
    pEstado IN VARCHAR2,
    pIdReprestamo IN VARCHAR2
) RETURN VARCHAR2 IS vStep VARCHAR2(2) := '0';
...
WHERE
    ...
    AND B.FECHA_ADICION = (
        SELECT
            MAX(x.FECHA_ADICION)
        FROM
            PR.PR_BITACORA_REPRESTAMO x
        WHERE
            x.CODIGO_ESTADO = pEstado
            AND x.ID_REPRESTAMO = pIdReprestamo
    )
    AND ROWNUM = 1;

EXCEPTION
WHEN NO_DATA_FOUND THEN vStep := '0';
...
FUNCTION P_Obt_Estado_Represtamo(
    pIdReprestamo IN VARCHAR2,
    pTipo IN VARCHAR2 DEFAULT 'A'
) -- A = Actual, O = Original
RETURN VARCHAR2 IS vEstado PR.PR_REPRESTAMOS.ESTADO % type;

BEGIN
SELECT
    CASE
        WHEN pTipo = 'A' THEN ESTADO
        WHEN pTipo = 'O' THEN ESTADO_ORIGINAL
    END INTO vEstado
FROM
    PR.PR_REPRESTAMOS r
WHERE
...
setError(
    pProgramUnit = > 'P_Obt_Estado_Represtamo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Notificar_Ayuda(pUsuario IN VARCHAR2) IS PRAGMA AUTONOMOUS_TRANSACTION;
...
BEGIN OPEN cReprestamos;

LOOP FETCH cReprestamos BULK COLLECT INTO vReprestamos
LIMIT
    500;

FOR i IN 1..vReprestamos.COUNT LOOP vStep := PR.PR_PKG_REPRESTAMOS.f_Step_Actual('AYS', vReprestamos(i).ID_REPRESTAMO);

p_generar_bitacora(
    vReprestamos(i).ID_REPRESTAMO,
    NULL,
    'AYN',
    vStep,
    'Notificaci¿n de Ayuda',
    pUsuario
);
...
BEGIN IA.LOGGER.ADDPARAMVALUEV('pUsuario', pUsuario);

setError(
    pProgramUnit = > 'P_Notificar_Ayuda',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Bloquear_Represtamo(
    pIdReprestamo IN VARCHAR2,
    pTipoIntento IN VARCHAR2,
    pRespuesta IN OUT VARCHAR2
) IS vCodigoEstado PR.PR_ESTADOS_REPRESTAMO.CODIGO_ESTADO % TYPE;

BEGIN CASE
    pTipoIntento
    WHEN 'I' THEN vCodigoEstado := 'BLI';

WHEN 'P' THEN vCodigoEstado := 'BLP';

ELSE pRespuesta := 'Tipo de Intento no es v¿lido.  Debe ser [I,P]';
...
setError(
    pProgramUnit = > 'P_Bloquear_Represtamo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Desbloquear_Represtamo(
    pIdReprestamo IN VARCHAR2,
...
/*pRespuesta := 'Fall¿, '||pIdReprestamo  ||' '|| pCodigoEstado ||' '||  pUsuario ||' '|| pObservacion;
 raise_application_error(-20100,  pRespuesta);*/
IF pCodigoEstado IS NOT NULL THEN -- Busca el estado original
BEGIN
SELECT
    ESTADO_ORIGINAL INTO vEstadoAnterior
FROM
    PR.PR_REPRESTAMOS P
WHERE
    P.ID_REPRESTAMO = pIdReprestamo
    AND P.ESTADO = pCodigoEstado;

EXCEPTION
WHEN NO_DATA_FOUND THEN vEstadoAnterior := NULL;

END;

ELSE pRespuesta := 'Fall¿, Al determinar el estado anterior en el Represtamo. Intente m¿s tarde';

RAISE_APPLICATION_ERROR(-20100, pRespuesta);

END IF;

IF vEstadoAnterior IS NOT NULL THEN OPEN cCanal(pIdReprestamo);

LOOP FETCH cCanal BULK COLLECT INTO vCanal
LIMIT
    500;

EXIT
WHEN cCanal % NOTFOUND;
...
CASE
    pCodigoEstado
    WHEN 'BLI' THEN vIntento := PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('MAX_INTENTOS_IDENTIFICACION');

BEGIN
UPDATE
    PR.PR_REPRESTAMOS r
SET
    R.INTENTOS_IDENTIFICACION = vIntento
WHERE
    R.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
    AND R.ID_REPRESTAMO = pIdReprestamo;
...
FOR i IN 1..vCanal.COUNT LOOP P_Desbloqueo_FrontEnd(
    pIdReprestamo,
    vCanal(i).CANAL,
    1,
    vIntento,
...
WHEN 'BLP' THEN vIntento := PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('MAX_INTENTOS_PIN');

BEGIN
UPDATE
    PR.PR_REPRESTAMOS r
SET
    R.INTENTOS_PIN = vIntento
WHERE
    R.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
    AND R.ID_REPRESTAMO = pIdReprestamo;
...
PR_PKG_REPRESTAMOS.p_generar_bitacora(
    ...
    vEstadoAnterior,
    NULL,
    NVL(pObservacion, 'Desbloqueo de cliente'),
    pUsuario
);
...
IF pRespuesta IS NULL THEN pRespuesta := 'El cliente' || pIdReprestamo || ' ha sido desbloqueado correctamente';
...
setError(
    pProgramUnit = > 'P_Desbloquear_Represtamo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Desbloqueo_FrontEnd(
    pIdReprestamo IN VARCHAR2,
    pCanal IN VARCHAR2,
...
BEGIN -- Desencriptar ruta y pass
vKey := F_Obt_Parametro_Represtamo_Raw('CIFRADO_MASTERKEY');

vRutaWallet := PA.DECIFRAR(
    F_Obt_Parametro_Represtamo_Raw('RUTA_WALLET'),
    vKey
);

vPassWallet := PA.DECIFRAR(
    F_Obt_Parametro_Represtamo_Raw('CLAVE_WALLET'),
    vKey
);
...
PROCEDURE P_Desactivar_Activar_FrontEnd(
    pIdReprestamo IN VARCHAR2,
    pActivacion IN NUMBER,
    pRespuesta IN OUT VARCHAR2
) IS vUrlAPI VARCHAR2(4000);
...
BEGIN IA.LOGGER.ADDPARAMVALUEV('pRespuesta', pRespuesta);

setError(
    pProgramUnit = > 'P_Desactivar_Activar_FrontEnd',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Notificar_Desbloqueo(
    pIdReprestamo IN NUMBER,
    pCodigoEstado IN VARCHAR2,
    pRespuesta IN OUT VARCHAR2
) IS CURSOR cCanal (pIdRep IN NUMBER) IS
SELECT
    C.CANAL,
    C.VALOR
FROM
...
BEGIN OPEN cCanal(pIdReprestamo);
...
FOR i IN 1..vCanal.COUNT LOOP IF vCanal(i).Canal = vSMS THEN -- Reemplazar el mensaje del parametro 
vMensaje := REPLACE(
    f_obt_parametro_Represtamo('TEXTO_SMS_DESBLOQUEO'),
    '[ESTADO]',
    pCodigoEstado
);
...
p_generar_bitacora(
    ...
    'DBA',
    NULL,
    'Notificaci¿n Desbloqueo Canal SMS',
    NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
);
...
PROCEDURE P_Notificar_Encuesta(
    pIdReprestamo IN NUMBER,
    pRespuesta IN OUT VARCHAR2
) IS CURSOR cCanal (pIdRep IN NUMBER) IS
SELECT
...
FOR i IN 1..vCanal.COUNT LOOP IF vCanal(i).Canal = vSMS THEN vUrlAPI := F_Obt_Parametro_Represtamo('RUTA_API_MANAGER') || 'api/Notification/SendSurveyLinkBySMS';

vMensaje := REPLACE(
    f_obt_parametro_Represtamo('TEXTO_SMS_ENCUESTA'),
    '[ESTADO]',
    vCanal(i).ESTADO
);
...
p_generar_bitacora(
    ...
    'NTE',
    NULL,
    'Notificaci¿n Encuesta Canal SMS',
    NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
);

ELSIF vCanal(i).CANAL = vEMAIL THEN -- Reemplazar el mensaje del parametro 
vMensaje := REPLACE(
    f_obt_parametro_Represtamo('TEXTO_EMAIL_ENCUESTA'),
    '[ESTADO]',
    vCanal(i).ESTADO
);
...
vSubject := REPLACE(
    f_obt_parametro_Represtamo('SUBJECT_EMAIL'),
    '[FECHA]',
...
p_generar_bitacora(
    ...
    'NTE',
    NULL,
    'Notificaci¿n Encuesta Canal EMAIL',
    NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
);
...
PROCEDURE P_Notificar_Desembolso(
    pIdReprestamo IN NUMBER,
    pCodigoEstado IN VARCHAR2,
    pRespuesta IN OUT VARCHAR2
) IS CURSOR cCanal (pIdRep IN NUMBER) IS
...
FOR i IN 1..vCanal.COUNT LOOP IF vCanal(i).Canal = vSMS THEN vUrlAPI := F_Obt_Parametro_Represtamo('RUTA_API_MANAGER') || 'api/Notification/SendReloanApprovedLinkBySMS';

vMensaje := REPLACE(
    f_obt_parametro_Represtamo('TEXTO_SMS_DESEMBOLSO'),
    '[ESTADO]',
    pCodigoEstado
);
...
p_generar_bitacora(
    ...
    'NBD',
    NULL,
    'Notificaci¿n Desembolso Canal SMS',
    NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
);

ELSIF vCanal(i).CANAL = vEMAIL THEN -- Reemplazar el mensaje del parametro 
vMensaje := REPLACE(
    f_obt_parametro_Represtamo('TEXTO_EMAIL_DESEMBOLSO'),
    '[ESTADO]',
    pCodigoEstado
);
...
p_generar_bitacora(
    ...
    'NBD',
    NULL,
    'Notificaci¿n Desembolso Canal EMAIL',
    NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
);
...
PROCEDURE P_Notificar_Reenvio_Link(
    pIdReprestamo IN NUMBER,
    pRespuesta IN OUT VARCHAR2
) IS CURSOR cCanal(pIdReprestamo IN NUMBER) IS
SELECT
    c.CANAL,
    c.VALOR,
    S.NOMBRES,
...
FOR x IN 1..vCanal.COUNT LOOP vMensaje := PR_PKG_REPRESTAMOS.F_Obt_Body_Mensaje(pIdReprestamo, vCanal(x).CANAL);

IF vCanal(x).CANAL = vSMS THEN vTipoCanal := 'SMS';

PR_PKG_REPRESTAMOS.Reenviar_Sms_Api(
    pIdReprestamo,
    vCanal(x).VALOR,
    vCanal(x).NOMBRES,
    vCanal(x).APELLIDOS,
...
IF pRespuesta IS NULL THEN pRespuesta := 'Notificacion Enviada';
...
FUNCTION F_Obt_Subject_Email(pIdReprestamo IN VARCHAR2) RETURN VARCHAR2 IS vSubject VARCHAR2(4000);

vNombres VARCHAR2(400);

BEGIN BEGIN
SELECT
    PA.OBT_NOMBRE_PERSONA(R.CODIGO_CLIENTE) Nombres INTO vNombres
FROM
    PR.PR_REPRESTAMOS r
WHERE
    r.ID_REPRESTAMO = pIdReprestamo;
...
EXCEPTION
WHEN NO_DATA_FOUND THEN vNombres := NULL;

END;

IF vNombres IS NOT NULL THEN vSubject := REPLACE(
    f_obt_parametro_Represtamo('SUBJECT_EMAIL'),
    '[NOMBRES]',
    vNombres
);
...
setError(
    pProgramUnit = > 'F_Obt_Subject_Email',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION F_Obt_Body_Mensaje(
    pIdReprestamo IN VARCHAR2,
    pCanal IN VARCHAR2
) RETURN VARCHAR2 IS vBody VARCHAR2(4000);
...
SELECT
    PF.PRIMER_NOMBRE,
    R.fecha_proceso --PA.OBT_NOMBRE_PERSONA(R.CODIGO_CLIENTE) Nombres
    INTO vNombres,
    vFecha
FROM
    PR.PR_REPRESTAMOS r,
...
SELECT
    TO_DATE(
        TO_CHAR(LAST_DAY(R.FECHA_PROCESO), 'DD/MM/YYYY') || ' 11:59:59 PM',
...
vBody := REPLACE(
    REPLACE(
        f_obt_parametro_Represtamo('TEXTO_SMS'),
        '[NOMBRES]',
        vNombres
    ),
    '[FECHA]',
    TO_CHAR(vUltimoDia, 'DD/MM/YYYY HH:MI:SS AM')
);

ELSIF pCanal = 2 THEN -- EMAIL
vBody := REPLACE(
    REPLACE(
        f_obt_parametro_Represtamo('TEXTO_EMAIL'),
        '[NOMBRES]',
        vNombres
    ),
    '[FECHA]',
    to_char(
        trunc(vFecha) + PR_PKG_REPRESTAMOS.F_OBT_PARAMETRO_REPRESTAMO('DIA_CADUCA_LINK'),
        'DD/MM/YYYY'
    )
);
...
setError(
    pProgramUnit = > 'F_Obt_Body_Mensaje',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Registrar_Solicitud(
    pIdReprestamo IN VARCHAR2,
    pUsuario IN VARCHAR2,
    pMensaje IN OUT VARCHAR2
) IS vCodCliente CLIENTES_B2000.COD_CLIENTE % TYPE;
...
BEGIN BEGIN
SELECT
    R.CODIGO_CLIENTE INTO vCodCliente
FROM
    PR.PR_REPRESTAMOS r
WHERE
    r.CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
...
EXCEPTION
WHEN NO_DATA_FOUND THEN pMensaje := 'Datos del Re-pr¿stamo no encontrados';
...
BEGIN
SELECT
    R.ID_REPRESTAMO INTO vIdreprestamoSolicitud
FROM
    PR.PR_SOLICITUD_REPRESTAMO r
WHERE
    r.CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
    AND r.ID_REPRESTAMO = pIdReprestamo;

EXCEPTION
WHEN NO_DATA_FOUND THEN vIdreprestamoSolicitud := 0;
...
IF vIdreprestamoSolicitud = 0 THEN BEGIN
INSERT INTO
    PR.PR_SOLICITUD_REPRESTAMO (
        CODIGO_EMPRESA,
        ID_REPRESTAMO,
...
EXCEPTION
WHEN OTHERS THEN pMensaje := 'Error Insertando los datos de la solicitud: ' || SQLERRM;
...
IF pIdReprestamo IS NOT NULL
AND F_Validar_Telefono(vTelefonoCelular) IS NOT NULL THEN
INSERT INTO
    PR.PR_CANALES_REPRESTAMO (
        CODIGO_EMPRESA,
        ID_REPRESTAMO,
...
VALUES
    (
        ...
        pIdReprestamo,
        1
        /*Obtener el valor de la tabla parametro*/
,
        F_Validar_Telefono(vTelefonoCelular),
        pUsuario,
        SYSDATE
    );
...
setError(
    pProgramUnit = > 'P_Registrar_Solicitud',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Registra_Solicitud_Dirigida(
    pIdReprestamo IN VARCHAR2,
    pUsuario IN VARCHAR2,
    pMensaje IN OUT VARCHAR2
) IS vCodCliente CLIENTES_B2000.COD_CLIENTE % TYPE;
...
EXCEPTION
WHEN NO_DATA_FOUND THEN pMensaje := 'Datos del Represtamo no encontrados';
...
BEGIN
SELECT
    CELULAR INTO v_Telefono_Celular
FROM
    PR.PR_CARGA_DIRECCIONADA C
...
IF pIdReprestamo IS NOT NULL
AND F_Validar_Telefono(v_Telefono_Celular) IS NOT NULL THEN
INSERT INTO
    PR.PR_CANALES_REPRESTAMO (
        CODIGO_EMPRESA,
        ID_REPRESTAMO,
...
VALUES
    (
        ...
        /*Obtener el valor de la tabla parametro*/
,
        F_Validar_Telefono(v_Telefono_Celular),
        pUsuario,
        SYSDATE
    );
...
setError(
    pProgramUnit = > 'P_Registra_Solicitud_Dirigida',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
SELECT
    CELULAR INTO v_Telefono_Celular
FROM
    PR.PR_CAMPANA_ESPECIALES C
    JOIN PR.PR_REPRESTAMOS R ON C.NO_CREDITO = R.NO_CREDITO
WHERE
    R.ID_REPRESTAMO = pIdReprestamo --AND C.ESTADO='T' 
    AND ROWNUM = 1;
...
BEGIN CREDITO := PR.PR_PKG_REPRESTAMOS.F_OBTENER_NUEVO_CREDITO(pIdReprestamo);

IF CREDITO = 1 THEN PR.PR_PKG_REPRESTAMOS.p_generar_bitacora(
    pIdReprestamo,
    NULL,
    'AN',
    NULL,
...
SET
    ESTADO = 'E',
    OBSERVACIONES = 'Represtamo anulado por salto de Tipo Credito',
    FECHA_MODIFICACION = SYSDATE,
    MODIFICADO_POR = NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER)
WHERE
    ESTADO = 'T'
...
ELSE IF PR.PR_PKG_REPRESTAMOS.F_Validar_Tipo_Represtamo(pIdReprestamo) THEN
UPDATE
    PR.PR_SOLICITUD_REPRESTAMO
SET
...
EXCEPTION
WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('No se encontraron datos para la consulta.');
...
setError(
    pProgramUnit = > 'P_Registra_Solicitud_Campana_Tipo_Credito',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
setError(
    pProgramUnit = > 'P_Registra_Solicitud_Campana',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Actualizar_Datos_Solicitud(
    pIdReprestamo IN VARCHAR2,
    pCanal IN VARCHAR2,
    pEstado IN VARCHAR2,
    pStep IN VARCHAR2,
    ...
    pMensaje IN OUT VARCHAR2
) IS BEGIN IF pEstado in ('LA', 'EP', 'SC') THEN BEGIN
UPDATE
    PR.PR_SOLICITUD_REPRESTAMO S
SET
    s.ESTADO = pEstado,
...
WHERE
    S.CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
    AND S.ID_REPRESTAMO = pIdReprestamo;

COMMIT;

EXCEPTION
WHEN OTHERS THEN pMensaje := 'Fallo al registrar la solicitud. Intente m¿s tarde.';

RAISE_APPLICATION_ERROR(-20010, pMensaje);

END;

ELSE pMensaje := 'No es posible actualizar la solicitud en el Estado ' || pEstado;
...
setError(
    pProgramUnit = > 'P_Actualizar_Datos_Solicitud',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Actualizar_Canal_Represtamo (
    pCod_persona IN VARCHAR2,
    pcod_area IN VARCHAR2,
    Pnum_telefono IN VARCHAR2,
    pMensaje IN OUT VARCHAR2
) IS BEGIN DECLARE vid_represtamo VARCHAR2(100);
...
FROM
    PR.PR_REPRESTAMOS RE
WHERE
    RE.CODIGO_CLIENTE = pCod_persona
    AND RE.ESTADO IN (
        select
            COLUMN_VALUE
        FROM
            TABLE(
                PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_ACTUALIZAR_CANAL_REPRESTAMO')
            )
    );
...
WHERE
    ...
    AND RE.ESTADO IN (
        ...
        FROM
            TABLE(
                PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_PARA_ACTUALIZACION_DE_EMAIL_TELEFONO')
            )
    );

BEGIN FOR A IN TELEFONOS_ACTUALIZAR LOOP vid_represtamo := A.ID_REPRESTAMO;

vEstado := A.ESTADO;

IF PR_PKG_REPRESTAMOS.F_VALIDAR_TELEFONO(pcod_area || pnum_telefono) IS NOT NULL THEN IF F_Existe_Canales(A.ID_REPRESTAMO) = FALSE THEN
INSERT INTO
    PR.PR_CANALES_REPRESTAMO (
        CODIGO_EMPRESA,
        ID_REPRESTAMO,
...
ELSE
UPDATE
    PR.PR_CANALES_REPRESTAMO S
SET
    S.CANAL = 1,
    S.VALOR = pcod_area || Pnum_telefono
WHERE
    S.ID_REPRESTAMO = A.ID_REPRESTAMO
    AND S.CANAL = 1;
...
UPDATE
    PR.PR_SOLICITUD_REPRESTAMO S
SET
    S.TELEFONO_CELULAR = pcod_area || Pnum_telefono
WHERE
    S.CODIGO_EMPRESA = 1
    AND S.ID_REPRESTAMO = A.ID_REPRESTAMO;

ELSE PR_PKG_REPRESTAMOS.p_generar_bitacora(
    vid_represtamo,
    NULL,
    vEstado,
    NULL,
    'No se pudo actualizar el canal del represtamo debido a que el telefono no cumple con el formato permitido: ' || pcod_area || pnum_telefono,
    USER
);
...
FOR A IN REPRESTAMOS LOOP vid_represtamo := A.ID_REPRESTAMO;
...
) LOOP IF A.ESTADO = B.COLUMN_VALUE THEN PR_PKG_REPRESTAMOS.p_generar_bitacora(
    A.ID_REPRESTAMO,
    NULL,
    'NP',
    NULL,
    'Canal actualizado y resprestamo en estado NP',
    USER
);
...
EXCEPTION
WHEN OTHERS THEN pMensaje := 'Error al ejecutar ' || SQLERRM;
...
setError(
    pProgramUnit = > 'P_Actualizar_Canal_Represtamo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Actualizar_Email_Represtamo (
    pCod_persona IN VARCHAR2,
    PEmail IN VARCHAR2,
    pMensaje IN OUT VARCHAR2
) IS BEGIN DECLARE vid_represtamo VARCHAR2(100);
...
BEGIN FOR A IN REPRESTAMOS LOOP vid_represtamo := A.ID_REPRESTAMO;
...
) LOOP IF A.ESTADO = B.COLUMN_VALUE THEN --vEstado
UPDATE
    PR.PR_SOLICITUD_REPRESTAMO
SET
    EMAIL = PEmail
...
setError(
    pProgramUnit = > 'P_Actualizar_Email_Represtamo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
/* BEGIN
 ...
 WHEN NO_DATA_FOUND THEN
 --SI NO SE ENCUENTRA NINGUN REGISTRO CREARA UNO NUEVO
 PR.PR_PKG_TRAZABILIDAD.PR_CREAR_BITACORA_DET ( 'RD_CARGA_PRECALIFICACION', 'RD_CARGA.ACTUALIZAR_ANULAR_REPRESTAMO', 'INICIADO', pMensaje ); 
 ...
 DBMS_OUTPUT.PUT_LINE ( 'pIDAPLICACION = ' || pIDAPLICACION );
...
setError(
    pProgramUnit = > 'P_Actualizar_Anular_Represtamo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Carga_Precalifica_Represtamo (pMensaje IN OUT VARCHAR2) IS BEGIN DECLARE CURSOR CUR_REPRESTAMO IS
SELECT
    ID_REPRESTAMO,
    ESTADO,
    XCORE_GLOBAL
...
pIDAPLICACION NUMBER;

BEGIN --1 
PR_PKG_REPRESTAMOS.P_Actualizar_Anular_Represtamo(
    pMensaje
    /*,pIDAPLICACION*/
);

--2
PR_PKG_REPRESTAMOS.Precalifica_Represtamo();

--3
PR_PKG_REPRESTAMOS.Actualiza_Precalificacion();

--4
PR_PKG_REPRESTAMOS.Actualiza_XCORE_CUSTOM();

FOR A IN CUR_REPRESTAMO LOOP --DBMS_OUTPUT.PUT_LINE ( 'Entra = '||A.ID_REPRESTAMO  );
PR_PKG_REPRESTAMOS.P_Registrar_Solicitud(
    A.ID_REPRESTAMO,
    NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER),
    VMSG
);
...
AND F_Existe_Canales(A.ID_REPRESTAMO) THEN PR.PR_PKG_REPRESTAMOS.P_Generar_Bitacora(
    A.ID_REPRESTAMO,
    NULL,
    'NP',
    NULL,
...
AND F_Existe_Canales(A.ID_REPRESTAMO) = FALSE THEN PR.PR_PKG_REPRESTAMOS.P_Generar_Bitacora(
    A.ID_REPRESTAMO,
    NULL,
    'CP',
    NULL,
...
setError(
    pProgramUnit = > 'P_Carga_Precalifica_Represtamo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_REGISTRO_SOLICITUD IS VMSG VARCHAR2(4000);
...
/*BEGIN
 ...
 END IF;           
 */
FOR A IN CUR_REPRESTAMO LOOP PR.PR_PKG_REPRESTAMOS.P_Registrar_Solicitud(
    A.ID_REPRESTAMO,
    NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER),
    VMSG
);
...
BEGIN pMensaje := 'ERROR CON EL STORE PROCEDURE REGISTRAR_SOLICITUD';

setError(
    pProgramUnit = > 'P_REGISTRO_SOLICITUD',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Carga_Precalifica_Cancelado (pMensaje IN OUT VARCHAR2) IS BEGIN --Crear bitacora Cabecera
/*PR_PKG_TRAZABILIDAD.PR_CREAR_BITACORA_CAB ( 'RD_CARGA_PRECALIFICACION', 'ACTIVO', Null,pMensaje);
 ...
 END;*/
DECLARE CURSOR CUR_REPRESTAMO IS
SELECT
    ID_REPRESTAMO,
    ESTADO,
    XCORE_GLOBAL
...
BEGIN -- Inicio del proceso de trazabilidad
--1  
...
FOR A IN CUR_REPRESTAMO LOOP DBMS_OUTPUT.PUT_LINE (
    'Entra AL CURSOR CUR_REPRESTAMO = ' || A.ID_REPRESTAMO
);
...
SET
    VALOR = VALOR ||(
        CASE
            WHEN NVL(REGEXP_COUNT(VALOR, '}'), 0) > 0 THEN ','
            ELSE ''
        END
    ) || '{"F":"' || TO_CHAR(SYSDATE, 'dd/mm/yyyy hh:mi:ss') || '","R":' || v_conteo || ',"E":' || NVL(REGEXP_COUNT(VALOR, '}') + 1, 1) || '}'
WHERE
    CODIGO_MVP = 'REPRESTAMOS'
    AND CODIGO_PARAMETRO = 'EJECUCIONES';
...
setError(
    pProgramUnit = > 'P_Carga_Precalifica_Cancelado',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Carga_Precalifica_Manual(pMensaje IN OUT VARCHAR2) IS BEGIN DECLARE CURSOR CUR_REPRESTAMO IS
SELECT
    ID_REPRESTAMO,
    ESTADO,
    XCORE_GLOBAL,
...
BEGIN v_proceso_activo := PR.PR_PKG_REPRESTAMOS.F_OBT_PARAMETRO_REPRESTAMO ('CARGA_DIRIGIDA_PROCESO_ACTIVO');
...
WHERE
    CODIGO_MVP = 'REPRESTAMOS'
    AND CODIGO_PARAMETRO = 'CARGA_DIRIGIDA_PROCESO_ACTIVO';
...
FOR A IN CUR_REPRESTAMO LOOP --DBMS_OUTPUT.PUT_LINE ( 'Entra = '||A.ID_REPRESTAMO  );
PR.PR_PKG_REPRESTAMOS.P_Registra_Solicitud_Dirigida(
    A.ID_REPRESTAMO,
    NVL(SYS_CONTEXT('APEX$SESSION', 'APP_USER'), USER),
    VMSG
);
...
--5
IF PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('VALIDAR_XCORE_CARGADIRIGIDA') = 'S' THEN PR.PR_PKG_REPRESTAMOS.PVALIDA_XCORE();
...
WHERE
    CODIGO_MVP = 'REPRESTAMOS'
    AND CODIGO_PARAMETRO = 'CARGA_DIRIGIDA_EJECUCIONES';
...
FOR A IN CUR_REPRESTAMO LOOP -- IF A.ESTADO = 'RE' THEN
p_generar_bitacora(
    A.ID_REPRESTAMO,
    NULL,
    'RE',
...
PROCEDURE P_Registrar_Rechazo(
    pIdReprestamo IN VARCHAR2,
    pCanal IN VARCHAR2,
    pIdRechazo IN VARCHAR2,
    pMensaje IN OUT VARCHAR2
) IS vDescripcion VARCHAR2(4000);
...
BEGIN -- Busca la Opcion Seleccionada
BEGIN
SELECT
    R.DESCRIPCION,
    CASE
        pCanal
        WHEN TO_CHAR(vSMS) THEN 'SMS'
        WHEN TO_CHAR(vEMAIL) THEN 'MAIL'
    END INTO vDescripcion,
    vCanalCore
FROM
    PR.PR_V_RECHAZO_REPRESTAMOS r
WHERE
    r.CODIGO_RECHAZO = pIdRechazo;

EXCEPTION
WHEN OTHERS THEN pMensaje := 'Fall¿, Opci¿n de rechazo no encontrada.';
...
--COMMIT;    
EXCEPTION
WHEN OTHERS THEN pMensaje := 'Fallo Registrando la raz¿n de declinaci¿n, Intente m¿s tarde';

RAISE_APPLICATION_ERROR(-20010, pMensaje);

END;

BEGIN IF vNoPromo = pIdRechazo THEN
UPDATE
    PA.PROMOCION_PERSONA p
SET
    P.AUTORIZADO = 'N',
    P.COD_ORIGEN = 'RD'
WHERE
...
EXCEPTION
WHEN OTHERS THEN pMensaje := 'Fallo el sistema de promociones del Core';
...
setError(
    pProgramUnit = > 'P_Registrar_Rechazo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE Enviar_Correo_API(
    ...
    pRespuesta IN OUT VARCHAR2
) IS vUrlBase VARCHAR2(1000) := OBT_PARAMETROS('1', 'PA', 'RUTA_API_ORDS');
...
BEGIN BEGIN APEX_WEB_SERVICE.g_request_headers.delete();
...
vUrlAPI := F_Obt_Parametro_Represtamo('RUTA_API_MANAGER') || 'api/Notification/SendReLoanEmail';

--'http://bma0112:8001/api/Notification/SendReLoanEmail';
...
BEGIN IA.LOGGER.ADDPARAMVALUEV('pEmail', pEmail);
...
setError(
    pProgramUnit = > 'Enviar_Correo_API',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE Enviar_Correo_API_ENCUESTA(
    pIdReprestamo IN NUMBER,
    pEmail IN VARCHAR2,
    pNombres IN VARCHAR2,
    pApellidos IN VARCHAR2,
...
PROCEDURE Enviar_Sms_Api(
    pIdreprestamo IN VARCHAR2,
    pTelefono IN VARCHAR2,
    pNombres IN VARCHAR2,
    pApellidos IN VARCHAR2,
    pTipoNotificacion IN VARCHAR2,
    -- SMS, WHATSAPP
    pFormatoMensaje IN VARCHAR2,
    pMensaje IN VARCHAR2,
    pRespuesta IN OUT VARCHAR2
) IS vNotificacion PA.tNotificacion := PA.tNotificacion();
...
BEGIN IF pTipoNotificacion = 'SMS' THEN BEGIN -- Set Header with Token
APEX_WEB_SERVICE.g_request_headers.delete ();
...
DBMS_OUTPUT.PUT_LINE (
    'IDREPRESTAMO DENTRO DE ENVIAR SMS: ' || pIdReprestamo
);
...
ELSIF pTipoNotificacion = 'WHAT' THEN BEGIN -- Usar API de WHATSAPP
...
BEGIN IA.LOGGER.ADDPARAMVALUEV('pTelefono', pTelefono);
...
PROCEDURE Enviar_SMS_API_DESBLOQUEO(
    pIdreprestamo IN VARCHAR2,
    pTelefono IN VARCHAR2,
    pNombres IN VARCHAR2,
    pApellidos IN VARCHAR2,
...
setError(
    pProgramUnit = > 'Enviar_SMS_API_DESBLOQUEO',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE Enviar_Sms_Api_ENCUESTA(
    pIdReprestamo IN NUMBER,
    pTelefono IN VARCHAR2,
    pNombres IN VARCHAR2,
    pApellidos IN VARCHAR2,
...
vUrlAPI := F_Obt_Parametro_Represtamo('RUTA_API_MANAGER') || 'api/Notification/ForwardingLinkByEmail';
...
FUNCTION F_Reenviar_Represtamo(
    pIdReprestamoAnt IN NUMBER,
    pUsuario IN VARCHAR2,
    pError IN OUT VARCHAR2
) RETURN NUMBER IS CURSOR cReprestamo(pIdReprestamo IN NUMBER) IS
SELECT
    *
FROM
    PR.PR_REPRESTAMOS r
...
FROM
    PR.PR_SOLICITUD_REPRESTAMO S
WHERE
    S.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
    AND S.ID_REPRESTAMO = pIdReprestamo;
...
FROM
    PR.PR_OPCIONES_REPRESTAMO o
WHERE
    o.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
    AND o.ID_REPRESTAMO = pIdReprestamo
    AND o.PLAZO > 0;
...
BEGIN -- Busca datos del represtamo anterior
OPEN cReprestamo(pIdReprestamoAnt);

LOOP FETCH cReprestamo BULK COLLECT INTO vReprestamo
LIMIT
    1;

FOR i IN 1..vReprestamo.COUNT LOOP vIdReprestamoNuevo := PR_PKG_REPRESTAMOS.f_genera_secuencia;
...
VALUES
    (
        vReprestamo(i).CODIGO_EMPRESA,
        vIdReprestamoNuevo,
        vReprestamo(i).CODIGO_CLIENTE,
        vReprestamo(i).FECHA_CORTE,
        vReprestamo(i).NO_CREDITO,
        'RE',
        vReprestamo(i).CODIGO_PRECALIFICACION,
        vReprestamo(i).DIAS_ATRASO,
        vReprestamo(i).FECHA_PROCESO,
        PA.PKG_NOTIFICACIONES.GENERAR_PIN_RANDOM(100, 999999),
        PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('MAX_INTENTOS_PIN'),
        PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('MAX_INTENTOS_IDENTIFICACION'),
        'N',
        vReprestamo(i).MTO_CREDITO_ACTUAL,
        vReprestamo(i).MTO_PREAPROBADO,
        vReprestamo(i).OBSERVACIONES,
        pUsuario,
        SYSDATE,
        'RE'
    );
...
EXCEPTION
WHEN OTHERS THEN pError := 'Error Insertando nuevo Represtamo';
...
IF vIdReprestamoNuevo IS NOT NULL THEN --  Inserta los canales del Represtamo Nuevo
...
FOR x IN 1..vCanal.COUNT LOOP BEGIN
INSERT INTO
    PR.PR_CANALES_REPRESTAMO (
        CODIGO_EMPRESA,
        ID_REPRESTAMO,
...
EXCEPTION
WHEN OTHERS THEN pError := 'Error Insertando los canales del Represtamo';
...
DBMS_OUTPUT.PUT_LINE(
    'INSERT CANAL ' || vIdReprestamoNuevo || ' CANAL:' || vCanal(x).CANAL
);
...
BEGIN OPEN cOpcion(pIdReprestamoAnt);

LOOP FETCH cOpcion BULK COLLECT INTO vOpcion
LIMIT
    500;

FOR o IN 1..vOpcion.COUNT LOOP BEGIN
INSERT INTO
    PR.PR_OPCIONES_REPRESTAMO (
        CODIGO_EMPRESA,
        ID_REPRESTAMO,
...
VALUES
    (
        vOpcion(o).CODIGO_EMPRESA,
        vIdReprestamoNuevo,
        vOpcion(o).PLAZO,
        vOpcion(o).MTO_PRESTAMO,
        vOpcion(o).MTO_DESCONTAR,
        vOpcion(o).MTO_DEPOSITAR,
        vOpcion(o).MTO_CUOTA,
        vOpcion(o).MTO_CARGOS,
        vOpcion(o).MTO_SEGURO_VIDA,
        vOpcion(o).MTO_SEGURO_DESEMPLEO,
        vOpcion(o).TASA,
        vOpcion(o).ORDEN,
        vOpcion(o).ESTADO,
        pUsuario,
        SYSDATE,
        vOpcion(o).MTO_CUOTA_TOTAL
    );

EXCEPTION
WHEN OTHERS THEN pError := 'Error Insertando las opciones del Represtamo ';
...
DBMS_OUTPUT.PUT_LINE(
    'INSERT OPCION ' || vIdReprestamoNuevo || ' PLAZO:' || vOpcion(o).PLAZO
);

END LOOP;

EXIT
WHEN cOpcion % NOTFOUND;
...
WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE(
    'DATOS DE OPCIONES NO ENCONTRADOS ' || pIdReprestamoAnt || ' ' || SQLERRM
);
...
BEGIN -- Generar los datos de la solicitud Solicitud
OPEN cSolicitud(pIdReprestamoAnt);

LOOP FETCH cSolicitud BULK COLLECT INTO vSolicitud
LIMIT
    500;

FOR s IN 1..vSolicitud.COUNT LOOP BEGIN
INSERT INTO
    PR.PR_SOLICITUD_REPRESTAMO (
        CODIGO_EMPRESA,
        ID_REPRESTAMO,
...
VALUES
    (
        vSolicitud(s).CODIGO_EMPRESA,
        vIdReprestamoNuevo,
        vSolicitud(s).NOMBRES,
        vSolicitud(s).APELLIDOS,
        vSolicitud(s).IDENTIFICACION,
        vSolicitud(s).FEC_NACIMIENTO,
        vSolicitud(s).SEXO,
        vSolicitud(s).NACIONALIDAD,
        vSolicitud(s).ESTADO_CIVIL,
        vSolicitud(s).TELEFONO_CELULAR,
        vSolicitud(s).TELEFONO_RESIDENCIA,
        vSolicitud(s).TELEFONO_TRABAJO,
        vSolicitud(s).EMAIL,
        vSolicitud(s).COD_DIRECCION,
        vSolicitud(s).TIP_DIRECCION,
        vSolicitud(s).DIRECCION,
        vSolicitud(s).PLAZO,
        vSolicitud(s).ESTADO,
        pUsuario,
        SYSDATE
...
EXCEPTION
WHEN OTHERS THEN pError := 'Error Insertando los datos de la solicitud del Represtamo';
...
EXCEPTION
WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE(
    'DATOS DE SOLICITUD NO ENCONTRADOS ' || pIdReprestamoAnt || ' ' || SQLERRM
);
...
PR_PKG_REPRESTAMOS.p_generar_bitacora(
    ...
    'AN',
    NULL,
    'Anulado por Solicitud nuevo Link',
    pUsuario
);
...
PR_PKG_REPRESTAMOS.p_generar_bitacora(
    ...
    'VR',
    NULL,
    'Preparado para la Validaci¿n Riesgo',
    pUsuario
);
...
PR_PKG_REPRESTAMOS.p_generar_bitacora(
    ...
    'NP',
    NULL,
    'Validaci¿n realizada por Riesgo',
    pUsuario
);
...
BEGIN IA.LOGGER.ADDPARAMVALUEN('pIdReprestamoAnt', pIdReprestamoAnt);

IA.LOGGER.ADDPARAMVALUEV('pUsuario', pUsuario);

setError(
    pProgramUnit = > 'F_Reenviar_Represtamo',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Anular_Represtamos_Inactivos(pIdReprestamo IN NUMBER DEFAULT NULL) IS -- Elaborado por Jose D¿az. 22/08/2022
...
WHERE
    ...
    and ESTADO in (
        ...
        FROM
            TABLE(
                PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_ANULAR_CREDITOS_CANCELADOS')
            )
    )
    and not exists (
        select
        ...
        where
            ...
            and estado in (
                ...
                FROM
                    TABLE(
                        PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_ANULAR_CREDITOS')
                    )
            )
    )
    and not exists(
        ...
        from
            pr_creditos_hi h
        where
            h.no_credito = a.no_credito
            and h.F_CANCELACION >= SYSDATE - TO_NUMBER(
                PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('DIAS_CANCELACION')
            )
...
BEGIN --Se procede a anular los represtamos asociados cr¿ditos que se cancelan en el dia por abono normal o cancelaci¿n de cr¿dito
FOR a in CUR_Anular_creditos_cancelados LOOP PR_PKG_REPRESTAMOS.p_generar_bitacora(
    a.id_represtamo,
    NULL,
    'CC',
    NULL,
    'Anulacion por cancelaci¿n de credito ' || a.no_credito,
    USER
);

END LOOP;

FOR a in CUR_Anular LOOP PR_PKG_REPRESTAMOS.p_generar_bitacora(
    a.id_represtamo,
    NULL,
    'AN',
    NULL,
...
FOR a in CUR_Anular_campana_especiales LOOP PR_PKG_REPRESTAMOS.p_generar_bitacora(
    a.id_represtamo,
    NULL,
    'AN',
    NULL,
...
BEGIN IA.LOGGER.ADDPARAMVALUEN('pId_represtamo', pIdReprestamo);

setError(
    pProgramUnit = > 'P_Anular_Represtamos_Inactivos',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FOR i IN 1..vCanal.COUNT LOOP IF vCanal(i).Canal = vSMS THEN -- Reemplazar el mensaje del parametro 
vMensaje := REPLACE(
    PR.PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('TEXTO_SMS_ESTADO'),
    '[ESTADO]',
    PR.PR_PKG_REPRESTAMOS.F_Obt_Descripcion_Estado(pCodigoEstado)
);
...
DBMS_OUTPUT.PUT_LINE (
    'IDREPRESTAMO DENTRO DEL P_Notificar_Estado: ' || vNotificacion.MENSAJE
);

DBMS_OUTPUT.PUT_LINE (
    'vNotificacion.CONTACTO: ' || vNotificacion.CONTACTO
);
...
FUNCTION F_Validar_Existe_IdDeclinar(pIdDeclinar IN VARCHAR2) RETURN BOOLEAN IS vExiste PLS_INTEGER := 0;
...
FROM
    PR.PR_V_RECHAZO_REPRESTAMOS R
WHERE
    R.CODIGO_RECHAZO = pIdDeclinar;

RETURN (vExiste > 0);

END F_Validar_Existe_IdDeclinar;

FUNCTION F_Validar_Tipo_Represtamo(pIdReprestamo IN NUMBER) RETURN BOOLEAN IS vExiste PLS_INTEGER := 0;
...
FROM
    PR.PR_REPRESTAMOS R
WHERE
    R.ID_REPRE_CAMPANA_ESPECIALES IS NOT NULL
    AND R.ID_REPRESTAMO = pIdReprestamo;
...
FUNCTION F_Validar_Tipo_Represtamo_Carga(pIdReprestamo IN NUMBER) RETURN BOOLEAN IS vExiste PLS_INTEGER := 0;
...
FROM
    PR.PR_REPRESTAMOS R
WHERE
    R.ID_CARGA_DIRIGIDA IS NOT NULL
    AND R.ID_REPRESTAMO = pIdReprestamo;

RETURN (vExiste > 0);

END F_Validar_Tipo_Represtamo_Carga;

FUNCTION F_Validar_Existe_Estado(pCodigoEstado IN VARCHAR2) RETURN BOOLEAN IS vExiste PLS_INTEGER := 0;
...
FROM
    PR.PR_ESTADOS_REPRESTAMO E
WHERE
    E.CODIGO_EMPRESA = F_Obt_Empresa_Represtamo
    AND E.CODIGO_ESTADO = pCodigoEstado;
...
FUNCTION F_Validar_Listas_PEP (
    ...
    p_codpersona IN VARCHAR2
) RETURN NUMBER IS BEGIN IF PA.P_DATOS_PERSONA.esta_en_lista_pep(p_codempresa, p_codpersona) THEN RETURN 1;
...
FUNCTION F_Validar_Lista_NEGRA (
    p_codempresa IN VARCHAR2,
    p_codpersona IN VARCHAR2
) RETURN NUMBER IS BEGIN IF PA.P_DATOS_PERSONA.esta_en_lista_negra(p_codempresa, p_codpersona) THEN RETURN 1;
...
FUNCTION F_Existe_Represtamo(pIdReprestamo IN NUMBER) RETURN BOOLEAN IS vExiste PLS_INTEGER := 0;
...
FUNCTION F_Existe_Solicitudes(pIdReprestamo IN NUMBER) RETURN BOOLEAN IS vExiste PLS_INTEGER := 0;
...
FUNCTION F_Existe_Credito(pIdReprestamo IN NUMBER) RETURN BOOLEAN IS vExiste PLS_INTEGER := 0;
...
FUNCTION F_Existe_Canales(pIdReprestamo IN NUMBER) RETURN BOOLEAN IS vExiste PLS_INTEGER := 0;

BEGIN
SELECT
    COUNT(1) INTO vExiste
FROM
    PR.PR_CANALES_REPRESTAMO R
WHERE
    R.CODIGO_EMPRESA = F_Obt_Empresa_Represtamo
    AND R.ID_REPRESTAMO = pIdReprestamo;
...
FUNCTION F_Existe_Opciones(pIdReprestamo IN NUMBER) RETURN BOOLEAN IS vExiste PLS_INTEGER := 0;

BEGIN
SELECT
    COUNT(1) INTO vExiste
FROM
    PR.PR_OPCIONES_REPRESTAMO R
WHERE
    R.CODIGO_EMPRESA = F_Obt_Empresa_Represtamo
    AND R.ID_REPRESTAMO = pIdReprestamo;
...
FUNCTION F_Obtener_Total_SMS_Enviados(pIdReprestamo IN NUMBER) RETURN NUMBER IS TOTAL NUMBER;
...
FROM
    xmltable (
        '/json/row' passing apex_json.to_xmltype(
...
FUNCTION F_Obtener_Nuevo_Credito(pIdReprestamo IN NUMBER) RETURN NUMBER IS NUEVO_TIPO NUMBER;

CURSOR CREDITO IS
SELECT
    TIPO_CREDITO_DESTINO
FROM
    PR.PR_REPRESTAMO_CAMPANA_DET
WHERE
    TIPO_CREDITO_ORIGEN = (
        SELECT
            TIPO_CREDITO
...
BEGIN IF PR.PR_PKG_REPRESTAMOS.F_Validar_Tipo_Represtamo(pIdReprestamo) THEN DBMS_OUTPUT.PUT_LINE('EL REPRESTAMO ES DE CAMPAÑA ESPECIAL');

FOR A IN CREDITO LOOP BEGIN
SELECT
    T.TIPO_CREDITO INTO NUEVO_TIPO
FROM
    PR.PR_TIPO_CREDITO_REPRESTAMO T,
    PR.PR_REPRESTAMOS R
WHERE
    T.TIPO_CREDITO = A.TIPO_CREDITO_DESTINO
    AND R.MTO_PREAPROBADO >= (
        SELECT
            MIN(MONTO_MIN)
        FROM
            PR.PR_PLAZO_CREDITO_REPRESTAMO
        WHERE
            TIPO_CREDITO = T.TIPO_CREDITO
    )
    AND R.MTO_PREAPROBADO <= (
        SELECT
            MAX(MONTO_MAX)
...
-- Retorna el valor encontrado y sale de la función
EXCEPTION
WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE(
    'No se encontró un nuevo tipo de crédito para el TIPO_CREDITO_DESTINO = ' || A.TIPO_CREDITO_DESTINO
);
        ...
        WHERE
            ...
            AND EXISTS (
                SELECT
                    1
                FROM
                    PR.PR_TIPO_CREDITO_REPRESTAMO
                WHERE
                    TIPO_CREDITO IN (NT.TIPO_CREDITO)
                    AND OBSOLETO = 0
                    AND (
                        CREDITO_CAMPANA_ESPECIAL != 'S'
                        OR CREDITO_CAMPANA_ESPECIAL IS NULL
                    )
            )
    ) INTO NUEVO_TIPO
FROM
    ...
    LEFT JOIN PR.PR_TIPO_CREDITO T ON T.TIPO_CREDITO = C.TIPO_CREDITO
    OR T.TIPO_CREDITO = H.TIPO_CREDITO
WHERE
    R.ID_REPRESTAMO = pIdReprestamo;
...
DBMS_OUTPUT.PUT_LINE (
    'NUEVO CREDITO ' || NUEVO_TIPO || 'REPRESTAMOS' || pIdReprestamo
);

EXCEPTION
WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE(
    'No se encontró un nuevo tipo de crédito en la parte ELSE'
);
...
FUNCTION F_Obtener_Credito_Cancelado(pIdReprestamo IN NUMBER, PMONTO IN NUMBER) RETURN NUMBER IS NUEVO_TIPO NUMBER;
...
BEGIN IF PR.PR_PKG_REPRESTAMOS.F_Validar_Tipo_Represtamo(pIdReprestamo) THEN FOR A IN CREDITO LOOP BEGIN
SELECT
    (
        SELECT
            NT.TIPO_CREDITO
...
EXCEPTION
WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE(
    'No se encontró un nuevo tipo de crédito para el TIPO_CREDITO_DESTINO = ' || A.TIPO_CREDITO
);
...
SELECT
    (
        ...
        WHERE
            PMONTO BETWEEN P.MONTO_MIN
            AND P.MONTO_MAX -- Coincidencias obligatorias
            ...
            AND (
                T.TIPO_CREDITO IN (
                    SELECT
                        TIPO_CREDITO
                    FROM
                        PR_TIPO_CREDITO_REPRESTAMO
                    WHERE
                        CREDITO_FMO = 'S'
                )
                OR T.FACILIDAD_CREDITIC = NT.FACILIDAD_CREDITIC
            ) --  El candidato final nunca puede ser FMO
            AND NT.TIPO_CREDITO NOT IN (
                SELECT
                    TIPO_CREDITO
                FROM
                    PR_TIPO_CREDITO_REPRESTAMO
            ...
            AND EXISTS (
                ...
                FROM
                    PR_TIPO_CREDITO_REPRESTAMO R
                WHERE
                    R.TIPO_CREDITO = NT.TIPO_CREDITO
                    AND R.OBSOLETO = 0
...
FROM
    ...
    LEFT JOIN PR_TIPO_CREDITO T ON T.TIPO_CREDITO = C.TIPO_CREDITO
    OR T.TIPO_CREDITO = H.TIPO_CREDITO
WHERE
    R.ID_REPRESTAMO = pIdReprestamo;
...
EXCEPTION
WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE(
    'No se encontró un nuevo tipo de crédito para el ID_REPRESTAMO = ' || pIdReprestamo
);
...
FUNCTION F_Obtener_plazo(
    pIdReprestamo IN NUMBER,
    pMtoSeleccionado IN NUMBER
) RETURN NUMBER IS NUEVO_PLAZO NUMBER;

V_TIPO_CREDITO NUMBER;

BEGIN -- Verificar si el tipo de préstamo es válido usando la función F_Validar_Tipo_Represtamo
IF PR.PR_PKG_REPRESTAMOS.F_Validar_Tipo_Represtamo(pIdReprestamo) THEN -- Iterar sobre los créditos si la validación es exitosa
BEGIN
SELECT
    MAX(R.PLAZO) INTO NUEVO_PLAZO
FROM
    PR_PLAZO_CREDITO_REPRESTAMO R
WHERE
    R.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.f_obt_Empresa_Represtamo
    AND R.TIPO_CREDITO = (
        SELECT
    ...
    AND pMtoSeleccionado BETWEEN R.MONTO_MIN
    AND R.MONTO_MAX
GROUP BY
    R.TIPO_CREDITO;
...
ELSE BEGIN -- Si la validación no es exitosa, ejecutar un bloque alternativo
SELECT
    MAX(R.PLAZO) INTO NUEVO_PLAZO
FROM
    PR_PLAZO_CREDITO_REPRESTAMO R
...
BEGIN pMensaje := 'ERROR EN LA FUNCION F_OBTENER_PLAZO';

setError(
    pProgramUnit = > 'F_OBTENER_PLAZO',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FUNCTION F_Existe_Plazo(
    pTipoCredito IN VARCHAR2,
    pPlazo IN NUMBER
) RETURN BOOLEAN IS vExiste PLS_INTEGER := 0;

BEGIN
SELECT
    COUNT(1) INTO vExiste
FROM
    PR.PR_PLAZO_CREDITO_REPRESTAMO P
WHERE
    P.CODIGO_EMPRESA = F_Obt_Empresa_Represtamo
...
FUNCTION F_TIENE_GARANTIA(pNoCredito IN NUMBER) RETURN NUMBER IS vExiste NUMBER := 0;
...
FROM
    ...
    PR_GARANTIAS C
WHERE
    A.codigo_empresa = F_Obt_Empresa_Represtamo
...
FUNCTION F_TIENE_GARANTIA_HISTORICO(pNoCredito IN NUMBER) RETURN NUMBER IS vExiste NUMBER := 0;
...
FUNCTION F_Obtiene_Desc_Bitacora(
    pIdReprestamo IN NUMBER,
    pEstado IN VARCHAR2
) RETURN VARCHAR2 IS vDescripcion PR.PR_BITACORA_REPRESTAMO.OBSERVACIONES % TYPE;

BEGIN BEGIN
SELECT
    B.OBSERVACIONES INTO vDescripcion
FROM
    PR.PR_BITACORA_REPRESTAMO B
WHERE
    B.CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
...
FUNCTION F_HORARIO_VALIDO_NOTIFICACION(pfecha IN DATE) RETURN NUMBER IS v_valido NUMBER;
...
SELECT
    COUNT(1) INTO v_validacion_feriado
FROM
    TABLE(
        PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros('NOT_DIAS_FERIADOS')
    )
WHERE
    TRIM(COLUMN_VALUE) IN (TRIM(TO_CHAR(pfecha, 'DD/MM/YYYY')));

IF v_validacion_feriado > 0 THEN RETURN 0;

END IF;

/*VALIDACION DE DIAS SEMANA HABILES*/
SELECT
    COUNT(1) INTO v_validacion_dias_semana
FROM
    TABLE(
        PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros('NOT_DIAS_SEMANA_VALIDOS')
    )
WHERE
    TRIM(COLUMN_VALUE) IN (TRIM(TO_CHAR(pfecha, 'DAY')));

SELECT
    TRIM(
        REGEXP_SUBSTR(
            PR.PR_PKG_REPRESTAMOS.F_Obt_Parametro_Represtamo('NOT_HORARIOS_SEMANA_VALIDOS'),
            '[^,]+',
            1,
            1
        )
    ) INTO v_hora_ini_semama
FROM
    DUAL;
...
SELECT
    ...
    ) INTO v_hora_fin_semama
FROM
    DUAL;

IF v_validacion_dias_semana > 0
AND TO_CHAR(pfecha, 'HH24:Mi') >= v_hora_ini_semama
AND TO_CHAR(pfecha, 'HH24:Mi') <= v_hora_fin_semama THEN RETURN 1;

END IF;

/*VALIDACION DE DIAS FIN SEMANA HABILES*/
SELECT
    COUNT(1) INTO v_validacion_dias_fin_semana
FROM
    TABLE(
        PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros('NOT_DIAS_FIN_SEMANA_VALIDOS')
    )
WHERE
    TRIM(COLUMN_VALUE) IN (TRIM(TO_CHAR(pfecha, 'DAY')));
...
SELECT
    TRIM(
        REGEXP_SUBSTR(
            PR.PR_PKG_REPRESTAMOS.F_Obt_Parametro_Represtamo('NOT_HORARIOS_FIN_SEMANA_VALIDOS'),
            '[^,]+',
            1,
            1
        )
    ) INTO v_hora_ini_fin_semama
FROM
    DUAL;
...
SELECT
    ...
    ) INTO v_hora_fin_fin_semama
FROM
    DUAL;

IF v_validacion_dias_fin_semana > 0
AND TO_CHAR(pfecha, 'HH24:Mi') >= v_hora_ini_fin_semama
AND TO_CHAR(pfecha, 'HH24:Mi') <= v_hora_fin_fin_semama THEN RETURN 1;
...
UPDATE
    PR.PR_SOLICITUD_REPRESTAMO S
SET
    S.NO_CREDITO = pNuevo_credito,
    S.ID_TEMPFUD = pIdTempfud,
    S.NOMARCHIVO = pNombreArchivo
WHERE
    S.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
...
BEGIN IA.LOGGER.ADDPARAMVALUEV('pIdReprestamo', pNum_Represtamo);
...
PROCEDURE P_Actualiza_Fud(
    ...
    pError OUT VARCHAR2
) IS vTempFud PR_SOLICITUD_REPRESTAMO.ID_TEMPFUD % TYPE := 0;
...
-- Prestamos
vIdTipoDocumento PLS_INTEGER := '452';

-- Formulario de Conozca
...
SELECT
    ...
    S.NOMARCHIVO,
    R.CODIGO_CLIENTE,
    R.NO_CREDITO CREDITO_ANTERIOR,
    S.NO_CREDITO CREDITO_NUEVO,
    S.PLAZO,
    O.TASA,
    O.MTO_PRESTAMO,
    CODIGO_AGENCIA INTO vTempFud,
    vNomArchivo,
    vCodigoCliente,
    vCreditoAnterior,
    vCreditoNuevo,
...
WHERE
    ...
    AND S.ESTADO IN (
        ...
        FROM
            TABLE(
                PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_PARA_ACTUALIZACION_FUD')
            )
    )
...
EXCEPTION
WHEN NO_DATA_FOUND THEN vTempFud := 0;
...
EXCEPTION
WHEN OTHERS THEN pError := 'Error actualizando la solicitud ' || SQLERRM;
...
DECLARE vResultado PA.PKG_DIR_PERSONAS.RESULTADO;

BEGIN PA.PKG_DIR_PERSONAS.Generar (
    pCod_Persona = > vCodigoCliente,
    pCod_Direccion = > null,
    --NVL(PA.OBTIENE_COD_DIRECCION (vCodigoCliente, pError),1) + 1,
    pTip_Direccion = > 1,
...
EXCEPTION
WHEN OTHERS THEN pError := 'Error generando la direcci¿n del cliente ' || vResultado.DESCRIPCION || ' ' || SQLERRM;
...
IF vTempFud <> 0 THEN
SELECT
    pais_sb INTO v_idpais
FROM
    pais
WHERE
    cod_pais = pCodPais;

DBMS_OUTPUT.PUT_LINE ('pCodPais = ' || pCodPais);

DBMS_OUTPUT.PUT_LINE ('pCodProvincia = ' || pCodProvincia);

DBMS_OUTPUT.PUT_LINE ('pCodCanton = ' || pCodCanton);

DBMS_OUTPUT.PUT_LINE ('pCodDistrito = ' || pCodDistrito);

Select
    lpad(RPAD(cod_canton, '0', 2), '0', 6) provincia,
    lpad(
        RPAD(cod_canton, '0', 2) || RPAD(cod_distrito, '0', 2),
        6
    ) municipio,
    lpad(
        RPAD(cod_canton, '0', 2) || RPAD(cod_distrito, '0', 2) || RPAD(cod_pueblo, '0', 2),
        6
    ) distrrito Into v_idprovincia,
    v_idmunicipio,
    v_distrito
From
    pa.pueblos
...
SET
    ...
    T.TasaCal = vTasa,
    T.MONTOCAL = vMonto,
    T.TRABAJO_DIRECCION = pTrabajoDireccion --T.IDAGENCIA = vIDAGENCIA
WHERE
    T.ID_TEMPFUD = vTempFud
    AND T.NOMARCHIVO = vNomArchivo;

EXCEPTION
WHEN OTHERS THEN pError := 'Error actualizando la FUD ' || SQLERRM;
...
UPDATE
    PA.PERSONAS_FISICAS PF
SET
    PF.EMAIL_USUARIO = pEmail,
    PF.TIPO_GEN_DIVISAS = pTipoGeneradorDivisas,
    PF.OCUPACION_CLASIF_NAC = pOcupacion
WHERE
    PF.COD_PER_FISICA = vCodigoCliente;

EXCEPTION
WHEN OTHERS THEN pError := 'Error actualizando datos de la persona ' || SQLERRM;
...
IF pTelefono IS NOT NULL THEN DECLARE vResultado PA.PKG_TEL_PERSONAS.RESULTADO;

BEGIN vCodArea := SUBSTR(pTelefono, 1, 3);

vNumTel := SUBSTR(pTelefono, 4);

--Descomentar que ira a produccion         
PA.PKG_TEL_PERSONAS.GENERAR (
    PCOD_PERSONA = > VCODIGOCLIENTE,
    PCOD_AREA = > VCODAREA,
...
EXCEPTION
WHEN OTHERS THEN pError := 'Error: ' || pError || ' ' || SQLERRM || ' ' || dbms_utility.format_error_backtrace;

RAISE_APPLICATION_ERROR(-20100, pError);

END;

PROCEDURE P_Procesa_Credito (
    pCodigo_Empresa IN NUMBER,
    pNum_Represtamo IN NUMBER,
    pPeriodicidad IN VARCHAR2 DEFAULT '05',
    --- (2) := '05'; ---In
...
CURSOR c_repre (Empresap NUMBER, pid_represtamop NUMBER) IS
SELECT
    a.id_represtamo,
    a.codigo_cliente,
    a.no_credito,
    ...
    c.mto_cuota,
    CASE
        WHEN nvl(c.mto_seguro_vida, 0) > 0 then 'TRUE'
        ELSE 'FALSE'
    END seguro_vida,
    CASE
        WHEN nvl(c.mto_cargos, 0) > 0 then 'TRUE'
        ELSE 'FALSE'
    END gastoslegales
FROM
    pr_represtamos a,
...
BEGIN OPEN CUR_tipo_PROYECTO;

FETCH CUR_tipo_PROYECTO INTO vTipoProyecto,
vCodigoEjecutivo;
...
IF c_repre % FOUND THEN dbms_output.put_line('Linea: ' || $ $ plsql_line);
...
FROM
    Pr_Creditos cre
WHERE
    Cre.codigo_Empresa = pCodigo_Empresa
    ...
    AND Cre.estado IN (
        ...
        FROM
            TABLE(
                PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_PROCESAR_CREDITOS')
            )
    );

--('D', 'V', 'M', 'E', 'J');
IF vCount = 0 THEN pError := 'Error: Cr¿dito base no encontrado en cartera vigente';
...
dbms_output.put_line(
    'Linea: ' || $ $ plsql_line || ' vRow_Repre.no_credito ' || vRow_Repre.no_credito
);
...
THEN dbms_output.put_line('Linea: ' || $ $ plsql_line);

BEGIN
SELECT
    SQ_TEMPFUD.NEXTVAL INTO vID_TEMPFUD
FROM
    DUAL;
...
BEGIN dbms_output.put_line(
    'Linea: ' || $ $ plsql_line || ' fudrow.PRIMERNOMBRE ' || fudrow.PRIMERNOMBRE || ' vTipoID: ' || vTipoID
);

--ia.api_portacredit.pcrearFud 
vCodigoEjecutivo := nvl(vCodigoEjecutivo, fudrow.IDOFICIAL);

PR.PR_PKG_REPRESTAMOS.P_Obtener_Nombres_Cliente (
    pNum_Represtamo,
    vPrimerNombre,
    vSegundoNombre,
    vPrimerApellido,
...
PR.PKG_SOLICITUD_CREDITO.PCREARFUD (
    ...
    pCODIGOPROYECTO = > vTipoProyecto,
    --fudrow.CODIGOPROYECTO),
    ...
    pMONTOCAL = > vRow_Repre.mto_prestamo,
    --fudrow.MONTOCAL,  ---Valor solicitud
    pPLAZOCAL = > vRow_Repre.plazo,
    -- fudrow.PLAZOCAL,   ----Valor solicitud
    pMONEDACAL = > fudrow.MONEDACAL,
    pDIAPAGOCAL = > fudrow.DIAPAGOCAL,
    pFRECUENCIACAL = > vPeriodicidad,
    ---fudrow.FRECUENCIACAL,  Frecuencia Mensual Fija por el momento
    ...
    pCUOTAPRESTAMOCAL = > vRow_Repre.mto_cuota,
    ---fudrow.CUOTAPRESTAMOCAL,
    pTOTALFINANCIARCAL = > fudrow.TOTALFINANCIARCAL,
    pTASACAL = > vRow_Repre.tasa_interes,
    --fudrow.TASACAL,  --Valor solicitud
    pGASTOSLEGALESCAL = > vRow_Repre.GASTOSLEGALES,
    -------------
    pSEGUROVIDACAL = > vRow_Repre.SEGURO_VIDA,
    ---fudrow.SEGUROVIDACAL,
    ...
    pFECHA_REGISTRO = > vFecha,
    --sysdate,--fudow.FECHA_REGISTRO,
...
IF vERRORDESCRIPTION IS NOT NULL THEN pError := 'Linea ' || $ $ plsql_line || ' Error creando tempfud vERRORDESCRIPTION' || vERRORDESCRIPTION;
...
EXCEPTION
WHEN OTHERS THEN pError := 'Linea ' || $ $ plsql_line || ' sqlerrm ' || SQLERRM;
...
SELECT
    NOCREDITO,
    NOMARCHIVO INTO vpNOCREDITOGenerado,
    vNombreArchivoNuevoGenerado
FROM
    TEMPFUD
WHERE
    ID_TEMPFUD = vid_tempfudGenerado;

pNuevo_credito := vpNOCREDITOGenerado;

DBMS_OUTPUT.put_line (
    'vNombreArchivoNuevoGenerado: ' || vNombreArchivoNuevoGenerado
);
...
IF vpNOCREDITOGenerado IS NOT NULL THEN BEGIN pr.pkg_recredito.p_genera_recredito (
...
DBMS_OUTPUT.put_line (
    'vNombreArchivoNuevo: ' || vNombreArchivoNuevoGenerado
);
...
IF pNuevo_credito IS NOT NULL
AND vid_tempfudGenerado IS NOT NULL
AND vNombreArchivoNuevoGenerado IS NOT NULL THEN -- Actualiza el numero de credito en Solicitud
PR_PKG_REPRESTAMOS.P_Actualiza_Credito_Solicitud(
    pNum_Represtamo,
    pNuevo_credito,
    vid_tempfudGenerado,
    vNombreArchivoNuevoGenerado,
...
ELSE pError := 'Linea ' || $ $ plsql_line || ' No se ha generado la Fud Correctamente pNuevo_credito: ' || pNuevo_credito || ' vID_TEMPFUD: ' || vid_tempfudGenerado || 'vNombreArchivoNuevo: ' || vNombreArchivoNuevoGenerado;
...
DECLARE vURL VARCHAR2(4000);

vIdAplication PLS_INTEGER := 2;

-- Prestamos
vIdTipoDocumento PLS_INTEGER := '231';
...
BEGIN -- Documento Inicial con el credito anterior para la carga
...
EXCEPTION
WHEN OTHERS THEN pError := pError || ' ' || dbms_utility.format_error_backtrace;

RAISE_APPLICATION_ERROR(-20100, pError);

END;

ELSE pError := 'Linea ' || $ $ plsql_line || ' No se ha insertado correctamente la URL del reporte vTipoID: ' || vTipoID || ' fudrow.NumDocumentoIdentidad: ' || fudrow.NumDocumentoIdentidad || ' pNuevo_credito: ' || pNuevo_credito || ' vRow_Repre.no_credito ' || vRow_Repre.no_credito;
...
ELSE pError := 'Error: id_represtamo ' || pNum_Represtamo || ' NO encontrado';
...
PROCEDURE P_Procesa_Credito_Cancelado (
    pCodigo_Empresa IN NUMBER,
    pNum_Represtamo IN NUMBER,
    pPeriodicidad IN VARCHAR2 DEFAULT '05',
    --- (2) := '05'; ---In
...
SELECT
    TIP_TELEFONO,
    TP.COD_AREA || TP.NUM_TELEFONO AS TELEFONO
FROM
    PA.TEL_PERSONAS TP
WHERE
    TP.COD_PERSONA = pNumCliente;
...
CURSOR CUR_tipo_PROYECTO IS
SELECT
    PR.PR_PKG_REPRESTAMOS.F_OBTENER_CREDITO_CANCELADO (pNum_Represtamo, pMtoPrestamo),
    c.codigo_ejecutivo
FROM
    PR.PR_REPRESTAMOS r,
    pr.PR_PLAZO_CREDITO_REPRESTAMO pl,
...
vTipoProyecto PR_CREDITOS.TIPO_CREDITO % TYPE;
...
IF vCount = 0 THEN pError := 'Error: Crédito base no encontrado en cartera vigente';
...
SELECT
    MTO_SEGURO_MIPYME,
    MTO_SEGURO_DESEMPLEO INTO vSeguroMipyme,
    vSeguroDesempleo
FROM
    PR_OPCIONES_REPRESTAMO
WHERE
    ID_REPRESTAMO = pNum_Represtamo
    AND PLAZO = vRow_Repre.plazo;

IF vSeguroMipyme > 0 THEN vSeguroMipyme := 1;

ELSE vSeguroMipyme := 0;

END IF;

IF vSeguroDesempleo > 0 THEN vSeguroDesempleo := 1;

ELSE vSeguroDesempleo := 0;

END IF;

END;

--- Obtengo el codigo de agencia desde el credito anterior     Norman Olivo 06/08/2025       
SELECT
    CODIGO_AGENCIA INTO vIdAgencia
FROM
    PR.PR_REPRESTAMOS R
    LEFT JOIN PR.PR_CREDITOS C ON C.NO_CREDITO = R.NO_CREDITO
WHERE
    R.ID_REPRESTAMO = pNum_Represtamo;
...
-- DBMS_OUTPUT.put_line ('fudrow.IDPAIS: ' ||vRow_Repre.no_credito);
--DBMS_OUTPUT.put_line ('fudrow.IDPAIS: ' || fudrow.IDPAIS);
BEGIN --dbms_output.put_line('Linea: '||$$plsql_line||' fudrow.PRIMERNOMBRE '||fudrow.PRIMERNOMBRE||' vTipoID: '||vTipoID);
--ia.api_portacredit.pcrearFud 
vCodigoEjecutivo := nvl(vCodigoEjecutivo, fudrow.IDOFICIAL);
...
PR.PKG_SOLICITUD_CREDITO.PCREARFUD (
    ...
    pDIRECCION_IDTIPOVIVIENDA = > fudrow.DIRECCION_IDTIPOVIVIENDA,
    pREF_DOMICILIO = > fudrow.REF_DOMICILIO,
    pTELEFONO_CASA = > fudrow.TELEFONO_CASA_FORMATO,
    --fudrow.TELEFONO_CASA,
    pTELEFONO_CELULAR = > fudrow.TELEFONO_CELULAR_FORMATO,
    --fudrow.TELEFONO_CELULAR,
    pNOMBRENEGOCIO = > fudrow.NOMBRENEGOCIO,
    pNOEMPLEADOS = > fudrow.NOEMPLEADOS,
    pRNC = > fudrow.RNC,
    pFAX = > fudrow.FAX,
    ...
    pUBICACION_NEG = > fudrow.UBICACION_NEG,
    pLUGARTRABAJO = > fudrow.LUGARTRABAJO,
    pFECHAINGRESO = > vFECHA_PROCESADA,
    --fudrow.FECHAINGRESO,
    pCARGO = > fudrow.CARGO,
    pTRABAJO_DIRECCION = > fudrow.TRABAJO_DIRECCION,
    pTRABAJO_IDSECTOR = > fudrow.TRABAJO_IDSECTOR,
...
BEGIN
SELECT
    PR.SQ_TEMPFUD_EXT.NEXTVAL INTO vID_TEMPFUD_EXT
FROM
    DUAL;
...
PR.PKG_SOLICITUD_CREDITO.P_CREAR_FUDExtendida (
    ...
    --PEPS
    pFUNCIONARIOGOB = > NULL,
    --VARCHAR2, --PEPS
    pESPEFICIQUEFUNCI = > NULL,
    pPARENTESCOFUNCI = > NULL,
    --VARCHAR2, --PARENTESCO PEPS
    ...
    pSEGUROINCENDIOpyme = > vSeguroMipyme,
    --malmanzar 16-03-2023
...
IF vERRORDESCRIPTION1 IS NOT NULL THEN pError := 'Linea ' || $ $ plsql_line || ' Error creando tempfud vERRORDESCRIPTION1' || vERRORDESCRIPTION1;
...
-- Para identificar los Represtamos que son de origen Campana Especiales y pueda activar la reutilizacion de la cedula.
IF PR_PKG_REPRESTAMOS.F_VALIDAR_TIPO_REPRESTAMO(pNum_Represtamo) THEN vComentario := 'Recuperacion';

END IF;

vIdTipoDocumento := '450';

-- CONSULTA LEXIS NEXIS SROBLES 28/11/2023
vDocumento := 'LEXISNEXIS';

vCodigoReferencia := '1:' || vRow_Repre.identificacion || ':' || vpNOCREDITOGenerado || ':' || vRow_Repre.no_credito || ':LEXISNEXIS' || ':' || vid_tempfudGenerado || vComentario;
...
setError(
    pProgramUnit = > 'P_PROCESA_CREDITO_CANCELADO',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
FOR A IN ACTUALIZA_TELEFONO LOOP IF NVL(pMtoPrestamo, 0) <= 100000.00 THEN IF A.TELEFONO = vCtelefono
AND A.TIP_TELEFONO = 'C' THEN
UPDATE
    PA.TEL_PERSONAS
SET
    NOTIF_DIGITAL = 'S',
    FECHA_NOTIF_DIGITAL = SYSDATE
WHERE
    TIP_TELEFONO = 'C'
...
SET
    ...
    FECHA_NOTIF_DIGITAL = SYSDATE
WHERE
    COD_PERSONA = vRow_Repre.codigo_cliente
    AND (COD_AREA || NUM_TELEFONO) <> vCtelefono;
...
/*FOR A IN ACTUALIZA_TELEFONO LOOP
 ...
 END IF;
 END LOOP;*/
ELSE pError := 'Error: tempfud no encontrada para el crédito base ' || vRow_Repre.no_credito;
...
PROCEDURE P_GENERA_DOCUMENTOS (pError OUT VARCHAR2) IS CURSOR CARGA_DOCUMENTOS IS
SELECT
    R.NO_CREDITO CREDITO_ANTERIOR,
    S.NO_CREDITO CREDITO_NUEVO,
    ID_TEMPFUD,
    NVL(ID_TEMPFEC, 0) ID_TEMPFEC,
    S.NOMARCHIVO,
    R.CODIGO_CLIENTE,
    S.IDENTIFICACION,
    S.ESTADO
...
WHERE
    ...
    AND R.ESTADO IN (
        ...
        FROM
            TABLE(
                PR.PR_PKG_REPRESTAMOS.F_Obt_Valor_Parametros ('ESTADOS_PARA_ENVIO_SIB_BURO')
            )
    )
...
BEGIN FOR A IN CARGA_DOCUMENTOS LOOP BEGIN
SELECT
    ESTADO_REPORTE,
    CODIGO_REFERENCIA INTO vESTADO,
    VCREDITO
FROM
    PA.PA_REPORTES_AUTOMATICOS
WHERE
    NOMBRE_ARCHIVO = 'LEXISNEXIS_' || A.CREDITO_NUEVO || '_' || A.CREDITO_ANTERIOR || '.pdf';
...
EXCEPTION
WHEN NO_DATA_FOUND THEN vESTADO := 'l';
...
FROM
    PA.PA_REPORTES_AUTOMATICOS
WHERE
    NOMBRE_ARCHIVO = 'BURO' || '_' || A.CREDITO_NUEVO || '_' || A.CREDITO_ANTERIOR || '.pdf';

EXCEPTION
WHEN NO_DATA_FOUND THEN vBuro := 1;
...
FROM
    PA.PA_REPORTES_AUTOMATICOS
WHERE
    NOMBRE_ARCHIVO = 'SIB' || '_' || A.CREDITO_NUEVO || '_' || A.CREDITO_ANTERIOR || '.pdf';

EXCEPTION
WHEN NO_DATA_FOUND THEN vSIB := 1;
...
IF vESTADO = 'S'
AND vBURO = 0
AND vSIB = 0 THEN vIdTipoDocumento := '193';

-- CONSULTA BURO DE CREDITO PRIVADO 
vDocumento := 'BURO';
...
vIdTipoDocumento := '194';

-- CONSULTA BUSCADOR DE GOOGLE
vDocumento := 'SIB';
...
BEGIN setError(
    pProgramUnit = > 'P_GENERA_DOCUMENTOS',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE p_Procesa_Fec(
    ...
    pError OUT VARCHAR2
) IS CURSOR cSol(pIdReprestamo IN NUMBER) IS
SELECT
    R.NO_CREDITO CREDITO_ANTERIOR,
    S.NO_CREDITO CREDITO_NUEVO,
    S.ID_TEMPFUD,
    ...
    S.ESTADO,
    OP.MTO_PRESTAMO,
    TF.ESPECIFIQUEDESTINO,
    PA.F_OBT_DESC_ACTIVIDAD(S.CODIGO_ACTIVIDAD) ACTIVIDAD_ECONOMICA
FROM
    PR.PR_SOLICITUD_REPRESTAMO S,
    PR.PR_REPRESTAMOS R,
...
-- Prestamos
vIdTipoDocumento PLS_INTEGER := '477';
...
IF pNum_Represtamo IS NULL THEN pError := 'Error el campo No.Solicitud no puede estar en blanco';

RETURN;

ELSIF pCODIGO_ACTIVIDAD IS NULL THEN pError := 'Error el campo Actividad Económica no puede estar en blanco';

RETURN;

ELSIF pMARGEN_BRUTO_STD IS NULL THEN pError := 'Error el campo Margen de Ganancia (%) no puede estar en blanco';

RETURN;

ELSIF pGASTOS_OPERATIVOS_STD IS NULL THEN pError := 'Error el campo Costos Operativos (% de ventas) no puede estar en blanco';

RETURN;

ELSIF pVENTAS_MENSUAL IS NULL THEN pError := 'Error el campo Ventas (Ingresos) no puede estar en blanco';

RETURN;

ELSIF pCOSTO_VENTAS IS NULL THEN pError := 'Error el campo Costos de Ventas no puede estar en blanco';

RETURN;

ELSIF pGASTOS_OPERATIVO IS NULL THEN pError := 'Error el campo Costos Operativos  no puede estar en blanco';

RETURN;

ELSIF pOTROS_INGRESOS IS NULL THEN pError := 'Error el campo Otros Ingresos no puede estar en blanco';

RETURN;

ELSIF pGASTOS_FAMILIARES IS NULL THEN pError := 'Error el campo Gastos Familiares no puede estar en blanco';

RETURN;

ELSIF pEXCEDENTE_FAMILIAR IS NULL THEN pError := 'Error el campo Excedente Familiar no puede estar en blanco';

RETURN;

ELSIF pREL_CUOTA_EXCED_FAM IS NULL THEN pError := 'Error el campo Relación Cuota / Excedente Familiar  no puede estar en blanco';
...
IF cSol % FOUND THEN IF vRow.ID_TEMPFEC = 0
OR vRow.ID_TEMPFEC IS NULL then BEGIN
UPDATE
    PR_SOLICITUD_REPRESTAMO
SET
    CODIGO_AGENCIA = pCOD_AGENCIA,
...
WHERE
    CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.F_Obt_Empresa_Represtamo
    AND ID_REPRESTAMO = pNum_Represtamo;

EXCEPTION
WHEN OTHERS THEN pError := 'Error actualizando los datos para generar la FEC. ' || SQLERRM;
...
UPDATE
    PA.PERSONAS_FISICAS PF
SET
    PF.COD_ACTIVIDAD = pCODIGO_ACTIVIDAD
WHERE
    PF.COD_PER_FISICA = vRow.CODIGO_CLIENTE;

EXCEPTION
WHEN OTHERS THEN pError := 'Error actualizando la actividad economica del cliente. ' || SQLERRM;
...
UPDATE
    PA.INFO_LABORAL I
SET
    i.MONTO = pVENTAS_MENSUAL + pOTROS_INGRESOS
WHERE
    i.COD_PER_FISICA = vRow.CODIGO_CLIENTE;

EXCEPTION
WHEN OTHERS THEN pError := 'Error actualizando la informaci¿n laboral del cliente. ' || SQLERRM;
...
SET
    T.ACTIVIDAD_CIIU = (
        SELECT
            I.PLAN_INVERSION
        FROM
            PA.ACTIVIDADES_ECONOMICAS_BC_CIIU A
        ...
        WHERE
            A.SEGREGACION_RD = pCODIGO_ACTIVIDAD
            AND ROWNUM <= 1
    ),
    --Plan de Inversion
    T.IDRAMA_CIIU = pCODIGO_ACTIVIDAD
WHERE
    T.ID_TEMPFUD = vRow.ID_TEMPFUD
    AND T.NOMARCHIVO = vRow.NOMARCHIVO;

EXCEPTION
WHEN OTHERS THEN pError := 'Error actualizando Actividad de la FUD ' || SQLERRM;
...
BEGIN PR.PKG_SOLICITUD_CREDITO.CREA_FEC_SOLICITUD (
    PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO,
    pNum_Represtamo,
    vMensaje,
    pError
...
EXCEPTION
WHEN OTHERS THEN pError := 'Error generando la FEC. ' || SQLERRM;
...
BEGIN
SELECT
    AC.CONCEPTO INTO vCodigoActividad
FROM
    PA.ACTIVIDADES_ECONOMICAS_BC_CIIU AC
    LEFT JOIN PR.PR_SOLICITUD_REPRESTAMO SR ON SR.CODIGO_ACTIVIDAD = AC.SEGREGACION_RD
WHERE
    AC.SEGREGACION_RD = pCODIGO_ACTIVIDAD
...
BEGIN PR.pkg_solicitud_credito.p_operaciones_realizar (
    ...
    /*CASE
     ...
     END,*/
    porigen_fondos = > vCodigoActividad,
    --vRow.ACTIVIDAD_ECONOMICA,
...
SELECT
    NVL(S.ID_TEMPFEC, 0) INTO vIdTemfec
FROM
    PR_SOLICITUD_REPRESTAMO S
WHERE
    CODIGO_EMPRESA = PR.PR_PKG_REPRESTAMOS.F_Obt_Empresa_Represtamo
...
IF vIdTemfec IS NOT NULL
AND vIdTemfec <> 0 THEN -- Generar Reporte BURO
BEGIN
...
vUrl := PA.PKG_TIPO_DOCUMENTO_PKM.UrlConozcaSuCliente(
    pCodCliente = > vRow.CODIGO_CLIENTE,
    pEmpresa = > PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
);
...
/* vIdTipoDocumento     := '204';
 ...
 COMMIT;
 END;   */
IF vTipo_Vida > 0 THEN vIdTipoDocumento := '204';

vDocumento := 'APOLIZA';

--POLIZA SEGURO DE VIDA / ENDOSO / DESISTIMIENTO
vCodigoReferencia := '1:' || vRow.IDENTIFICACION || ':' || vRow.CREDITO_NUEVO || ': :' || vDocumento;
...
IF vTipo_Mipyme > 0 THEN vIdTipoDocumento := '218';

vDocumento := 'SMIPYME';

--POLIZA SEGURO INCENDIO Y LINEAS ALIADAS / ENDOSO / DESESTIMIENTO
vCodigoReferencia := '1:' || vRow.IDENTIFICACION || ':' || vRow.CREDITO_NUEVO || ': :' || vDocumento;
...
FROM
    PR_POLIZAS_X_CREDITO
WHERE
    TO_NUMBER(NO_CREDITO) = TO_NUMBER(vRow.CREDITO_NUEVO)
    AND TO_NUMBER(TIPO_POLIZA) = 24;
...
IF vTipo_Desempleo > 0 THEN vIdTipoDocumento := '882';
...
ELSE pError := 'La FEC ya ha sido generada anteriormente ID:' || vIdTemfec;
...
PROCEDURE setError(
    ...
    pIdError OUT NUMBER
) IS pPackageName CONSTANT IA.LOG_ERROR.PACKAGENAME % TYPE := 'PR_PKG_REPRESTAMOS';

BEGIN IA.LOGGER.LOG(
    INOWNER = > SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA'),
...
IF IA.LOGGER.VPARAMLIST.COUNT > 0 THEN IA.LOGGER.VPARAMLIST.DELETE;
...
---
PROCEDURE P_JOB_CREA_CREDITO_S IS CURSOR cur_represtamos_x_procesar IS
SELECT
    id_represtamo,
    codigo_empresa
FROM
...
pPeriodicidad VARCHAR2 (5) := PR_PKG_REPRESTAMOS.f_obt_parametro_Represtamo('PERIODICIDAD_CUOTA');

--DEFAULT MENSUAL
pNuevo_credito NUMBER;

---Out
pError VARCHAR2 (1000);

BEGIN FOR a in cur_represtamos_x_procesar LOOP PR_PKG_REPRESTAMOS.P_Procesa_Credito (
    a.Codigo_Empresa,
    a.id_represtamo,
    pPeriodicidad,
    --- (2) := '05'; ---In
...
PROCEDURE P_JOB_CREA_CREDITO IS CURSOR cur_represtamos_x_procesar IS
SELECT
    R.id_represtamo,
    R.codigo_empresa,
    O.MTO_PRESTAMO,
...
BEGIN FOR a in cur_represtamos_x_procesar LOOP PR_PKG_REPRESTAMOS.P_Procesa_Credito_Cancelado (
    a.Codigo_Empresa,
    a.id_represtamo,
    pPeriodicidad,
    --- (2) := '05'; ---In
...
SELECT
    NO_CREDITO INTO V_NO_CREDITO
FROM
    PR.PR_SOLICITUD_REPRESTAMO
WHERE
    ID_REPRESTAMO = A.ID_REPRESTAMO;

IF V_NO_CREDITO IS NOT NULL
AND A.ESTADO IN ('SC') THEN
UPDATE
    PR.PR_REPRESTAMOS
SET
    ESTADO = 'CRS'
...
PROCEDURE P_JOB_CREA_ACTUALIZA_CORE IS CURSOR c_creditos IS
SELECT
    NO_CREDITO,
    TIPO_CREDITO,
    MONTO_CREDITO,
...
FROM
    PR_CREDITOS
WHERE
    FECHA_MODIFICACION >= SYSDATE - (5 /(24 * 60));
...
BEGIN FOR A IN C_CREDITOS LOOP
SELECT
    S.NO_CREDITO,
    S.TIPO_CREDITO,
    O.MTO_PRESTAMO,
    S.PLAZO,
    O.MTO_SEGURO_DESEMPLEO,
    O.MTO_SEGURO_MIPYME,
    S.ID_REPRESTAMO INTO VNO_CREDTIO,
    VTIPO_CREDITO,
    VMONTO,
    VPLAZO,
    MTO_SEGURO_DESEMPLEO,
...
FROM
    PR_SOLICITUD_REPRESTAMO S
    LEFT JOIN PR_OPCIONES_REPRESTAMO O ON O.ID_REPRESTAMO = S.ID_REPRESTAMO
WHERE
    S.NO_CREDITO = A.NO_CREDITO;

BEGIN
SELECT
    MONTO_A_PAGAR INTO MONTO_A_PAGAR_17
FROM
    PR_POLIZAS_X_CREDITO
WHERE
    NO_CREDITO = A.NO_CREDITO
...
EXCEPTION
WHEN NO_DATA_FOUND THEN MONTO_A_PAGAR_17 := 0;
...
BEGIN
SELECT
    MONTO_A_PAGAR INTO MONTO_A_PAGAR_24
FROM
    PR_POLIZAS_X_CREDITO
WHERE
    NO_CREDITO = A.NO_CREDITO
...
EXCEPTION
WHEN NO_DATA_FOUND THEN MONTO_A_PAGAR_24 := 0;
...
SET
    R.ESTADO = 'CRN'
WHERE
    R.ID_REPRESTAMO = ID_REPRESTAMO;

END IF;

-- DBMS_OUTPUT.PUT_LINE ( 'A.ESTADO = ' || A.ESTADO );
IF A.TIPO_CREDITO != VTIPO_CREDITO
OR A.MONTO_CREDITO != VMONTO
OR A.PLAZO_SEGUN_UNIDAD != VPLAZO
OR MONTO_A_PAGAR_17 != MTO_SEGURO_MIPYME
OR MONTO_A_PAGAR_24 != MTO_SEGURO_DESEMPLEO THEN --DBMS_OUTPUT.PUT_LINE ( 'Entro ' );
PR.PR_PKG_REPRESTAMOS.P_ACTUALIZAR_OPCION_FRONT (
    A.NO_CREDITO,
    A.TIPO_CREDITO,
    A.MONTO_CREDITO,
    A.PLAZO_SEGUN_UNIDAD,
    PMENSAJEERROR
);
...
BEGIN setError(
    pProgramUnit = > 'P_JOB_CREA_ACTUALIZA_CORE',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_CARGA_DE08(PFECHAREGULATORIA IN DATE) IS CURSOR CARGADE08 IS
SELECT
    IDDEUDOR,
    FECHACORTE,
    NOMBRES,
...
PROCEDURE P_CARGA_DE05(PFECHACASTIGO IN DATE) IS CURSOR CARGADE05 IS
SELECT
    CEDULA,
    CLIENTE,
    ENTIDAD
...
BEGIN BEGIN -- Authentication
vMasterKey := pa.Obt_Parametro_General_raw ('MKDB', 'PR');

vUsername := PA.DECIFRAR(
    pa.Obt_Parametro_General_raw('DBUSR', 'PR'),
    vMasterKey
);

vPass := PA.DECIFRAR(
    pa.Obt_Parametro_General_raw('DBPWD', 'PR'),
    vMasterKey
);

-- Determina el String Connection
SELECT
    vUsername || '/' || vPass || '@' || REPLACE(
...
EXCEPTION
WHEN NO_DATA_FOUND THEN vConnexion := 'ia/ia@ADMQA1';

END;

-- Ejecutar el Oracle Report
BEGIN PA.PKG_REPORTS.AgregaParametro ('p_ciudad', '"' || p_Ciudad || '"', FALSE);

PA.PKG_REPORTS.AgregaParametro ('P_CODIGO_CLIENTE', p_Codigo_Cliente, FALSE);

vReporteBlob := PA.PKG_REPORTS.GeneraReporte (
    pConexion = > vConnexion,
...
EXCEPTION
WHEN OTHERS THEN pError := 'Error generando el reporte ' || vReportName || ' del cliente ' || p_Codigo_Cliente || ' ' || SQLERRM;

RAISE_APPLICATION_ERROR(-20100, pError);

END;

IF LENGTH(vReporteBlob) > 0 THEN DBMS_OUTPUT.PUT_LINE('Reporte Descargado ' || LENGTH(vReporteBlob));

-- Crear el archivo PDF
BEGIN PA.PKG_REPORTS.EscribeArchivo(
    pblobdata = > vReporteBlob,
    pdirectory = > vDirectorio,
    pfilename = > vNombreArchivo
);

EXCEPTION
WHEN OTHERS THEN pError := 'Error creando el archivo ' || vNombreArchivo || ' ' || SQLERRM;
...
IF p_SubirFileFlow THEN -- Subir documento a FileFlow
BEGIN -- Determina el Crédito anterior
BEGIN
SELECT
    s.NO_CREDITO_CANCELADO INTO vNoCreditoAnterior
FROM
    PR.PR_CANCELACION_CREDITOS s
WHERE
    s.NO_CREDITO = p_No_Credito;

EXCEPTION
WHEN NO_DATA_FOUND THEN vNoCreditoAnterior := ' ';
...
BEGIN
SELECT
    F.ID_TEMPFUD INTO vIdTemFud
FROM
    PR.TEMPFUD F
WHERE
    F.NOCREDITO = p_No_Credito;

EXCEPTION
WHEN NO_DATA_FOUND THEN vIdTemFud := ' ';
...
BEGIN
SELECT
    IDENTIFICACION INTO vIdentificacion
FROM
    PR.PR_SOLICITUD_REPRESTAMO
WHERE
    NO_CREDITO = p_No_Credito;

EXCEPTION
WHEN NO_DATA_FOUND THEN vIdentificacion := ' ';
...
EXCEPTION
WHEN OTHERS THEN pError := 'Error cargando reporte a FileFlow ' || pError || ' ' || SQLERRM;
...
BEGIN
SELECT
    COUNT(*) INTO v_estadoActivo
FROM
    PR_REPRESTAMO_CAMPANA
where
    ESTADO = 1;

IF p_estado = '1'
AND v_estadoActivo > 0 THEN pError := 'Error - Ya existe una campaña activa ' || SQLERRM;

--Luego de pasar la validacion realizare la insercion de los datos
ELSE
INSERT INTO
    PR.PR_REPRESTAMO_CAMPANA(
        CODIGO_EMPRESA,
        NOMBRE,
        DESCRIPCION,
        ESTADO,
...
EXCEPTION
WHEN NO_DATA_FOUND THEN v_estadoActivo := 0;

COMMIT;

WHEN OTHERS THEN pError := 'Error - Ya existe una campaña activa ' || SQLERRM;
...
BEGIN --Selecciono la cantidad de los registros que tengan el estado en 1 o Activo
SELECT
    COUNT(*) INTO v_estadoActivo
FROM
    PR.PR_REPRESTAMO_CAMPANA
...
IF v_estadoActivo > 0
AND p_estado = 1 THEN pError := 'Error - Ya existe una campaña activa ' || SQLERRM;
...
SET
    ...
    FECHA_MODIFICACION = SYSDATE
WHERE
    CODIGO_CAMPANA = p_codigoCampana
...
UPDATE
    PR.PR_REPRESTAMO_CAMPANA_DET RCD
SET
    ESTADO = 2
WHERE
    CODIGO_CAMPANA = p_codigoCampana
    AND EXISTS (
        SELECT
            1
        FROM
            PR.PR_TIPO_CREDITO_REPRESTAMO TCR
        WHERE
            TCR.TIPO_CREDITO = RCD.TIPO_CREDITO_DESTINO
...
SET
    ESTADO = 'I'
WHERE
    TCR.CREDITO_CAMPANA_ESPECIAL = 'S'
    AND EXISTS (
        SELECT
            1
        FROM
            PR.PR_REPRESTAMO_CAMPANA_DET RCD
        WHERE
            RCD.CODIGO_CAMPANA = p_codigoCampana
...
EXCEPTION
WHEN OTHERS THEN pError := 'Error - ' || SQLERRM;

DECLARE vIdError PLS_INTEGER := 0;

BEGIN IA.LOGGER.ADDPARAMVALUEV('p_codigoEmpresa', p_codigoEmpresa);
...
setError(
    pProgramUnit = > 'P_Actualizar_Campana',
    pPieceCodeName = > NULL,
    pErrorDescription = > SQLERRM,
    pErrorTrace = > DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
    pEmailNotification = > NULL,
...
PROCEDURE P_Inactivar_Campana AS PRAGMA AUTONOMOUS_TRANSACTION;
...
EXCEPTION
WHEN OTHERS THEN vError := 'Error - ' || SQLERRM;
...
SELECT
    ...
    NVL(T.ID_TEMPFUD, ''),
    NVL(T.NOMARCHIVO, ''),
    NVL(T.MONTOSOLICITADO, '') INTO p_tipo_documento_identidad,
...
WHERE
    S.ID_REPRESTAMO = p_id_represtamo;

EXCEPTION
WHEN NO_DATA_FOUND THEN p_tipo_documento_identidad := '';
...
SELECT
    ...
    S.VENTAS_MENSUAL INTO p_no_credito,
    p_id_represtamo_out,
    p_nombres,
    p_codigo_cliente,
    p_fecha,
...
FROM
    PR.PR_SOLICITUD_REPRESTAMO S
    JOIN PR.PR_OPCIONES_REPRESTAMO O ON O.CODIGO_EMPRESA = S.CODIGO_EMPRESA
    AND O.ID_REPRESTAMO = S.ID_REPRESTAMO
    AND O.PLAZO = S.PLAZO
    JOIN PR.PR_REPRESTAMOS R ON R.CODIGO_EMPRESA = S.CODIGO_EMPRESA
    AND R.ID_REPRESTAMO = S.ID_REPRESTAMO
    JOIN PA.AGENCIA A ON A.COD_EMPRESA = S.CODIGO_EMPRESA
    AND A.COD_AGENCIA = S.CODIGO_AGENCIA
WHERE
    S.CODIGO_EMPRESA = PR_PKG_REPRESTAMOS.F_OBT_EMPRESA_REPRESTAMO
    AND S.ID_REPRESTAMO = p_id_represtamo;

EXCEPTION
WHEN NO_DATA_FOUND THEN p_no_credito := NULL;
...
PROCEDURE P_ACTUALIZA_COMENTARIO_CAMPANA(pComentario in VARCHAR2) IS vUltimaFecha DATE;

BEGIN
SELECT
    MAX(FECHA_ADICION) INTO vUltimaFecha
FROM
    PR.PR_CAMPANA_ESPECIALES;
...
WHERE
    FECHA_ADICION = vUltimaFecha;

COMMIT;

END;

PROCEDURE P_ACTUALIZAR_CAMPO_APPADEMI(pIdReprestamo IN VARCHAR2, pOrigen IN VARCHAR2) IS BEGIN
UPDATE
    PR.PR_SOLICITUD_REPRESTAMO
SET
    ORIGEN = pOrigen
...
END P_ACTUALIZAR_CAMPO_APPADEMI;

PROCEDURE PR_AUDIT_TRACK (
    p_empresa IN NUMBER DEFAULT 1,
    p_id_campana IN NUMBER DEFAULT TO_NUMBER(TO_CHAR(SYSDATE, 'YYYYMM')),
    p_SP_name IN VARCHAR2,
    p_tag IN VARCHAR2 DEFAULT NULL,
    p_bind_date IN DATE DEFAULT NULL,
    p_bind_num IN NUMBER DEFAULT NULL,
    p_object_name IN VARCHAR2 DEFAULT NULL,
    p_object_type IN VARCHAR2 DEFAULT 'PACKAGE BODY',
    p_log_descartes IN VARCHAR2 DEFAULT 'N' -- 'S' = inserta Dxx (descartados)
) IS --------------------------------------------------------------------
...
-- SELECT completo entre START/END
l_sql_work CLOB;

-- SELECT limpio para parsing
l_base_from CLOB;
...
-- pon FALSE cuando termines de depurar
-- Extrae filtros /*Fnn: Nombre*/ y devuelve nombres y condiciones AND...
TYPE t_conds IS TABLE OF CLOB INDEX BY PLS_INTEGER;

TYPE t_names IS TABLE OF VARCHAR2(200) INDEX BY PLS_INTEGER;

PROCEDURE debug_put(p_label VARCHAR2, p_txt CLOB) IS l_pos PLS_INTEGER := 1;

BEGIN IF NOT c_debug
OR p_txt IS NULL THEN RETURN;

END IF;

DBMS_OUTPUT.PUT_LINE(
    '>>> ' || p_label || ' (' || NVL(DBMS_LOB.GETLENGTH(p_txt), 0) || ' bytes) >>>'
);

WHILE l_pos <= DBMS_LOB.GETLENGTH(p_txt) LOOP DBMS_OUTPUT.PUT_LINE(DBMS_LOB.SUBSTR(p_txt, 32000, l_pos));
...
--------------------------------------------------------------------
-- Inserta una fila de tracking (transacción autónoma)
--------------------------------------------------------------------
PROCEDURE ins_row(
    p_emp NUMBER,
    p_nom VARCHAR2,
...
BEGIN
INSERT INTO
    PR.PR_TRACKING_FILTROS_REPRESTAMOS (
        CODIGO_TRACKING,
        CODIGO_EMPRESA,
        NOMBRE_FILTRO,
        CANTIDAD_CLIENTES,
        ADICIONADO_POR,
        FECHA_ADICION,
        IDENTIFICADOR_CAMPANA,
        NOMBRE_PROCEDIMIENTO
    )
VALUES
    (
        PR.SEQ_PR_TRACKING_FILTROS.NEXTVAL,
        p_emp,
        p_nom,
        p_cnt,
...
--------------------------------------------------------------------
-- Log de error (transacción autónoma)
--------------------------------------------------------------------
PROCEDURE log_err(p_emp NUMBER, p_msg VARCHAR2, p_camp NUMBER) IS PRAGMA AUTONOMOUS_TRANSACTION;
...
--------------------------------------------------------------------
-- Inferir el nombre del objeto llamador (este package)
--------------------------------------------------------------------
FUNCTION infer_object_name RETURN VARCHAR2 IS BEGIN RETURN REPLACE(UPPER($ $ PLSQL_UNIT), '"', '');

END infer_object_name;

--------------------------------------------------------------------
-- Extraer del fuente el bloque SELECT entre los marcadores
--------------------------------------------------------------------
FUNCTION get_select_from_source(
    p_obj_name IN VARCHAR2,
    p_obj_type IN VARCHAR2,
    p_tag IN VARCHAR2
) RETURN CLOB IS l_text CLOB := NULL;

s_tag_pos PLS_INTEGER := 0;
...
BEGIN FOR r IN (
    ...
    FROM
        user_source
    WHERE
        name = UPPER(p_obj_name)
        AND type = UPPER(p_obj_type)
    ORDER BY
        line
) LOOP l_text := NVL(l_text, '') || r.text;

END LOOP;

IF p_tag IS NOT NULL THEN s_tag_pos := INSTR(l_text, '-- AUDIT TAG: ' || p_tag);

IF s_tag_pos = 0 THEN RAISE_APPLICATION_ERROR(
    -20010,
    'No se encontró "-- AUDIT TAG: ' || p_tag || '".'
);

END IF;

s_pos := INSTR(l_text, '-- AUDIT SELECT START', s_tag_pos);

e_pos := INSTR(l_text, '-- AUDIT SELECT END', s_pos);

ELSE s_pos := INSTR(l_text, '-- AUDIT SELECT START');
...
IF s_pos = 0
OR e_pos = 0
OR e_pos <= s_pos THEN RAISE_APPLICATION_ERROR(-20011, 'Faltan "-- AUDIT SELECT START/END".');

END IF;

RETURN REPLACE(
    REPLACE(
        SUBSTR(l_text, s_pos, e_pos - s_pos),
        '-- AUDIT SELECT START',
        ''
    ),
    '-- AUDIT SELECT END',
...
PROCEDURE parse_filters(
    p_sql IN CLOB,
    p_first_after_where IN PLS_INTEGER,
    o_names OUT t_names,
    o_conds OUT t_conds,
    o_n OUT PLS_INTEGER
) IS l_start PLS_INTEGER;
...
WHILE l_start > 0 LOOP l_endc := INSTR(p_sql, '*/', l_start);

IF l_endc = 0 THEN RAISE_APPLICATION_ERROR(-20013, 'Comentario de filtro sin cierre "*/".');
...
l_next := REGEXP_INSTR(p_sql, '/\*F[0-9]{2}\s*:', l_endc + 2, 1, 0, 'i');

IF l_next = 0 THEN l_cond := SUBSTR(p_sql, l_endc + 2);

ELSE l_cond := SUBSTR(p_sql, l_endc + 2, l_next - (l_endc + 2));

END IF;

l_cond := TRIM(l_cond);

-- debe empezar por AND ...
v_n := v_n + 1;

o_conds(v_n) := l_cond;

o_names(v_n) := NVL(l_name, 'Filtro ' || TO_CHAR(v_n, 'FM00'));
...
--------------------------------------------------------------------
-- Construye " FROM ... WHERE 1=1"
--------------------------------------------------------------------
PROCEDURE build_base_fragment(p_sql IN CLOB, o_base OUT CLOB) IS p_where PLS_INTEGER := INSTR(UPPER(p_sql), 'WHERE 1=1');

p_from PLS_INTEGER := 0;

BEGIN IF p_where = 0 THEN RAISE_APPLICATION_ERROR(
    -20022,
    'No se encontró "WHERE 1=1" en el SELECT auditado.'
);

END IF;

p_from := REGEXP_INSTR(
    SUBSTR(p_sql, 1, p_where -1),
    '\bFROM\b',
    1,
    1,
    0,
...
IF p_from = 0 THEN p_from := INSTR(UPPER(SUBSTR(p_sql, 1, p_where -1)), ' FROM ');

END IF;

IF p_from = 0 THEN debug_put('SELECT LIMPIO (no se halló FROM)', p_sql);

RAISE_APPLICATION_ERROR(
    -20021,
    'No se encontró FROM en el SELECT auditado.'
);
...
o_base := SUBSTR(
    p_sql,
    p_from,
    p_where + LENGTH('WHERE 1=1') - p_from
);

END build_base_fragment;

--------------------------------------------------------------------
-- Ejecuta COUNT(*) con binds (muestra el SQL)
--------------------------------------------------------------------
...
BEGIN sql_txt := 'SELECT COUNT(*) ' || p_base || NVL(p_acc, '');

-- por si alguna variante de P_FECHA_CORTE quedó sin normalizar:
sql_txt := REGEXP_REPLACE(
    sql_txt,
    '\bP_FECHA_CORTE\b',
    ':v_fecha_corte',
    1,
...
has_date := REGEXP_LIKE(sql_txt, ':[[:space:]]*v_fecha_corte', 'i');

has_num := REGEXP_LIKE(sql_txt, ':[[:space:]]*p1', 'i');

IF c_debug THEN DBMS_OUTPUT.PUT_LINE('>>> ' || p_label || ' COUNT SQL >>>');

DBMS_OUTPUT.PUT_LINE(sql_txt);

IF has_date THEN DBMS_OUTPUT.PUT_LINE(
    '    :v_fecha_corte = ' || TO_CHAR(p_date, 'YYYY-MM-DD')
);

END IF;

IF has_num THEN DBMS_OUTPUT.PUT_LINE('    :p1 = ' || NVL(TO_CHAR(p_num), 'NULL'));

END IF;

DBMS_OUTPUT.PUT_LINE('<<< ' || p_label || ' COUNT SQL <<<');

END IF;

cur := DBMS_SQL.OPEN_CURSOR;

BEGIN -- ... dentro de exec_count, justo antes de DBMS_SQL.PARSE:
--sql_txt := REGEXP_REPLACE(sql_txt, '\bP_FECHA_CORTE\b', ':v_fecha_corte', 1, 0, 'i');
DBMS_SQL.PARSE(cur, sql_txt, DBMS_SQL.NATIVE);

-- bind por nombre UNA sola vez, aunque aparezca varias veces en el texto
IF has_date THEN DBMS_SQL.BIND_VARIABLE(cur, 'v_fecha_corte', p_date);

END IF;

IF has_num THEN DBMS_SQL.BIND_VARIABLE(cur, 'p1', p_num);

END IF;

DBMS_SQL.DEFINE_COLUMN(cur, 1, v_cnt);

ret := DBMS_SQL.EXECUTE(cur);

IF DBMS_SQL.FETCH_ROWS(cur) > 0 THEN DBMS_SQL.COLUMN_VALUE(cur, 1, v_cnt);
...
EXCEPTION
WHEN OTHERS THEN IF DBMS_SQL.IS_OPEN(cur) THEN DBMS_SQL.CLOSE_CURSOR(cur);
...
BEGIN -- 1) Objeto llamador
l_obj_name := NVL(p_object_name, infer_object_name);

IF l_obj_name IS NULL THEN RAISE_APPLICATION_ERROR(
    -20020,
    'No pude inferir el objeto llamador. Pasa p_object_name => ''PR_PKG_REPRESTAMOS''.'
);

END IF;

-- 2) SELECT del fuente
l_sql_src := get_select_from_source(l_obj_name, p_object_type, p_tag);

-- 3) Limpieza + normalización de binds (ROBUSTA)
l_sql_work := REGEXP_REPLACE(l_sql_src, '--.*$', '', 1, 0, 'm');

-- Mapear de forma segura cualquier variante de P_FECHA_CORTE
-- 1) Casos tipo: b.fecha_corte =  P_FECHA_CORTE
--l_sql_work := REGEXP_REPLACE(l_sql_work,'b\.fecha_corte\s*=\s*P_FECHA_CORTE','b.fecha_corte = :v_fecha_corte',1, 0, 'i');
l_sql_work := REGEXP_REPLACE(
    l_sql_work,
    '(\band\s+b\.fecha_corte\s*=\s*)P_FECHA_CORTE\b',
    '\1:v_fecha_corte',
    1,
    0,
    'i'
...
-- Casos genéricos "= P_FECHA_CORTE" o "=:P_FECHA_CORTE"
l_sql_work := REGEXP_REPLACE(
    l_sql_work,
    '=\s*:?\s*P_FECHA_CORTE\b',
    ' = :v_fecha_corte',
...
--l_sql_work := REGEXP_REPLACE(l_sql_work,'=\s*:?\s*P_FECHA_CORTE\b',' = :v_fecha_corte',1, 0, 'i');
l_sql_work := REGEXP_REPLACE(
    l_sql_work,
    '(^|[^A-Z0-9_])P_FECHA_CORTE([^A-Z0-9_]|$)',
    '\1:v_fecha_corte\2',
    1,
    0,
    'i'
...
-- V_ATRASO_30 -> :p1
l_sql_work := REGEXP_REPLACE(l_sql_work, '\bV_ATRASO_?30\b', ':p1', 1, 0, 'i');

-- Sanity check final por si quedara alguno sin mapear
IF REGEXP_LIKE(l_sql_work, '\bP_FECHA_CORTE\b', 'i') THEN debug_put(
    'WARN',
    'Quedó P_FECHA_CORTE sin mapear. Reemplazo forzado...'
);
...
debug_put('SELECT COMPLETO LIMPIO', l_sql_work);

-- 4) Fragmento base: " FROM ... WHERE 1=1"
build_base_fragment(l_sql_work, l_base_from);

debug_put('BASE FROM', l_base_from);

-- 5) F00 = Base ya con fecha & fuente (alias b es PA.PA_DETALLADO_DE08)
DECLARE base_seed CLOB := ' AND b.fecha_corte = :v_fecha_corte AND b.fuente = ''PR''';
...
p_where PLS_INTEGER := INSTR(UPPER(l_sql_work), 'WHERE 1=1');

acc CLOB;

prev_cnt NUMBER := NULL;

v_delta NUMBER := 0;

-- <--- nueva
BEGIN -- F00 (universo base)
exec_count(
    'F00',
    l_base_from,
    base_seed,
...
-- 6) Parseo de filtros y acumulación desde la semilla
parse_filters(
    l_sql_work,
    p_where + LENGTH('WHERE 1=1'),
    v_names,
    v_conds,
    v_n
);
...
-- empezamos a acumular desde la base (fecha & fuente)
FOR i IN 1..v_n LOOP acc := acc || ' ' || v_conds(i);
...
-- DESCARTADOS en este filtro:
v_delta := CASE
    WHEN prev_cnt IS NULL THEN 0
    ELSE GREATEST(prev_cnt - l_cnt, 0)
END;

-- Guardar el delta en el renglón Fxx
ins_row(
    p_empresa,
    'F' || TO_CHAR(i, 'FM00') || ' - ' || v_names(i),
    l_cnt,
    p_id_campana
);

-- Avanzamos el acumulado
prev_cnt := l_cnt;
...
EXCEPTION
WHEN OTHERS THEN log_err(p_empresa, SQLERRM, p_id_campana);

END PR_AUDIT_TRACK;

-- =========================================================
-- TRACK: helper para guardar conteos en PR_TRACKING_FILTROS_REPRESTAMOS
-- =========================================================
PROCEDURE PR_AUDIT_PUSH_STEP(
    p_empresa IN NUMBER,
    p_id_campana IN NUMBER,
    p_sp_name IN VARCHAR2,
    p_nombre_filtro IN VARCHAR2,
