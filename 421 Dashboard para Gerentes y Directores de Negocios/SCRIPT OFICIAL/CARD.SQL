WITH latest_bitacora AS (
    ...
    LEFT JOIN pa.agencia ag
        ON ag.cod_empresa = r.codigo_empresa AND ag.cod_agencia = c.codigo_agencia
    JOIN pa.empleados p
        ON ag.gerente = p.id_empleado AND ag.cod_empresa = TO_NUMBER(p.cod_empresa)
    ...
    WHERE
        ...
        AND (
            ...
            OR r.fecha_proceso >= TRUNC(TO_DATE(:P133_FROM_DATE, 'DD/MM/YYYY HH24:MI:SS'))
        )
        AND (
            :P133_TO_DATE IS NULL
            OR r.fecha_proceso < TRUNC(TO_DATE(:P133_TO_DATE, 'DD/MM/YYYY HH24:MI:SS')) + 1
        )
...
FROM
    (
        SELECT
            ...
            'javascript:apex.item("P133_TIPO_DESEMBOLSO").setValue(null);apex.item("P133_ESTADO").setValue(''' || (
                SELECT
                ...
                FROM
                    TABLE(pr.pr_pkg_represtamos.f_obt_valor_parametros('ESTADOS_PREAPROBADOS'))
            ) || ''');apex.event.trigger("#TABLE_133","apexrefresh");' AS card_link,
            1 AS order_col
        FROM
            DUAL
        UNION ALL
