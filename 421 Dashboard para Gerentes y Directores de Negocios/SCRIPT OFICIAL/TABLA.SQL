WITH latest_bitacora AS (
    ...
    SELECT
        r.fecha_proceso AS "FECHA_PROCESO",
        r.id_represtamo AS "ID Représtamo",
        ...
        CASE WHEN s.email IS NOT NULL THEN 'Sí' ELSE 'No' END AS "Correo Electrónico",
        CASE
        ...
        pa.obt_nombre_empleado(NVL(c.codigo_empresa, h.codigo_empresa), NVL(c.codigo_ejecutivo, h.codigo_ejecutivo)) AS "Oficial",
        pa.obt_nombre_empleado(r.codigo_empresa, lb.adicionado_por) AS "Atendido Por",
        r.estado AS codigo_estado_para_filtro
    FROM
        pr.pr_represtamos r
    LEFT JOIN pr.pr_solicitud_represtamo s ON s.id_represtamo = r.id_represtamo AND s.codigo_empresa = r.codigo_empresa
    LEFT JOIN pr.pr_creditos c ON c.no_credito = r.no_credito AND c.codigo_empresa = r.codigo_empresa
    LEFT JOIN pr.pr_creditos_hi h ON h.no_credito = r.no_credito AND h.codigo_empresa = r.codigo_empresa
    LEFT JOIN pa.agencia ag ON ag.cod_empresa = r.codigo_empresa AND ag.cod_agencia = NVL(c.codigo_agencia, h.codigo_agencia)
    LEFT JOIN pr.pr_tipo_credito tc ON tc.codigo_empresa = r.codigo_empresa AND tc.tipo_credito = NVL(s.tipo_credito, NVL(c.tipo_credito, h.tipo_credito))
    LEFT JOIN pr.pr_canales_represtamo a_canal ON a_canal.id_represtamo = r.id_represtamo AND a_canal.canal = 1
    JOIN pa.empleados p ON ag.gerente = p.id_empleado AND ag.cod_empresa = TO_NUMBER(p.cod_empresa)
    LEFT JOIN latest_bitacora lb ON lb.id_represtamo = r.id_represtamo AND lb.rn = 1
    LEFT JOIN has_cry_status hcs ON hcs.id_represtamo = r.id_represtamo
    WHERE
        --SUBSTR(P.EMAIL1, 1, INSTR(P.EMAIL1, '@') - 1) = V('APP_USER')
        SUBSTR(p.email1, 1, INSTR(p.email1, '@') - 1) = 'vcasilla'
        AND p.esta_activo = 'S'
        AND NOT EXISTS (
            SELECT 1
            FROM pr_carga_direccionada cd
            WHERE cd.no_credito = r.no_credito AND cd.estado = 'F' AND TRUNC(cd.fecha_adicion) = TRUNC(r.fecha_adicion)
        )
        AND (:P133_FROM_DATE IS NULL OR r.fecha_proceso >= TRUNC(TO_DATE(:P133_FROM_DATE, 'DD/MM/YYYY HH24:MI:SS')))
        AND (:P133_TO_DATE IS NULL OR r.fecha_proceso < TRUNC(TO_DATE(:P133_TO_DATE, 'DD/MM/YYYY HH24:MI:SS')) + 1)
)
SELECT
    "FECHA_PROCESO" AS "Fecha Proceso",
    "ID Représtamo",
    "Código Cliente",
    "Cliente",
    "Identificación",
