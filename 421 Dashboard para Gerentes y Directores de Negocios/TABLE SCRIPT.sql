WITH latest_bitacora AS (
    ...
    SELECT
        ...
        CASE WHEN s.email IS NULL THEN 'No' ELSE 'Sí' END AS "Correo Electrónico",
        CASE
    ...
    FROM
        pa.empleados p
    JOIN pa.agencia a_emp ON TO_NUMBER(p.cod_empresa) = a_emp.cod_empresa AND TO_NUMBER(p.cod_agencia_labora) = a_emp.cod_agencia
    JOIN pr.pr_solicitud_represtamo s ON TO_NUMBER(p.cod_empresa) = s.codigo_empresa AND TO_NUMBER(p.cod_agencia_labora) = s.codigo_agencia
    JOIN pr.pr_represtamos r ON r.id_represtamo = s.id_represtamo AND r.codigo_empresa = s.codigo_empresa
    JOIN latest_bitacora lb ON lb.id_represtamo = s.id_represtamo AND lb.rn = 1
    LEFT JOIN has_cry_status hcs ON hcs.id_represtamo = s.id_represtamo
...
WHERE
    (
        :P133_ESTADO IS NULL OR codigo_estado IN (
            SELECT
                column_value
            FROM
    ...
    AND (
        :P133_TIPO_DESEMBOLSO IS NULL OR "Tipo de Desembolso" = :P133_TIPO_DESEMBOLSO
    )
ORDER BY "FECHA_ESTADO" DESC, "ID Représtamo" DESC;