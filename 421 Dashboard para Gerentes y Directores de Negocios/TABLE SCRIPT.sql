...
SELECT
    ...
    (TRUNC(R.FECHA_PROCESO) + CAST(PR.PR_PKG_REPRESTAMOS.F_OBT_PARAMETRO_REPRESTAMO('DIA_CADUCA_LINK') AS NUMBER)) AS "Fecha_Vencimiento_Link",
...
JOIN PA.AGENCIA A_EMP
    ON TO_NUMBER(P.COD_EMPRESA) = A_EMP.COD_EMPRESA
    AND TO_NUMBER(P.COD_AGENCIA_LABORA) = A_EMP.COD_AGENCIA
JOIN PR.PR_SOLICITUD_REPRESTAMO S
    ON TO_NUMBER(P.COD_EMPRESA) = S.CODIGO_EMPRESA
...
LEFT JOIN PR.PR_CREDITOS C ON R.NO_CREDITO = C.NO_CREDITO AND R.CODIGO_EMPRESA = C.CODIGO_EMPRESA
LEFT JOIN PR.PR_CREDITOS_HI H ON R.NO_CREDITO = H.NO_CREDITO AND R.CODIGO_EMPRESA = H.CODIGO_EMPRESA
...
LEFT JOIN PR.PR_CANALES_REPRESTAMO A_CANAL ON R.ID_REPRESTAMO = A_CANAL.ID_REPRESTAMO AND A_CANAL.CANAL = 1
WHERE
    ...
    AND P.ESTA_ACTIVO = 'S'
    AND NOT EXISTS (
        SELECT 1 FROM PR_CARGA_DIRECCIONADA CD WHERE CD.NO_CREDITO = R.NO_CREDITO AND CD.ESTADO = 'F' AND TRUNC(CD.FECHA_ADICION) = TRUNC(R.FECHA_ADICION)
    )
ORDER BY
