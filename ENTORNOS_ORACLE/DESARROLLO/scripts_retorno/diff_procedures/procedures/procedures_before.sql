-- PROCEDURE Precalifica_Represtamo (before)
   PROCEDURE Precalifica_Represtamo  IS 
      ...
      -- Se valida que sea a una sola firma        
        ...
        AND NOT EXISTS (
                SELECT 1
                  FROM PR_GARANTIAS_X_CREDITO gx
                       JOIN PR_GARANTIAS g
                         ON g.codigo_empresa = gx.codigo_empresa
                        AND g.numero_garantia = gx.numero_garantia
                 WHERE gx.codigo_empresa = a.codigo_empresa
                   AND gx.no_credito = a.no_credito
        ...
        AND NOT EXISTS (
                SELECT 1
                 ...
                 WHERE lp.cod_persona = CAST(a.codigo_cliente AS VARCHAR2(15))
                   AND lp.fec_vencimiento >= TRUNC(SYSDATE) + 1)
        -- Se valida que los clientes no esten en lista NEGRA
        AND NOT EXISTS (
                SELECT 1
                  FROM LISTA_NEGRA ln
                 WHERE ln.cod_persona = CAST(a.codigo_cliente AS VARCHAR2(15))
...
-- PROCEDURE Precalifica_Repre_Cancelado (before)
...
-- PROCEDURE Precalifica_Repre_Cancelado_hi (before)
...
-- PROCEDURE Precalifica_Represtamo_fiadores (before)
...
-- PROCEDURE Precalifica_Represtamo_fiadores_hi (before)
