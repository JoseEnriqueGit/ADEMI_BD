    ...
    ******************************************************************************/
    ...
    if pErrornum is null then
        BEGIN
            SELECT b.nombre,
                   b.es_fisica,
                   nvl((select nvl(gerente,a.cod_oficial) from pa.agencia where cod_agencia=a.cod_agencia),a.cod_oficial) cod_oficial,
                   c.tipo_de_persona,
                   cod_sec_contable
              INTO vdesc_nombre,
                   vesfisica,
        ...
        IF (vcodoficial IS NOT NULL) THEN
            ...
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    ...
                    pErrornum:=204;
                    pMensaje_Error := 'Error buscando Ejecutivo del Cliente en la DB, favor verificar codigo de cliente';
        ...
        IF vnum_cuenta IS NOT NULL AND vEsCliente > 0  THEN
        
              DECLARE
                   vURL                 VARCHAR2(4000);
                   vIdAplication        PLS_INTEGER := 37; -- CUENTASDEAHORROS
                   vIdTipoDocumento     PLS_INTEGER := '618'; -- Formulario de Conozca
                   vCodigoReferencia    VARCHAR2(100) := vnum_cuenta; --pCodigoCliente||':'||vSolicitudNumero;
                ...
                BEGIN
                   ...
                   PA.PKG_TIPO_DOCUMENTO_PKM.InsertUrlReporte( pCodigoReferencia   => vCodigoReferencia,
                                                    ...
                                                    pNombreArchivo      => vDocumento||'_'||vnum_cuenta||'_'||pCodigo_cliente||'.pdf',   
                                                    pRespuesta          => pMensaje_Error
                                                   );
                                
                                                                               
                    vIdTipoDocumento := '762';  -- CONSULTA BURO DE CREDITO PRIVADO 
                    vDocumento       := 'BURO';  
                    vCodigoReferencia := vTipoId||':'||vNumId||':'||vnum_cuenta||': :'||vDocumento||': '; 
                    ...
                    vIdTipoDocumento := '810';  -- CONSULTA BUSCADOR DE GOOGLE
                    vDocumento       := 'SIB';  
                    vCodigoReferencia := vTipoId||':'||vNumId||':'||vnum_cuenta||': :'||vDocumento;     
                    --vNombreArchivo    := vDocumento||'_'||vSolicitudNumero||'_'||inSolicitudTarjeta.CodigoCliente;                                                                                                 
                    PA.PKG_TIPO_DOCUMENTO_PKM.InsertUrlReporte( pCodigoReferencia   => vCodigoReferencia,
                ...
                EXCEPTION WHEN OTHERS THEN 
                   pMensaje_Error := pMensaje_Error||' '||dbms_utility.format_error_backtrace;      
                   RAISE_APPLICATION_ERROR(-20104, pMensaje_Error);
                END;
