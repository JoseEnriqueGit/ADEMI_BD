...
PROCEDURE Precalifica_Repre_Cancelado_hi IS 
 ...
 PROCEDURE Precalifica_Carga_Dirigida IS
  ...
  PROCEDURE Actualiza_Preca_Dirigida IS 
    ...
    CURSOR ReprestamosAProcesarCLS IS
 ...
 --Validacion del DE08 
  /*  IF PR.PR_PKG_REPRESTAMOS.f_obt_parametro_represtamo('VALIDAR_CLASIFICACION_SIB_CARGADIRIGIDA') = 'S' THEN
     ...
     FOR a in CUR_DE08_SIB LOOP
         ...
         FOR A IN ReprestamosAProcesarCLS LOOP
            vEstado := 'CLS';
            vComentario := 'Cliente no encontrado en DE08_SIB, clasificación aprobada por defecto';
            PR.PR_PKG_REPRESTAMOS.p_generar_bitacora(a.id_represtamo,NULL,vEstado,NULL,vComentario, USER);
        END LOOP;
