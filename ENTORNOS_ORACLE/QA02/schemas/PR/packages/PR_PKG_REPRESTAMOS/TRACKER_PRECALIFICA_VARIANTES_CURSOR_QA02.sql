/*
  QA02 - PR.PR_PKG_REPRESTAMOS
  Tracker read-only y liviano para variantes de CREDITOS_PROCESAR.

  Objetivo:
  - Contar candidatos descartados por los filtros del cursor solamente.
  - Ejecutar un bloque a la vez en Toad.
  ...
  Nota:
  - ROWNUM <= LOTE_DE_CARAGA_REPRESTAMO se reporta al final como estimado,
    porque los cursores no tienen ORDER BY.
*/

--------------------------------------------------------------------------------
-- 1) Precalifica_Repre_Cancelado - solo filtros del cursor.
--------------------------------------------------------------------------------
WITH params AS (
...
SELECT 'Precalifica_Repre_Cancelado' proceso,
       p.orden,
       p.filtro,
...
UNION ALL
SELECT 'Precalifica_Repre_Cancelado',
       99,
...
--------------------------------------------------------------------------------
-- 2) Precalifica_Repre_Cancelado_hi - solo filtros del cursor.
--------------------------------------------------------------------------------
WITH params AS (
    SELECT TO_NUMBER(PR.PR_PKG_REPRESTAMOS.F_OBT_PARAMETRO_REPRESTAMO('LOTE_DE_CARAGA_REPRESTAMO')) lote,
           TO_NUMBER(PR.PR_PKG_REPRESTAMOS.F_OBT_PARAMETRO_REPRESTAMO('DIAS_CANCELACION')) dias_cancelacion,
...
SELECT 'Precalifica_Repre_Cancelado_hi' proceso,
       p.orden,
       p.filtro,
       b.cantidad candidatos_antes,
       a.cantidad candidatos_pasan,
  ...
  JOIN conteos b ON b.orden = CASE WHEN p.orden = 0 THEN 0 ELSE p.orden - 1 END
UNION ALL
SELECT 'Precalifica_Repre_Cancelado_hi',
       99,
       'ROWNUM <= LOTE_DE_CARAGA_REPRESTAMO',
       c.cantidad,
       LEAST(c.cantidad, p.lote),
...
--------------------------------------------------------------------------------
-- 3) Precalifica_Represtamo_fiadores - solo filtros del cursor.
--------------------------------------------------------------------------------
WITH params AS (
...
conteos AS (
    ...
    SELECT 17, COUNT(*) FROM s17 UNION ALL
    SELECT 18, COUNT(*) FROM s18
),
pasos AS (
    SELECT 0 orden, 'BASE: PR_CREDITOS' filtro FROM dual UNION ALL
    SELECT 1, 'TIPO_CREDITO existe en PR_TIPO_CREDITO_REPRESTAMO' FROM dual UNION ALL
...
SELECT 'Precalifica_Represtamo_fiadores' proceso,
       p.orden,
       p.filtro,
       b.cantidad candidatos_antes,
       a.cantidad candidatos_pasan,
  ...
  JOIN conteos b ON b.orden = CASE WHEN p.orden = 0 THEN 0 ELSE p.orden - 1 END
UNION ALL
SELECT 'Precalifica_Represtamo_fiadores',
       99,
       'ROWNUM <= LOTE_DE_CARAGA_REPRESTAMO',
       c.cantidad,
       LEAST(c.cantidad, p.lote),
...
--------------------------------------------------------------------------------
-- 4) Precalifica_Represtamo_fiadores_hi - solo filtros del cursor.
--------------------------------------------------------------------------------
WITH params AS (
    SELECT TO_NUMBER(PR.PR_PKG_REPRESTAMOS.F_OBT_PARAMETRO_REPRESTAMO('LOTE_DE_CARAGA_REPRESTAMO')) lote,
           TO_NUMBER(PR.PR_PKG_REPRESTAMOS.F_OBT_PARAMETRO_REPRESTAMO('DIAS_CANCELACION')) dias_cancelacion,
