     ...
     BEGIN
      ...
      WHEN NO_DATA_FOUND
         THEN
            ...
            pn_errornumber :=409;
            pv_errorMessage:='Error de validacion - La cuenta de efectivo no existe o no pertence al solicitante';
            RETURN; 
      END;*/
...
PROCEDURE getCancellationInfocd_api(pbody                    IN BLOB,
                                   ...
                                   pv_currencyCode           OUT VARCHAR2,
                                   pn_openAmount             OUT NUMBER,
                                   pn_interestAcumulated     OUT NUMBER,
                                   pv_productType            OUT VARCHAR2,
      ...
      BEGIN
        ...
        v_datos_apertura.num_cuenta          := JSON_VALUE(pbody, '$.cancellationInfo.accountNumber');
        ...
        IF v_datos_apertura.num_id IS NULL THEN
                    pn_errornumber  :=502;
                    pv_errorMessage :='Debe ingresar un numero de identificacion del cliente';
                    RETURN;
        END IF;

        IF v_datos_apertura.cod_tipo_id IS NULL THEN
                    pn_errornumber  :=503;
                    pv_errorMessage :='Debe ingresar un tipo de identificacion para buscar el cliente';
                    RETURN;
        END IF;

        IF v_datos_apertura.cod_persona IS NULL THEN
                    pn_errornumber:=505;
                    pv_errorMessage:='El codigo de cliente es necesario para la solicitud de cancelacion de certificado digital';
                    RETURN;
        END IF;   

        /*IF v_datos_apertura.num_cuenta IS NULL THEN
                    pn_errornumber:=506;
                    pv_errorMessage:='No ha ingresado un numero de cuenta para acreditar los fondo';
                    RETURN;
        END IF;*/
        ...
        END IF;
         IF v_datos_apertura.num_certificado IS NULL THEN
                    pn_errornumber:=508;
                    pv_errorMessage:='Es necesirio indicar el certificado digital a cancelar';
                    RETURN;
        END IF;

        BEGIN
            SELECT cer.num_certificado,cer.monto,ROUND(cer.mon_acum_int_cap,2) interesacumulado,ROUND(cer.cre_mes,2) interes_corriente,
            cer.porcentaje_renta, ROUND(ROUND(cer.cre_mes,2)*(cer.porcentaje_renta/100),2) monto_renta,'DIGITAL_MONEYMARKET',
            DECODE(cod_moneda,'1','DOP','2','USD','3','EUR') idmoneda
            INTO pv_certificatenumber, pn_openAmount, pn_accruedInterest,pn_currentInterest,pn_taxesRate,pn_taxesAmount,pv_productType,pv_currencyCode
            FROM cd.cd_certificado  cer
            WHERE cod_empresa=v_datos_apertura.cod_empresa
            AND num_certificado=v_datos_apertura.num_certificado
            AND cliente=v_datos_apertura.cod_persona AND estado='A';
        EXCEPTION
        WHEN NO_DATA_FOUND THEN
            pn_errornumber:=509;
            pv_errorMessage:='El certificado no existe o no pertenece a este cliente identificacion '||v_datos_apertura.num_id;
            RETURN;
        END;
          pn_interestAcumulated:=nvl(pn_accruedInterest,0)+(nvl(pn_currentInterest,0)-nvl(pn_taxesAmount,0));
        BEGIN
                getInfoCancellationcd(pv_companyId            => v_datos_apertura.cod_empresa,
                                      ...
                                      pn_accountNumber        => v_datos_apertura.num_cuenta,
                                      pv_cancellationType     => pv_cancellationType,
                                      pn_cancellationAmount   => pn_cancellationAmount,
                                      pn_penalAmount          => pn_penalAmount,
                                      pn_creditAccount        => pn_accountNumber,
                                      pn_errornumber          => pn_errornumber,
                                      pv_errorMessage         => pv_errorMessage);
        END;
           IF NVL(pn_penalAmount,0)>0 THEN
           BEGIN
           SELECT tasa_penalidad
           INTO pn_penalRate
           FROM  CD.CD_PENALIDAD_CANCELACION PEN, CD.CD_CERTIFICADO CER
             WHERE cer.cod_empresa=v_datos_apertura.cod_empresa
                AND cer.num_certificado=v_datos_apertura.num_certificado
                AND PEN.COD_EMPRESA=CER.COD_EMPRESA
                AND PEN.COD_PRODUCTO = CER.COD_PRODUCTO
                AND CER.PLA_DIAS BETWEEN PEN.LIMITE_INFERIOR AND PEN.LIMITE_SUPERIOR;
           EXCEPTION
           WHEN NO_DATA_FOUND THEN
           pn_penalRate:=0;
           END;
           ...
           pn_penalAmount:=0;
           END IF;
           --pv_errorMessage:='Llego el JSON';
           dbms_output.put_line('cuenta'||pn_accountNumber);
       END;      
PROCEDURE Cancellationcd_api(pbody                    IN BLOB,
       ...
       pd_openingDate       DATE;
       dCancellationDate    DATE;
       List_producto        varchar2(500):=PA.PARAM.PARAMETRO_X_EMPRESA('1', 'CD_CERT_DIGITAL', 'CD');
      ...
      BEGIN
        ...
        pn_openAmount:=0;
        pn_interestAcumulated:=0;
        pn_accountNumber:=null;
        pn_penalRate:=0;
        pn_penalAmount:=0;
        pn_autorizationNumber:=0;
        pn_CreditAmount:=0;
        pv_cancellationDate:=TO_CHAR(TRUNC(SYSDATE),'dd/mm/yyyy');
        v_datos_apertura.cod_empresa         := JSON_VALUE(pbody, '$.cancellationInfo.companyId');
        v_datos_apertura.cod_persona         := JSON_VALUE(pbody, '$.cancellationInfo.customerId');
        v_datos_apertura.num_id              := JSON_VALUE(pbody, '$.cancellationInfo.identificationNumber');
        v_datos_apertura.cod_tipo_id         := JSON_VALUE(pbody, '$.cancellationInfo.identificationType');
        ...
        END IF;
         IF v_datos_apertura.num_certificado IS NULL THEN
                    ...
                    ELSE
                    BEGIN
                        SELECT NUM_CERTIFICADO, 'DIGITAL_MONEYMARKET'
                        INTO pv_certificateNumber, pv_productType
                        FROM CD.CD_CERTIFICADO
                        WHERE COD_EMPRESA=v_datos_apertura.cod_empresa
                        AND NUM_CERTIFICADO=v_datos_apertura.num_certificado
                        AND cod_moneda=v_datos_apertura.cod_moneda
                        AND COD_PRODUCTO  IN (SELECT REGEXP_SUBSTR(List_producto, '[^,]+', 1, LEVEL) AS DATA
                             FROM DUAL
                             CONNECT BY REGEXP_SUBSTR(List_producto, '[^,]+', 1, LEVEL) IS NOT NULL);
                        EXCEPTION
                        WHEN NO_DATA_FOUND THEN
                        pv_productType:='';
                        pv_certificateNumber:='';
        ...
        END IF;
            ...
            CancellationCertificate(pv_companyId             => v_datos_apertura.cod_empresa,
                                   pn_customerId             => v_datos_apertura.cod_persona,
                                   pv_identificationNumber   => v_datos_apertura.num_id,
                                   pv_identificationType     => v_datos_apertura.cod_tipo_id,
                                   pv_certificateNumber      => v_datos_apertura.num_certificado,
                                   ...
                                   pn_accountNumber          => v_datos_apertura.num_cuenta,
                                   pv_cancellationType       => pv_cancellationType,
                                   pn_openAmount             => pn_openAmount,
                                   pn_interestAcumulated     => pn_interestAcumulated,
                                   pn_cancellationAmount     => pn_cancellationAmount,
                                   pn_penalAmount            => pn_penalAmount,
                                   pn_creditAccount          => pn_accountNumber,
                                   pd_cancellationDate       => dCancellationDate,
                                   pn_errornumber            => pn_errornumber,
                                   pv_errorMessage           => pv_errorMessage);               
              pv_cancellationDate:=TO_CHAR(dCancellationDate,'DD/MM/YYYY');       
              pn_creditAmount:=   pn_cancellationAmount;              
           END;
     ...
     BEGIN
       ...
       BEGIN
            ...
            BEGIN
                ...
                BEGIN
                    UPDATE PA.INFO_PROD_SOL SET MONTO_INICIAL=v_datos_apertura.monto_apertura, 
                                                COD_MONEDA=v_datos_apertura.cod_moneda
                    WHERE COD_PERSONA=v_datos_apertura.cod_persona
                    AND TIPO_PRODUCTO='CERTIFICADOS'
                    AND PROPOSITO='CERTIFICADO';
                        IF SQL%NOTFOUND THEN
                            BEGIN
                                ...
                                VALUES (v_datos_apertura.cod_persona,'CERTIFICADOS',v_datos_apertura.cod_moneda,'CERTIFICADO',v_datos_apertura.monto_apertura,'EFECTIVO','AHORROS',USER,SYSDATE);
                            END;
                        END IF;
                EXCEPTION
                WHEN OTHERS THEN
                 pv_errorMessage:='Ha ocurrido un eror registando el product info por favor intentar mas tarde.';
                END;
                COMMIT;
              END;
END;   
     ...
     /* IF dfechacalcd<>dfechacalcc THEN
      ...
      WHERE cef.cod_empresa     =   pv_companyId        AND 
          ...
          --cef.cod_cliente     =   pn_customerId       AND
            ...
            ppr.cod_producto    =  cef.cod_producto
            AND EXISTS
           (SELECT 1 FROM PA.CUENTA_CLIENTE_RELACION
           WHERE num_cuenta     =   pv_accountNumber 
            AND codigo_cliente  =   pn_customerId
            AND (tipo_relacion  =   'O' OR PRINCIPAL='S')
            UNION
            SELECT 1 FROM CC.CUENTA_EFECTIVO 
            WHERE NUM_CUENTA    =   pv_accountNumber
            AND COD_CLIENTE     =   pn_customerId);
            EXCEPTION
      WHEN NO_DATA_FOUND
         THEN
            nSaldoDisponible:= 0;
...
nmonto_total        NUMBER;
list_productos      VARCHAR2(500):=PA.PARAM.PARAMETRO_X_EMPRESA('1', 'CD_CERT_DIGITAL', 'CD');

BEGIN
pv_cancellationType     :='ATMATURITY';
pn_cancellationAmount   :=0;
pn_penalAmount          :=0;
pn_creditAccount         :=NULL;
 BEGIN
         SELECT fecha_hoy
           INTO dfechacalcd
           FROM pa.calendario_b2000
    ...
    SELECT DECODE(t2.ind_redencion_ant,'S','N','S'),
                ...
                t1.monto,t1.fec_ult_cap,DECODE(T1.TIP_CERTIFICADO,'CU',t1.num_cuenta,pn_accountNumber),t1.COD_AGENCIA
    INTO ves_penable, vpenalidad_renov, vcalcula_penalidad, nbase_calculo,vtip_certificado, 
     ...
     nmonto_apertura,dfechaUltcap, pn_creditAccount,vcodagencia
    ...
    AND t1.cliente          =   pn_customerId
    AND T1.ESTADO       =   'A'
    AND t2.cod_empresa  =   t1.cod_empresa AND
        t2.cod_producto =   t1.cod_producto
    AND t1.cod_producto IN (SELECT REGEXP_SUBSTR(list_productos, '[^,]+', 1, LEVEL) AS DATA
                             FROM DUAL
                             CONNECT BY REGEXP_SUBSTR(list_productos, '[^,]+', 1, LEVEL) IS NOT NULL);
    EXCEPTION
    WHEN NO_DATA_FOUND THEN
    ves_penable:='N';
    vcodagencia:=NULL;
    ...
    nInteresAcum:=0;
    pn_errorNumber:=415; 
    pv_errorMessage:='No existe un certificado digital registrado con el n¿mero '||pv_certificateNumber;
    RETURN;
    END;
                    ...
                    ninteresNetoGanado := ROUND (NVL (ninteresmes, 0.00), 2) -(ROUND (ROUND(NVL (ninteresmes, 0.00), 2) * (nporcentajeRenta/100), 2));
                     nmonto_total:=nvl(nmonto_apertura,0)+ nvl(ninteresNetoGanado,0);
                         BEGIN
                            ...
                            pn_creditAccount         :=pn_accountNumber;
                         END;    
              ELSIF vtip_certificado != 'CU' THEN    --si es capitalizable -- Capitalizable Tasa Variable / Tasa Fija

                 ninteresGanado := ROUND (NVL (ninteresAcum, 0.00) + nvl (ninteresMes, 0.00), 2);   
                 ninteresnetoGanado := (ROUND (NVL (ninteresMes, 0.00), 2)
                                  - ROUND (ROUND (NVL (ninteresMes, 0.00), 2) * (nporcentajeRenta/100), 2));
      ...
      --
           IF ((dfechacalcd - dfechaUltRenov) < 7)
            ...
            pv_cancellationType:='ATMATURITY';
            ELSE
            pv_cancellationType:='ANTICIPATED';
             END IF;
           END IF;
        END;
        IF (ves_penable = 'S') Then 
           IF (pv_cancellationType = 'ANTICIPATED')
           THEN
...
PROCEDURE  CancellationCertificate(pv_companyId             IN  NUMBER,
                            ...
                            IS
ves_penable         VARCHAR2(1):='';
pn_openingRate      NUMBER;
pn_idmovimiento     NUMBER;
vconsecutivo        NUMBER;
pv_rateCode         VARCHAR2(5):='';
vporcentaje_renta   NUMBER;
vpenalidad_renov    VARCHAR2(1):='';
vcalcula_penalidad  VARCHAR2(1):='';
nbase_calculo       NUMBER;
vtip_certificado    VARCHAR2(3):='';
...
nmonto_total        NUMBER;
List_producto       VARCHAR2(500):=PA.PARAM.PARAMETRO_X_EMPRESA('1', 'CD_CERT_DIGITAL', 'CD');
vcodsec_contable    PA.PERSONAS.COD_SEC_CONTABLE%TYPE;
vtitular            pa.personas.nombre%TYPE;
pv_productcode      pa.productos.cod_producto%TYPE;
pn_plazo            cd.cd_certificado.pla_dias%TYPE;
vcartera            cd.cd_cartera.cod_cartera%TYPE;
vcod_productacta    pa.productos.cod_producto%TYPE;
vcuentacontable     CG.cg_catalogo_x_empresa.cuenta_contable%TYPE;
vummovimientofuente NUMBER:=0;
vnummovtod          NUMBER:=0;
vrefautorizacion    NUMBER:=0;
vnumautorizacion    NUMBER:=0;
vnumasiento         NUMBER:=0;
vtransacc           VARCHAR2(5):='58';
vsubtransacc        VARCHAR2(5):='5';
vtransacd           VARCHAR2(5):='';
vsubtransacd        VARCHAR2(5):='';
nInteresMesNeto     NUMBER:=0;
nmonto_principal    NUMBER:=0;
nInteresMesRenta    NUMBER:=0;
nMontoRenta         NUMBER:=0;
nInteresCorriente   NUMBER:=0;
ninteresAcum        NUMBER:=0;
vcuenta_liqui_int   CG.CG_CATALOGO_X_EMPRESA.cuenta_contable%TYPE;
vcuenta_provi_int   CG.CG_CATALOGO_X_EMPRESA.cuenta_contable%TYPE;
vcuenta_renta_int   CG.CG_CATALOGO_X_EMPRESA.cuenta_contable%TYPE;
vcuenta_penalidad   CG.CG_CATALOGO_X_EMPRESA.cuenta_contable%TYPE;
...
pv_errorMessage         :='OK';
    BEGIN
      ...
      EXCEPTION
         ...
         THEN
            ...
            RETURN; 
      END;
    IF dfechacalcd IS NOT NULL THEN
    pd_cancellationDate:=dfechacalcd;
    END IF;

            BEGIN
                getInfoCancellationcd(pv_companyId             => pv_companyId,
                                       pn_customerId           => pn_customerId,
                                       pv_identificationNumber => pv_identificationNumber,
                                       pv_identificationType   => pv_identificationType,
                                       pv_certificateNumber    => pv_certificateNumber,
                                       pv_currencyCode         => pv_currencyCode,
                                       pn_accountNumber        => pn_accountNumber,
                                       pv_cancellationType     => pv_cancellationType,
                                       pn_cancellationAmount   => pn_cancellationAmount,
                                       pn_penalAmount          => pn_penalAmount,
                                       pn_creditAccount        => pn_creditAccount,
                                       pn_errornumber          => pn_errornumber,
                                       pv_errorMessage         => pv_errorMessage);
           END;
    IF pn_errornumber<>200 THEN
         RETURN;
     ELSE
     IF pv_cancellationType='ATMATURITY' THEN
            vtransacd:=ntiptra_canc_venc;
            vsubtransacd:=nsubtiptra_canc_venc;
            --var cancelacion a vencimiento CD
           -- vtransacc     :='4';
           -- vsubtransacc  :='1';
        ELSIF pv_cancellationType='ANTICIPATED' THEN
            vtransacd:=ntiptra_canc_anti;
            vsubtransacd:=nsubtiptra_canc_anti;
            --var cancelacion anticipada CD
            --vtransacc   :='4';
            --vsubtransacc :='2';
     END IF;
      pn_penalAmount:=nvl( pn_penalAmount,0);
    BEGIN
    SELECT      t1.cod_agencia,
                DECODE(t2.ind_redencion_ant,'S','N','S'),
                NVL(t2.ind_penalidad_renov,'N'),
                NVL(t2.ind_calcula_penalidad,'N'),
                t1.base_calculo, forma_calculo_interes,
                ROUND(t1.cre_mes,2) interescorriente,round(t1.mon_acum_int_cap,2) interesAcum,
                t1.monto,
                CASE WHEn NVL(cre_mes,0)>0 THEN ROUND(NVL(t1.cre_mes,0)*(NVL(t1.porcentaje_renta,0)/100),2) ELSE 0 END MontoRetencion,
                t1.fec_emision,
                t1.fec_ult_renov,t1.cod_producto,
                t1.monto,t1.fec_ult_cap,DECODE(T1.TIP_CERTIFICADO,'CU',t1.num_cuenta,pn_accountNumber),t1.pla_dias
    INTO vcodagencia,ves_penable, vpenalidad_renov, vcalcula_penalidad, nbase_calculo,vtip_certificado, 
     nInteresCorriente,nInteresAcum,pn_openAmount, nMontoRenta, dfechaEmision,dfechaUltRenov, 
     pv_productCode, nmonto_apertura,dfechaUltcap, pn_creditAccount,pn_plazo
    FROM cd.cd_certificado t1 , cd.cd_producto_x_empresa t2
    WHERE t1.cod_empresa    =   pv_companyid
    AND t1.num_certificado  =   pv_certificateNumber
    AND T1.cod_moneda       =   pv_currencyCode
    AND T1.ESTADO       =   'A'
    AND t2.cod_empresa  =   t1.cod_empresa AND
        t2.cod_producto =   t1.cod_producto
    AND t1.cod_producto IN (SELECT REGEXP_SUBSTR(List_producto, '[^,]+', 1, LEVEL) AS DATA
                             FROM DUAL
                             CONNECT BY REGEXP_SUBSTR(List_producto, '[^,]+', 1, LEVEL) IS NOT NULL);
    EXCEPTION
    WHEN NO_DATA_FOUND THEN
    ...
    nMontoRenta:=0;
    pn_errorNumber:=415; 
    pv_errorMessage:='No existe un numero de certificado digital registrado con el numero '||pv_certificateNumber;
    RETURN;
    END;
      pn_penalAmount:=NVL(nmonto_penalidad,0);
    pn_interestAcumulated:=nInteresAcum+nInteresCorriente-nMontoRenta;
    nmonto_total:=nmonto_apertura+NVL(pn_interestAcumulated,0)-NVL(nmonto_penalidad,0);
    nmonto_principal:=nmonto_apertura+nInteresAcum;
        BEGIN
        SELECT cod_producto
        INTO vcod_productacta
        FRoM cc.cuenta_efectivo
        WHERE cod_empresa='1'
        AND num_cuenta=pn_creditAccount
        AND ind_estado=1;
        EXCEPTION
        WHEN NO_DATA_FOUND THEN
        vcod_productacta:=NULL;
        pn_errorNumber:=416; 
        pv_errorMessage:='La cuenta no existe o no esta activa '||pn_creditAccount;
        RETURN;
        END;
      ...
      BEGIN
          SELECT pro.cod_cartera,car.cuenta_contable,cuenta_contable_interes
          INTO vcartera,vcuentacontable,vcuenta_liqui_int
          FROM cd.cd_cartera car, cd.cd_producto_x_empresa pro
          WHERE codigo_empresa          =   pv_companyId             AND
          (pn_plazo BETWEEN plazo_inicio AND plazo_fin       )       AND
          clasificacion                 =   'N'                      AND
          vencido                       =   'N'                      AND
          cod_sec_contable              =   vcodsec_contable         AND
          forma_pago_int                =   DECODE(pro.forma_calculo_interes,'CU','N','CV','C','C') AND
      ...
      WHEN NO_DATA_FOUND THEN
       pn_errornumber :=412;
       pv_errorMessage:='No existe la cartera, x ni los parametros contable para este producto con los terminos solitados';
       RETURN; 
      END; 
      cc.ccmov.agrega_movimiento (pv_companyId,
                                  vcodAgencia,
                                 'CC',
                                  ...
                                  vummovimientofuente,
                                  nmonto_total,
                                  'CANCELACION DE CERTIFICADO DIGITAL '||pv_certificateNumber,
                                  'CD',
                                  NULL,
                                  'N',
                                  vnummovtod,
    ...
    IF NVL(vnummovtod,0)= 0 THEN
         pn_errornumber :=413;
        pv_errorMessage:='El monto de cancelacion no pudo ser acreditado a la cuenta de efectivo especificada';
        ROlLBACK;
       RETURN; 
    END IF;
      dbms_output.put_line('mov. cuenta'||vnummovtod);
      ...
      aplica_contabilidad(pv_companyId,
                          ...
                          vrefautorizacion);
      END;
      IF pn_errornumber IS NOT NULL OR pv_errorMessage IS NOT NULL THEN
            ROLLBACK;
      END IF;
    ...
    cg_utl.lineas_del_asiento(pv_companyId,
                              ...
                              vsubtransacd,
                              vnummovtod,
                              'Cancelacion de Certificado Digital pago capital mas interes acumulado',
                              dfechacalcd,
                              dfechacalcd,
                              dfechacalcd,
                              vnumasiento, --nNumAsiento,
    ...
    IF NVL(ninteresCorriente,0) > 0 THEN
       nInteresMesNeto:=NVL(ninteresCorriente,0)-NVL(nMontoRenta,0);
    --pn_taxesAmount:=nInteresMesRenta;
    BEGIN
    cg_utl.lineas_del_asiento(pv_companyId,
                              vcodAgencia,
                              'BCD',
                              ...
                              vsubtransacd,
                              vnummovtod,
                              'Cancelaci¿n de Certificado Digital pago interes corriente',
                              dfechacalcd,
                              dfechacalcd,
                              dfechacalcd,
                              vnumasiento, --nNumAsiento,
    ...
    cg_utl.lineas_del_asiento(pv_companyId,
                              ...
                              vsubtransacd,
                              vnummovtod,
                              'Cancelaci¿n de Certificado Digital descuento Renta',
                              dfechacalcd,
                              dfechacalcd,
                              dfechacalcd,
                              vnumasiento, --nNumAsiento,
    ...
    IF NVL(pn_penalAmount,0) > 0 THEN
     ...
     INTO vcuenta_penalidad
     FROM CG.CG_CUENTAS_X_CONCEPTO_SECTOR
     WHERE codigo_aplicacion    =   'BCD' 
     AND codigo_sub_aplicacion  =   vcodproducto        AND
     cod_sec_contable           =   vcodsec_contable    AND
     concepto                   =   'PENALIDAD';
     EXCEPTION
     WHEN NO_DATA_FOUND THEN
      vcuenta_penalidad:=NULL;
     END;
    ...
    cg_utl.lineas_del_asiento(pv_companyId,
                              ...
                              vsubtransacd,
                              vnummovtod,
                              'Cancelaci¿n de Certificado Digital descuento penalidad por cancelacion anticipada',
                              dfechacalcd,
                              dfechacalcd,
                              dfechacalcd,
                              vnumasiento, --nNumAsiento,
    ...
    END IF;
        ...
        IF nMontoDiferencia=0 THEN
                ...
                BEGIN
                UPDATE cc.cuenta_efectivo
                SET sal_total_cta   =   sal_total_Cta+nvl(pn_cancellationAmount,0)
                WHERE cod_empresa   =   pv_companyId  AND
                      num_cuenta    =   pn_CreditAccount;
                END;
      ...
      BEGIN
       ...
       INSERT INTO cd.cd_movimiento (cod_empresa,
            ...
            VALUES (pv_companyId,
                    ...
                    vsubtransacd,
                    1,
                    'Cancelacion de Certificado Digital',
                    TO_CHAR(pn_cancellationAmount,'9,999,999,999.99'),
                    dfechacalcd,
                    TO_CHAR(pn_cancellationAmount,'9,999,999,999.99'),
                    USER,
                    -- p_cierre.dfechacal);
                    ...
                    pv_rateCode,
                    NULL,
                    'Cancelacion Certificado Digital'
                    );      
        EXCEPTION
         WHEN OTHERS
         THEN
