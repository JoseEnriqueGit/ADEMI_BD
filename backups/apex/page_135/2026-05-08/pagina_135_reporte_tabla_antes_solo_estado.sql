...
-- Estado: ANTES del cambio "vigencia solo por ESTADO"
-- Fecha: 2026-05-08
-- Snapshot identico al final de 2026-05-01 (con filtro de productos digitales).
...
LEFT JOIN PA.PERSONAS p
    ON cd.CLIENTE = p.COD_PERSONA
LEFT JOIN PA.PERSONAS_FISICAS pf
    ON p.COD_PER_FISICA = pf.COD_PER_FISICA
LEFT JOIN PA.AGENCIA ag
    ON cd.COD_AGENCIA = ag.COD_AGENCIA
   AND cd.COD_EMPRESA = ag.COD_EMPRESA
...
WHERE
    ...
    AND (
        ...
        OR (
            :P135_FILTRO_CARD = 'Certificados Vigentes'
            AND cd.ESTADO IN ('A', 'R')
            AND (cd.FEC_VENCIMIENTO >= TRUNC(SYSDATE) OR cd.FEC_VENCIMIENTO IS NULL)
        )
        OR (
            :P135_FILTRO_CARD = 'Certificados Vencidos'
            AND (
                cd.ESTADO IN ('C', 'P', 'N', 'I')
                OR (cd.FEC_VENCIMIENTO < TRUNC(SYSDATE) AND cd.FEC_VENCIMIENTO IS NOT NULL)
            )
        )
        OR (
            :P135_FILTRO_CARD = 'Clientes Externos'
            AND NOT EXISTS (
                SELECT 1
                FROM PA.EMPLEADOS e
                WHERE e.COD_PER_FISICA = cd.CLIENTE
                  AND e.ESTA_ACTIVO = 'S'
            )
        )
        OR (
            :P135_FILTRO_CARD = 'Empleados'
            AND EXISTS (
                SELECT 1
                FROM PA.EMPLEADOS e
                WHERE e.COD_PER_FISICA = cd.CLIENTE
        ...
        OR (
            :P135_FILTRO_CARD = 'Monto en Pesos'
            AND cd.COD_MONEDA = '1'
            AND cd.ESTADO IN ('A', 'R')
            AND (cd.FEC_VENCIMIENTO >= TRUNC(SYSDATE) OR cd.FEC_VENCIMIENTO IS NULL)
        )
        OR (
            :P135_FILTRO_CARD = 'Monto en Dólares'
            AND cd.COD_MONEDA = '2'
            AND cd.ESTADO IN ('A', 'R')
            AND (cd.FEC_VENCIMIENTO >= TRUNC(SYSDATE) OR cd.FEC_VENCIMIENTO IS NULL)
        )
