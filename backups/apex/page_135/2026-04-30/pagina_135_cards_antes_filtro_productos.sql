...
-- BLOQUE 1: TOTAL GENERAL
    SELECT 
        ...
        1 AS ORDER_COL,
        NULL AS VALOR_FILTRO  -- <--- Cambiamos 'TOTAL' por NULL
    ...
    FROM CertificadosBase 
    WHERE ESTADO IN ('A', 'R') AND (FEC_VENCIMIENTO >= TRUNC(SYSDATE) OR FEC_VENCIMIENTO IS NULL)
    ...
    -- BLOQUE 4: MONTOS VIGENTES PESOS (NO HACE CLIC)
    SELECT 
        'Monto en Pesos', 
    ...
    FROM CertificadosBase 
    WHERE COD_MONEDA = '1' AND ESTADO IN ('A', 'R') AND (FEC_VENCIMIENTO >= TRUNC(SYSDATE) OR FEC_VENCIMIENTO IS NULL)

    UNION ALL

    -- BLOQUE 5: MONTOS VIGENTES DOLARES (NO HACE CLIC)
    SELECT 
        'Monto en Dólares', 
        ...
        NULL AS VALOR_FILTRO
    FROM CertificadosBase 
    WHERE COD_MONEDA = '2' AND ESTADO IN ('A', 'R') AND (FEC_VENCIMIENTO >= TRUNC(SYSDATE) OR FEC_VENCIMIENTO IS NULL)

    UNION ALL

    -- BLOQUE 6: TIPOS DE CLIENTE (EXTERNOS)
    SELECT 
        'Clientes Externos', 
        TO_CHAR(COUNT(*), 'FM999,999,999,990'), 
    ...
    -- BLOQUE 7: TIPOS DE CLIENTE (EMPLEADOS)
    SELECT 
        'Empleados', 
        TO_CHAR(COUNT(*), 'FM999,999,999,990'), 
        'Cantidad Certificados', 
    ...
    -- BLOQUE 8: MONTO CANCELADOS PESOS (NO HACE CLIC)
    SELECT 
    ...
    FROM CertificadosBase 
    WHERE COD_MONEDA = '1' 
      AND (ESTADO IN ('C', 'P', 'N', 'I') OR (ESTADO IN ('A', 'R') AND FEC_VENCIMIENTO < TRUNC(SYSDATE)))

    UNION ALL

    -- BLOQUE 9: MONTO CANCELADOS DOLARES (NO HACE CLIC)
    SELECT 
...
SELECT 
    ...
    ORDER_COL,
    VALOR_FILTRO,
    CASE WHEN VALOR_FILTRO IS NOT NULL THEN 'pointer' ELSE 'default' END AS CSS_CURSOR
FROM Tarjetas
ORDER BY ORDER_COL;
