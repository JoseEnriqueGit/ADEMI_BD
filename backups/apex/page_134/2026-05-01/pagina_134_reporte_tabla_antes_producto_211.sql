SELECT
    ...
    END AS TIPO_CLIENTE,
    (SELECT NUM_ID FROM PA.ID_PERSONAS idp WHERE idp.COD_PERSONA = ce.COD_CLIENTE AND ROWNUM = 1) AS IDENTIFICACION,
    CASE
    ...
    CASE
        WHEN EXISTS (
            SELECT 1 FROM CC.CUENTA_EFECTIVO c2 WHERE c2.COD_CLIENTE = ce.COD_CLIENTE AND c2.NUM_CUENTA <> ce.NUM_CUENTA
            UNION ALL
            SELECT 1 FROM PR.PR_CREDITOS pr WHERE TO_CHAR(pr.CODIGO_CLIENTE) = ce.COD_CLIENTE
            UNION ALL
            SELECT 1 FROM CD.CD_CERTIFICADO cd WHERE cd.CLIENTE = ce.COD_CLIENTE
            UNION ALL
            SELECT 1 FROM TC.TC_TARJETAS tc WHERE tc.COD_CLIENTE = ce.COD_CLIENTE
        ) THEN 'CLIENTE EXISTENTE'
        ELSE 'CLIENTE NUEVO'
    END AS TIPO_VINCULACION
FROM
    CC.CUENTA_EFECTIVO ce
LEFT JOIN PA.PRODUCTOS prod ON ce.COD_PRODUCTO = prod.COD_PRODUCTO AND ce.COD_EMPRESA = prod.COD_EMPRESA
LEFT JOIN PA.PERSONAS p ON ce.COD_CLIENTE = p.COD_PERSONA
LEFT JOIN PA.PERSONAS_FISICAS pf ON p.COD_PER_FISICA = pf.COD_PER_FISICA
LEFT JOIN PA.AGENCIA ag ON ce.COD_AGENCIA = ag.COD_AGENCIA AND ce.COD_EMPRESA = ag.COD_EMPRESA
...
WHERE
    ...
    AND (
        (:P134_LOGICA_FECHA = 'AND'
            ...
            AND (ce.FEC_CANCELAC IS NOT NULL AND TRUNC(ce.FEC_CANCELAC) BETWEEN TO_DATE(:P134_FROM_CANC, 'DD-MM-YYYY') AND TO_DATE(:P134_TO_CANC, 'DD-MM-YYYY'))
        )
        OR
    ...
    AND (
        ...
        OR (:P134_FILTRO_CARD = 'Total cuentas activas' AND ce.FEC_CANCELAC IS NULL)
        OR (:P134_FILTRO_CARD = 'Total cuentas activas empleados' AND ce.FEC_CANCELAC IS NULL AND EXISTS (SELECT 1 FROM PA.EMPLEADOS e WHERE e.COD_PER_FISICA = ce.COD_CLIENTE AND e.ESTA_ACTIVO = 'S'))
        OR (:P134_FILTRO_CARD = 'Total cuentas activas cliente' AND ce.FEC_CANCELAC IS NULL AND NOT EXISTS (SELECT 1 FROM PA.EMPLEADOS e WHERE e.COD_PER_FISICA = ce.COD_CLIENTE AND e.ESTA_ACTIVO = 'S'))
        OR (:P134_FILTRO_CARD = 'Total Cuentas Canceladas' AND ce.FEC_CANCELAC IS NOT NULL)
    )
ORDER BY
