...
--            UPDATE PR.PR_REPRESTAMOS
--            SET ESTADO = 'CHCH'
--            WHERE ID_REPRESTAMO = v_candidates(i).id_represtamo;
...
PROCEDURE P_ANALIZAR_RESULTADOS_CAMPANA IS
BEGIN
    MERGE INTO PR.PR_CHAMPION_CHALLENGE_LOG log
    USING (
        SELECT
            lg.id_represtamo,
            lg.cod_cliente,
            lg.grupo_asignado,
            r.fecha_proceso AS fecha_inicio_oferta,
            R.no_credito,
            -- Evidencia de desembolso digital (estado 'CRY' en bitácora)
            (SELECT 'S' FROM DUAL WHERE EXISTS (
                SELECT 1 FROM pr_bitacora_represtamo WHERE id_represtamo = lg.id_represtamo AND CODIGO_ESTADO IN('CRY', 'CRD')
            )) AS evidencia_digital,
            -- Buscamos un crédito NUEVO para ese cliente, desembolsado en los 30 días siguientes a la oferta
            (SELECT cred.no_credito FROM pr_creditos cred
             WHERE cred.codigo_cliente = lg.cod_cliente
               AND cred.estado = 'D'
               AND cred.f_primer_desembolso BETWEEN r.fecha_proceso AND r.fecha_proceso + 30
               AND cred.no_credito != r.no_credito
               AND ROWNUM = 1
            ) AS no_credito_desembolsado,
            (SELECT cred.monto_desembolsado FROM pr_creditos cred
             WHERE cred.codigo_cliente = lg.cod_cliente AND cred.estado = 'D'
               AND cred.f_primer_desembolso BETWEEN r.fecha_proceso AND r.fecha_proceso + 30
               AND cred.no_credito != r.no_credito AND ROWNUM = 1
            ) AS monto_desembolsado_real,
            (SELECT cred.f_primer_desembolso FROM pr_creditos cred
             WHERE cred.codigo_cliente = lg.cod_cliente AND cred.estado = 'D'
               AND cred.f_primer_desembolso BETWEEN r.fecha_proceso AND r.fecha_proceso + 30
               AND cred.no_credito != r.no_credito AND ROWNUM = 1
            ) AS fecha_desembolso_real
        FROM
            PR.PR_CHAMPION_CHALLENGE_LOG lg
        JOIN
            PR.PR_REPRESTAMOS r ON lg.id_represtamo = r.id_represtamo
        WHERE
            lg.tipo_desembolso IS NULL
    ) src
    ON (log.id_represtamo = src.id_represtamo)
    WHEN MATCHED THEN
        UPDATE SET
            log.es_crossover =
                CASE
                    WHEN src.grupo_asignado = 'CHAMPION' AND src.evidencia_digital IS NULL AND src.no_credito_desembolsado IS NOT NULL THEN 'S'
                    ELSE 'N'
                END,
            log.tipo_desembolso =
                CASE
                    WHEN src.evidencia_digital = 'S' THEN 'DIGITAL'
                    WHEN src.no_credito_desembolsado IS NOT NULL THEN 'TRADICIONAL'
                    ELSE log.tipo_desembolso
                END,
            log.no_credito_nuevo_core = NVL(log.no_credito_nuevo_core, src.no_credito_desembolsado),
            log.monto_preaprobado = src.monto_desembolsado_real,
            log.fecha_desembolso_core = src.fecha_desembolso_real;

    FOR rec IN (
        SELECT id_represtamo FROM PR.PR_CHAMPION_CHALLENGE_LOG
        WHERE es_crossover = 'S' AND tipo_desembolso = 'TRADICIONAL'
    ) LOOP
        P_Generar_Bitacora(
            pIdReprestamo  => rec.id_represtamo,
            pEstado        => 'CHCHA',
            pObservaciones => 'Crossover: Desembolso detectado por canal tradicional.',
            pUsuario       => 'JOB_ANALYZE_CAMPAIGN',
            pCanal         => NULL,
            pStep          => NULL
        );
    END LOOP;
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
END P_ANALIZAR_RESULTADOS_CAMPANA;
